...
import com.android.fastergallery.data.DecodeUtils;
...
import com.android.fasterphotos.data.Pools.SimplePool;
import com.android.fasterphotos.data.Pools.SynchronizedPool;
...
public class MediaCacheUtils {
	...
	private static final SimplePool<byte[]> mBufferPool = new SynchronizedPool<byte[]>(
			5);
	...
	public static void initialize(Context context) {
		...
		mTargetThumbnailSize = resources
				.getDimensionPixelSize(R.dimen.size_thumbnail);
		mTargetPreviewSize = resources
				.getDimensionPixelSize(R.dimen.size_preview);
	}

	public static int getTargetSize(MediaSize size) {
		return (size == MediaSize.Thumbnail) ? mTargetThumbnailSize
				: mTargetPreviewSize;
	}

	public static boolean downsample(File inBitmap, MediaSize targetSize,
			File outBitmap) {
		...
		Bitmap bitmap = DecodeUtils.decodeThumbnail(sJobStub,
				inBitmap.getPath(), options, size, MediaItem.TYPE_THUMBNAIL);
	...
	public static boolean downsample(Bitmap inBitmap, MediaSize size,
			File outBitmap) {
