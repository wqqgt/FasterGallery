...
class MediaCacheDatabase extends SQLiteOpenHelper {
	...
	private static final String[] PROJECTION_ID = { Columns._ID, };

	private static final String[] PROJECTION_CACHED = { Columns._ID,
			Columns.MEDIA_SIZE, Columns.SIZE_IN_BYTES, };

	private static final String[] PROJECTION_CACHE_SIZE = { "SUM("
			+ Columns.SIZE_IN_BYTES + ")" };

	private static final String[] PROJECTION_DELETE_OLD = { Columns._ID,
			Columns.URI, Columns.MEDIA_SIZE, Columns.SIZE_IN_BYTES,
			Columns.LAST_ACCESS, };

	public static final String CREATE_TABLE = "CREATE TABLE " + TABLE + "("
			+ Columns._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
			+ Columns.URI + " TEXT NOT NULL," + Columns.MEDIA_SIZE
			+ " INTEGER NOT NULL," + Columns.LAST_ACCESS + " INTEGER NOT NULL,"
			+ Columns.SIZE_IN_BYTES + " INTEGER NOT NULL," + "UNIQUE("
			+ Columns.URI + ", " + Columns.MEDIA_SIZE + "))";
	...
	public static final String WHERE_CLEAR_CACHE_LARGE = WHERE_CLEAR_CACHE
			+ " AND " + WHERE_NOT_THUMBNAIL;
	...
	public Long getCached(Uri uri, MediaSize size) {
		...
		String[] whereArgs = { uri.toString(), String.valueOf(size.getValue()), };
		Cursor cursor = db.query(TABLE, PROJECTION_ID, where, whereArgs, null,
				null, null);
		Long id = null;
	...
	public MediaSize executeOnBestCached(Uri uri, MediaSize size, Action action) {
		...
		String[] whereArgs = { uri.toString(), String.valueOf(size.getValue()), };
		Cursor cursor = db.query(TABLE, PROJECTION_CACHED, where, whereArgs,
				null, null, orderBy);
	...
	public void deleteOldCached(boolean includeThumbnails, long deleteSize,
			Action action) {
		...
		db.beginTransaction();
		try {
			Cursor cursor = db.query(TABLE, PROJECTION_DELETE_OLD, where, null,
					null, null, Columns.LAST_ACCESS);
			while (cursor.moveToNext()) {
				long id = cursor.getLong(0);
			...
			cursor.close();
			String[] whereArgs = { String.valueOf(lastAccess), };
			String whereDelete = includeThumbnails ? WHERE_CLEAR_CACHE
					: WHERE_CLEAR_CACHE_LARGE;
	...
	private long getCacheSize(String where) {
		SQLiteDatabase db = getReadableDatabase();
		Cursor cursor = db.query(TABLE, PROJECTION_CACHE_SIZE, where, null,
				null, null, null);
		long size = -1;
		if (cursor.moveToNext()) {
