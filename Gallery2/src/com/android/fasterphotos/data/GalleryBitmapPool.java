...
import com.android.fasterphotos.data.Pools.Pool;
import com.android.fasterphotos.data.Pools.SynchronizedPool;
import com.android.fasterphotos.data.SparseArrayBitmapPool.Node;
...
public class GalleryBitmapPool {
	...
	private GalleryBitmapPool(int capacityBytes) {
		...
		mPools[POOL_INDEX_SQUARE] = new SparseArrayBitmapPool(
	...
	private static GalleryBitmapPool sInstance = new GalleryBitmapPool(
			CAPACITY_BYTES);
	...
	public boolean put(Bitmap b) {
		...
		SparseArrayBitmapPool pool = getPoolForDimensions(b.getWidth(),
				b.getHeight());
		if (pool == null) {
			b.recycle();
			return false;
