...
public class FilterUtils {
	...
	public static void setupMenuItems(GalleryActionBar actionBar, Path path,
			boolean inAlbum) {
		...
		actionBar.setClusterItemVisibility(CLUSTER_BY_ALBUM, !inAlbum
				|| ctype == 0);
		...
		setMenuItemAppliedEnabled(actionBar, R.string.show_all, ftype == 0,
				ftype != 0 && ftypef == 0, fcurrent == 0);
	}
	...
	private static void setMenuItemApplied(GalleryActionBar model, int id,
			boolean applied, boolean updateTitle) {
		model.setClusterItemEnabled(id, !applied);
	}

	private static void setMenuItemAppliedEnabled(GalleryActionBar model,
			int id, boolean applied, boolean enabled, boolean updateTitle) {
