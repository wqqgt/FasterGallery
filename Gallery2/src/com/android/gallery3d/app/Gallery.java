...
public final class Gallery extends AbstractGalleryActivity implements
	...
	private String getContentType(Intent intent) {
		...
		if (type != null) {
			return GalleryUtils.MIME_TYPE_PANORAMA360.equals(type) ? MediaItem.MIME_TYPE_JPEG
					: type;
		}
		...
		if (slideshow) {
			...
			if (uri == null) {
				...
				if (!singleItemOnly) {
					...
					// fired
					// from notification), back button should behave the same as
					// up button
					// rather than taking users back to the home screen
					if (intent.getBooleanExtra(PhotoPage.KEY_TREAT_BACK_AS_UP,
							false)
	...
	public boolean onGenericMotionEvent(MotionEvent event) {
		...
		if (isTouchPad) {
			float maxX = event.getDevice().getMotionRange(MotionEvent.AXIS_X)
					.getMax();
			float maxY = event.getDevice().getMotionRange(MotionEvent.AXIS_Y)
					.getMax();
			...
			MotionEvent touchEvent = MotionEvent.obtain(event.getDownTime(),
					event.getEventTime(), event.getAction(), x, y,
					event.getMetaState());
