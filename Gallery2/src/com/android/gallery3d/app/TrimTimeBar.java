...
/**
 * The trim time bar view, which includes the current and total time, the
 * progress bar, and the scrubbers for current time, start and end time for
 * trimming.
 */
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mShowTimes) {
			canvas.drawText(stringForTime(mCurrentTime), mTimeBounds.width()
					/ 2 + getPaddingLeft(), mTimeBounds.height() / 2
					+ mTrimStartScrubberTop, mTimeTextPaint);
