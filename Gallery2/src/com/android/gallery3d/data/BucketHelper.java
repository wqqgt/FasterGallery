...
class BucketHelper {
	...
	private static final String[] PROJECTION_BUCKET_IN_ONE_TABLE = {
			ImageColumns.BUCKET_ID, "MAX(datetaken)",
	...
	// When query from the Images or Video tables, we only need to group by
	// BUCKET_ID.
	private static final String BUCKET_GROUP_BY_IN_ONE_TABLE = "1) GROUP BY (1";

	public static BucketEntry[] loadBucketEntries(JobContext jc,
	...
	private static void updateBucketEntriesFromTable(JobContext jc,
			ContentResolver resolver, Uri tableUri,
			HashMap<Integer, BucketEntry> buckets) {
		Cursor cursor = resolver.query(tableUri,
				PROJECTION_BUCKET_IN_ONE_TABLE, BUCKET_GROUP_BY_IN_ONE_TABLE,
				null, null);
		if (cursor == null) {
		...
		try {
			while (cursor.moveToNext()) {
				...
				if (entry == null) {
					entry = new BucketEntry(bucketId,
	...
	private static BucketEntry[] loadBucketEntriesFromImagesAndVideoTable(
		...
		HashMap<Integer, BucketEntry> buckets = new HashMap<Integer, BucketEntry>(
				64);
		if ((type & MediaObject.MEDIA_TYPE_IMAGE) != 0) {
			updateBucketEntriesFromTable(jc, resolver,
					Images.Media.EXTERNAL_CONTENT_URI, buckets);
		}
		if ((type & MediaObject.MEDIA_TYPE_VIDEO) != 0) {
			updateBucketEntriesFromTable(jc, resolver,
					Video.Media.EXTERNAL_CONTENT_URI, buckets);
		}
		BucketEntry[] entries = buckets.values().toArray(
				new BucketEntry[buckets.size()]);
	...
	private static BucketEntry[] loadBucketEntriesFromFilesTable(JobContext jc,
			ContentResolver resolver, int type) {
		Uri uri = getFilesContentUri();

		Cursor cursor = resolver.query(uri, PROJECTION_BUCKET, BUCKET_GROUP_BY,
	...
	private static String getBucketNameInTable(ContentResolver resolver,
			Uri tableUri, int bucketId) {
		String selectionArgs[] = new String[] { String.valueOf(bucketId) };
		Uri uri = tableUri.buildUpon().appendQueryParameter("limit", "1")
				.build();
	...
	public static String getBucketName(ContentResolver resolver, int bucketId) {
		if (ApiHelper.HAS_MEDIA_PROVIDER_FILES_TABLE) {
			String result = getBucketNameInTable(resolver,
					getFilesContentUri(), bucketId);
			return result == null ? "" : result;
		} else {
			String result = getBucketNameInTable(resolver,
					Images.Media.EXTERNAL_CONTENT_URI, bucketId);
			if (result != null)
				return result;
			result = getBucketNameInTable(resolver,
					Video.Media.EXTERNAL_CONTENT_URI, bucketId);
			return result == null ? "" : result;
		}
	}
