...
public class TimeClustering extends Clustering {
	...
	private long mLargeClusterSplitTime = mClusterSplitTime
			/ PARTITION_CLUSTER_SPLIT_TIME_FACTOR;
	...
	private void setTimeRange(long timeRange, int numItems) {
		...
		mClusterSplitTime = Utils.clamp(mClusterSplitTime,
				MIN_CLUSTER_SPLIT_TIME_IN_MS, MAX_CLUSTER_SPLIT_TIME_IN_MS);
		mLargeClusterSplitTime = mClusterSplitTime
				/ PARTITION_CLUSTER_SPLIT_TIME_FACTOR;
		mMinClusterSize = Utils.clamp(mMinClusterSize, MIN_MIN_CLUSTER_SIZE,
			...
			if (numCurrClusterItems == 0) {
				...
				if (isGeographicallySeparated(prevItem, currentItem)) {
					...
					itemAddedToCurrentCluster = true;
						...
						&& numCurrClusterItems < mMinClusterSize
	...
	private int getPartitionIndexForCurrentCluster() {
		...
		if (numCurrClusterItems > minClusterSize + 1) {
			for (int i = minClusterSize; i < numCurrClusterItems
					- minClusterSize; i++) {
				...
				float change = Math.max(diff1 / (diff2 + 0.01f), diff2
						/ (diff1 + 0.01f));
	...
	private static boolean isGeographicallySeparated(SmallItem itemA,
			SmallItem itemB) {
		...
		double distance = GalleryUtils.fastDistanceMeters(
				Math.toRadians(itemA.lat), Math.toRadians(itemA.lng),
				Math.toRadians(itemB.lat), Math.toRadians(itemB.lng));
	...
	public String generateCaption(Context context) {
		...
		if (minDay.substring(4).equals(maxDay.substring(4))) {
			...
			if (minDay.equals(maxDay)) {
				...
				String dateRangeWithYear = DateUtils.formatDateTime(context,
						minTimestamp, flags | DateUtils.FORMAT_SHOW_YEAR);
