...
public class FilterDeleteSet extends MediaSet implements ContentListener {
	...
	public ArrayList<MediaItem> getMediaItem(int start, int count) {
		...
		ArrayList<MediaItem> base = mBaseSet.getMediaItem(start + i, count
				+ (j - i));
	...
	// We apply the pending requests in the mRequests to construct mCurrent in
	// reload().
	@Override
	public long reload() {
		...
		if (!mCurrent.isEmpty()) {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (item == null)
					continue;
				Path p = item.getPath();
				// Find the matching path in mCurrent, if found move it to
				// result
