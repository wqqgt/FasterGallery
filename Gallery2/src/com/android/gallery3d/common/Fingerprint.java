...
public class Fingerprint {
	...
	/**
	 * Name of the digest algorithm we use in
	 ...
	 * 
	 * @param byteCount
	 *            length of original data will be stored at byteCount[0] as a
	 *            side product of the fingerprint calculation
	 */
	public static Fingerprint fromInputStream(InputStream stream,
	...
	public static Fingerprint fromStreamId(String streamId) {
		if ((streamId == null) || !streamId.startsWith(STREAM_ID_CS_PREFIX)
		...
		for (int idx = STREAM_ID_CS_PREFIX.length(); idx < STREAM_ID_CS_01_LENGTH; idx += 2) {
			int value = (toDigit(streamId, idx) << 4)
					| toDigit(streamId, idx + 1);
	 ...
	 * 
	 * @param streamIdList
	 *            list of stream id's to be scanned
	 ...
	 * 
	 * Stream id string is limited to 40 characters, which could be digits,
	 * lower case ASCII and underscores.
	 */
	...
	private static int toDigit(String streamId, int index) {
		...
		if (digit < 0) {
			throw new IllegalArgumentException("illegal hex digit in "
					+ streamId);
		}
		return digit;
	}
