...
public class OverScroller {
	...
	public void fling(int startX, int startY, int velocityX, int velocityY,
		...
		fling(startX, startY, velocityX, velocityY, minX, maxX, minY, maxY, 0,
				0);
	}
	...
	public int timePassed() {
		...
		final long startTime = Math.min(mScrollerX.mStartTime,
				mScrollerY.mStartTime);
	...
	static class SplineOverScroller {
		...
		static {
			...
			for (int i = 0; i < NB_SAMPLES; i++) {
				...
				SPLINE_POSITION[i] = coef * ((1.0f - x) * START_TENSION + x)
						+ x * x * x;
		...
		private void adjustDuration(int start, int oldFinal, int newFinal) {
			...
			if (index < NB_SAMPLES) {
				...
				final float timeCoef = t_inf + (x - x_inf) / (x_sup - x_inf)
						* (t_sup - t_inf);
				mDuration *= timeCoef;
			}
		}
		...
		private double getSplineFlingDistance(int velocity) {
			...
			return mFlingFriction * PHYSICAL_COEF
		...
		private void fitOnBounceCurve(int start, int end, int velocity) {
			...
			final float distanceToApex = velocity * velocity / 2.0f
					/ Math.abs(mDeceleration);
			final float distanceToEdge = Math.abs(end - start);
			final float totalDuration = (float) Math.sqrt(2.0
		...
		private void startBounceAfterEdge(int start, int end, int velocity) {
			mDeceleration = getDeceleration(velocity == 0 ? start - end
					: velocity);
		...
		private void startAfterEdge(int start, int min, int max, int velocity) {
			if (start > min && start < max) {
				Log.e("OverScroller",
						"startAfterEdge called from a valid position");
				mFinished = true;
				return;
			}
			...
			if (keepIncreasing) {
				...
				if (totalDistance > Math.abs(overDistance)) {
					fling(start, velocity, positive ? min : start,
							positive ? start : max, mOver);
				} else {
		...
		private void onEdgeReached() {
			...
			float distance = mVelocity * mVelocity
					/ (2.0f * Math.abs(mDeceleration));
		...
		boolean update() {
			...
			case SPLINE: {
				...
				mCurrVelocity = velocityCoef * mSplineDistance
						/ mSplineDuration * 1000.0f;
				break;
			}
