...
/**
 * Helper class to execute an AsyncTask in parallel if SDK version is 11 or
 * newer.
 */
public class AsyncTaskUtil {
	...
	static {
		if (Build.VERSION.SDK_INT >= 11) {
			try {
				sExecutor = (Executor) AsyncTask.class.getField(
						"THREAD_POOL_EXECUTOR").get(null);
	...
	public static <Param> void executeInParallel(AsyncTask<Param, ?, ?> task,
			Param... params) {
		if (Build.VERSION.SDK_INT < 11) {
			task.execute(params);
		} else {
