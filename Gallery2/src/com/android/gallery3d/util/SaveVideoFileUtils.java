...
public class SaveVideoFileUtils {
	...
	// This function can decide which folder to save the video file, and
	// generate
	...
	public static SaveVideoFileInfo getDstMp4FileInfo(String fileNameFormat,
		...
		if ((dstFileInfo.mDirectory == null)
				|| !dstFileInfo.mDirectory.canWrite()) {
			dstFileInfo.mDirectory = new File(
		...
		dstFileInfo.mFileName = new SimpleDateFormat(fileNameFormat)
				.format(new Date(System.currentTimeMillis()));

		dstFileInfo.mFile = new File(dstFileInfo.mDirectory,
				dstFileInfo.mFileName + ".mp4");
		return dstFileInfo;
	}
	...
	private static File getSaveDirectory(ContentResolver contentResolver,
			Uri uri) {
		final File[] dir = new File[1];
		querySource(contentResolver, uri, new String[] { VideoColumns.DATA },
	...
	public static Uri insertContent(SaveVideoFileInfo mDstFileInfo,
		...
		String[] projection = new String[] { VideoColumns.DATE_TAKEN,
