...
public class MtpPagerAdapter extends PagerAdapter {
	...
	public int translatePositionWithLabels(int position) {
		if (mModel == null)
			return -1;
		return mModel
				.getPositionWithoutLabelsFromPosition(position, mSortOrder);
	}
	...
	public Object instantiateItem(ViewGroup container, int position) {
		...
		if (mReusableView != null) {
			...
			v = (MtpFullscreenView) mInflater.inflate(R.layout.ingest_fullsize,
					container, false);
		}
		MtpObjectInfo i = mModel.getWithoutLabels(position, mSortOrder);
		v.getImageView().setMtpDeviceAndObjectInfo(mModel.getDevice(), i,
				mGeneration);
