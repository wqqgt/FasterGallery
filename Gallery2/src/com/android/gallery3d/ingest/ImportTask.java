...
public class ImportTask implements Runnable {
	...
	public interface Listener {
		void onImportProgress(int visitedCount, int totalCount,
	...
	public ImportTask(MtpDevice device,
			Collection<MtpObjectInfo> objectsToImport, String destAlbumName,
			Context context) {
		...
		try {
			...
			for (MtpObjectInfo object : mObjectsToImport) {
				...
				if (GalleryUtils.hasSpaceForSize(object.getCompressedSize())) {
					importedPath = new File(dest, object.getName())
							.getAbsolutePath();
					if (!mDevice.importFile(object.getObjectHandle(),
							importedPath)) {
						importedPath = null;
					}
				}
