...
public class IngestService extends Service implements ImportTask.Listener,
	...
	public void onCreate() {
		...
		mNotificationBuilder.setSmallIcon(android.R.drawable.stat_notify_sync) // TODO
																				// drawable
				.setContentIntent(
						PendingIntent.getActivity(this, 0, new Intent(this,
								IngestActivity.class), 0));
	...
	protected void setClientActivity(IngestActivity activity) {
		...
		if (mClientActivity == null) {
			if (mNeedRelaunchNotification) {
				mNotificationBuilder.setProgress(0, 0, false).setContentText(
						getResources().getText(R.string.ingest_scanning_done));
				mNotificationManager.notify(
						NotificationIds.INGEST_NOTIFICATION_SCANNING,
						mNotificationBuilder.build());
			}
			return;
		}
		mNotificationManager
				.cancel(NotificationIds.INGEST_NOTIFICATION_IMPORTING);
		mNotificationManager
				.cancel(NotificationIds.INGEST_NOTIFICATION_SCANNING);
	...
	protected void importSelectedItems(SparseBooleanArray selected,
			Adapter adapter) {
		...
		ImportTask task = new ImportTask(mDevice, importHandles,
				BucketNames.IMPORTED, this);
		task.setListener(this);
		mNotificationBuilder.setProgress(0, 0, true).setContentText(
				getResources().getText(R.string.ingest_importing));
	...
	public void onImportProgress(int visitedCount, int totalCount,
		...
		mNeedRelaunchNotification = false;
		if (mClientActivity != null) {
			mClientActivity.onImportProgress(visitedCount, totalCount,
					pathIfSuccessful);
		}
		mNotificationBuilder.setProgress(totalCount, visitedCount, false)
				.setContentText(
						getResources().getText(R.string.ingest_importing));
		mNotificationManager.notify(
				NotificationIds.INGEST_NOTIFICATION_IMPORTING,
				mNotificationBuilder.build());
	}
	...
	public void onImportFinish(Collection<MtpObjectInfo> objectsNotImported,
		...
		if (mClientActivity != null) {
			...
			mNotificationBuilder.setProgress(0, 0, false).setContentText(
					getResources().getText(R.string.import_complete));
			mNotificationManager.notify(
					NotificationIds.INGEST_NOTIFICATION_IMPORTING,
					mNotificationBuilder.build());
		}
	...
	public void onObjectIndexed(MtpObjectInfo object, int numVisited) {
		...
		if (mClientActivity != null) {
			...
			// Throttle the updates to one every PROGRESS_UPDATE_INTERVAL_MS
			// milliseconds
			long currentTime = SystemClock.uptimeMillis();
			if (currentTime > mLastProgressIndexTime
					+ PROGRESS_UPDATE_INTERVAL_MS) {
				mLastProgressIndexTime = currentTime;
				mNotificationBuilder.setProgress(0, numVisited, true)
						.setContentText(
								getResources()
										.getText(R.string.ingest_scanning));
				mNotificationManager.notify(
						NotificationIds.INGEST_NOTIFICATION_SCANNING,
						mNotificationBuilder.build());
			}
	...
	private static final class ScannerClient implements
