...
public class MtpBitmapFetch {
	...
	public static Bitmap getThumbnail(MtpDevice device, MtpObjectInfo info) {
		...
		o.inBitmap = GalleryBitmapPool.getInstance().get(o.outWidth,
				o.outHeight);
		o.inMutable = true;
	...
	public static BitmapWithMetadata getFullsize(MtpDevice device,
			MtpObjectInfo info) {
		return getFullsize(device, info, sMaxSize);
	}

	public static BitmapWithMetadata getFullsize(MtpDevice device,
			MtpObjectInfo info, int maxSide) {
		byte[] imageBytes = device.getObject(info.getObjectHandle(),
		...
		if (maxSide > 0) {
			...
			o.inJustDecodeBounds = false;
			created = BitmapFactory.decodeByteArray(imageBytes, 0,
					imageBytes.length, o);
		} else {
			created = BitmapFactory.decodeByteArray(imageBytes, 0,
					imageBytes.length);
