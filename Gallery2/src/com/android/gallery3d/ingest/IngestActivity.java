...
public class IngestActivity extends Activity implements
	...
	private OnItemClickListener mOnItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> adapterView, View itemView,
			...
			mGridView.setItemChecked(position, !mGridView
	...
	private MultiChoiceModeListener mMultiChoiceModeListener = new MultiChoiceModeListener() {
		...
		public void onItemCheckedStateChanged(ActionMode mode, int position,
			...
			if (mAdapter.itemAtPositionIsBucket(position)) {
				SparseBooleanArray checkedItems = mGridView
				...
				int nextSectionStart = mAdapter.getPositionForSection(mAdapter
						.getSectionForPosition(position) + 1);
				if (nextSectionStart == position)
					nextSectionStart = mAdapter.getCount();

				boolean rangeValue = false; // Value we want to set all of the
											// bucket items to

				// Determine if all the items in the bucket are currently
				// checked, so that we
				// can uncheck them, otherwise we will check all items in the
				// bucket.
		...
		case R.id.import_items:
			if (mActiveActionMode != null) {
				mHelperService.importSelectedItems(
						mGridView.getCheckedItemPositions(), mAdapter);
	...
	private void showWarningView(int textResId) {
		if (mWarningView == null) {
			...
			mWarningText = (TextView) mWarningView
					.findViewById(R.id.ingest_warning_view_text);
		}
	...
	private class PositionMappingCheckBroker extends CheckBroker implements
			OnClearChoicesListener {
		...
		private int mapPagerToGridPosition(int position) {
			if (position != mLastMappingPager) {
				...
				mLastMappingGrid = mAdapter
						.translatePositionWithoutLabels(position);
			}
		...
		private int mapGridToPagerPosition(int position) {
			if (position != mLastMappingGrid) {
				...
				mLastMappingPager = mPagerAdapter
						.translatePositionWithLabels(position);
			}
		...
		public boolean isItemChecked(int position) {
			return mGridView.getCheckedItemPositions().get(
					mapPagerToGridPosition(position));
		}
	...
	private void setSwitcherMenuState(MenuItem menuItem,
	...
	private void setFullscreenPagerVisibility(boolean visible) {
		...
		if (visible) {
			if (mPagerAdapter == null) {
				mPagerAdapter = new MtpPagerAdapter(this,
						mPositionMappingCheckBroker);
			...
			mFullscreenPager
					.setCurrentItem(
							mPagerAdapter
									.translatePositionWithLabels(pickFullscreenStartingPosition()),
							false);
		} else if (mPagerAdapter != null) {
			mGridView.setSelection(mAdapter
					.translatePositionWithoutLabels(mFullscreenPager
							.getCurrentItem()));
	...
	public void onObjectIndexed(MtpObjectInfo object, int numVisited) {
		...
		mProgressState.message = getResources().getQuantityString(
				R.plurals.ingest_number_of_items_scanned, numVisited,
				numVisited);
	...
	public void onSorting() {
		...
		mProgressState.reset();
		mProgressState.max = 0;
		mProgressState.message = getResources().getString(
				R.string.ingest_sorting);
		mHandler.sendEmptyMessage(ItemListHandler.MSG_PROGRESS_UPDATE);
	}
	...
	public void onImportProgress(final int visitedCount, final int totalCount,
		...
		mProgressState.title = getResources().getString(
				R.string.ingest_importing);
		mHandler.sendEmptyMessage(ItemListHandler.MSG_PROGRESS_UPDATE);
	}
	...
	// This is static and uses a WeakReference in order to avoid leaking the
	// Activity
