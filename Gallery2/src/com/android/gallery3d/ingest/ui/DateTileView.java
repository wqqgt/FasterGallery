...
public class DateTileView extends FrameLayout {
	private static String[] sMonthNames = DateFormatSymbols.getInstance()
			.getShortMonths();
	...
	public static boolean refreshLocale() {
		...
		if (!currentLocale.equals(sLocale)) {
			...
			sMonthNames = DateFormatSymbols.getInstance(sLocale)
					.getShortMonths();
			return true;
		} else {
			return false;
	...
	public void setDate(int date, int month, int year) {
		if (date != mDate) {
			mDate = date;
			mDateTextView.setText(mDate > 9 ? Integer.toString(mDate) : "0"
					+ mDate);
		}
