...
public class MtpImageView extends ImageView {
	...
	private WeakReference<MtpImageView> mWeakReference = new WeakReference<MtpImageView>(
			this);
	...
	private static final FetchImageHandler sFetchHandler = FetchImageHandler
			.createOnNewThread();
	...
	public void setMtpDeviceAndObjectInfo(MtpDevice device,
			MtpObjectInfo object, int gen) {
		...
		synchronized (mFetchLock) {
			...
			if (mFetchPending)
				return;
			mFetchPending = true;
			sFetchHandler.sendMessage(sFetchHandler.obtainMessage(0,
					mWeakReference));
		}
	}

	protected Object fetchMtpImageDataFromDevice(MtpDevice device,
			MtpObjectInfo info) {
	...
	private void updateDrawMatrix() {
		...
		if (rotated90) {
			mDrawMatrix.postTranslate(-dheight * scale * 0.5f, -dwidth * scale
					* 0.5f);
			mDrawMatrix.postRotate(mLastRotationDegrees);
			mDrawMatrix.postTranslate(dwidth * scale * 0.5f, dheight * scale
					* 0.5f);
		}
		...
		if (!rotated90 && mLastRotationDegrees > 0) {
			...
			mDrawMatrix.postRotate(mLastRotationDegrees, vwidth / 2,
					vheight / 2);
		}
	...
	private static class FetchImageHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (device == null)
				return;
			Object result = parent.fetchMtpImageDataFromDevice(device,
					objectInfo);
			if (result == null)
				return;
			synchronized (parent.mFetchLock) {
				...
				sFetchCompleteHandler.sendMessage(sFetchCompleteHandler
						.obtainMessage(0, parent.mWeakReference));
			}
		}
	}
