...
public class IconFactory {
	 ...
	 * 
	 * @param sourceImage
	 *            image to create an icon from.
	 * @param iconWidth
	 *            width of the icon bitmap.
	 * @param iconHeight
	 *            height of the icon bitmap.
	 * @param scale
	 *            if true, stretch sourceImage to fit the icon dimensions.
	 * @return an icon bitmap with the dimensions iconWidth:iconHeight.
	 */
	public static Bitmap createIcon(Bitmap sourceImage, int iconWidth,
			int iconHeight, boolean scale) {
		if (sourceImage == null) {
		...
		if (sourceWidth == 0 || sourceHeight == 0 || iconWidth == 0
				|| iconHeight == 0) {
			throw new IllegalArgumentException(
					"Bitmap with dimension 0 used as input");
		}
	 ...
	 * 
	 * @param dest
	 *            bitmap into which to draw the icon.
	 * @param sourceImage
	 *            image to create an icon from.
	 * @param scale
	 *            if true, stretch sourceImage to fit the destination.
	 */
