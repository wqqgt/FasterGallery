...
public class SaveCopyTask extends AsyncTask<ImagePreset, Void, Uri> {
	...
	public SaveCopyTask(Context context, Uri sourceUri, File destination,
			Callback callback) {
	...
	protected Uri doInBackground(ImagePreset... params) {
		...
		while (noBitmap) {
			try {
				...
				Bitmap bitmap = ImageLoader.loadMutableBitmap(context,
						sourceUri, options);
				if (bitmap == null) {
					return null;
				}
				...
				if (putExifData(this.destinationFile, exif, bitmap)) {
					...
					uri = insertContent(context, sourceUri,
							this.destinationFile, saveFileName, time);
				}
				noBitmap = false;
