...
public class MasterImage implements RenderingRequestCaller {
	...
	private float mMaxScaleFactor = 3.0f; // TODO: base this on the current view
											// / image
	...
	private void renderHistoryPreview() {
		...
		if (historyPreset != null) {
			...
			if (preview != null) {
				...
				Bitmap historyPreview = Bitmap.createScaledBitmap(preview, w,
						h, true);
				historyPreset.setPreviewImage(historyPreview);
				RenderingRequest.post(historyPreview, historyPreset,
						RenderingRequest.ICON_RENDERING, this);
			}
		}
	}
	...
	public void updatePresets(boolean force) {
		if (force || mGeometryOnlyPreset == null) {
			...
			if (force || mGeometryOnlyPreset == null
				...
				RenderingRequest.post(mLoader.getOriginalBitmapLarge(),
						mGeometryOnlyPreset,
						RenderingRequest.GEOMETRY_RENDERING, this);
			}
		}
		if (force || mFiltersOnlyPreset == null) {
			...
			if (force || mFiltersOnlyPreset == null
				...
				RenderingRequest.post(mLoader.getOriginalBitmapLarge(),
						mFiltersOnlyPreset, RenderingRequest.FILTERS_RENDERING,
						this);
			}
		}
		invalidatePreview();
	...
	public void setCurrentFilterRepresentation(
			FilterRepresentation currentFilterRepresentation) {
	...
	private Matrix getImageToScreenMatrix(boolean reflectRotation) {
		...
		if (geo == null || mLoader == null
				|| mLoader.getOriginalBounds() == null || mImageShowSize.x == 0) {
			return new Matrix();
		}
		Matrix m = geo.getOriginalToScreen(reflectRotation, mLoader
				.getOriginalBounds().width(), mLoader.getOriginalBounds()
				.height(), mImageShowSize.x, mImageShowSize.y);
		Point translate = getTranslation();
		float scaleFactor = getScaleFactor();
		m.postTranslate(translate.x, translate.y);
		m.postScale(scaleFactor, scaleFactor, mImageShowSize.x / 2.0f,
	...
	public void needsUpdateHighResPreview() {
		...
		RenderingRequest.post(null, mPreset,
				RenderingRequest.HIGHRES_RENDERING, this);
		invalidateHighresPreview();
	}
	...
	public void needsUpdatePartialPreview() {
		...
		RenderingRequest.post(null, mPreset,
				RenderingRequest.PARTIAL_RENDERING, this, bounds, new Rect(0,
						0, mImageShowSize.x, mImageShowSize.y));
		invalidatePartialPreview();
	}
