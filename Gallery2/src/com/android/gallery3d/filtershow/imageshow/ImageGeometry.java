...
public abstract class ImageGeometry extends ImageShow {
	...
	protected void updateScale() {
		RectF bounds = getUntranslatedStraightenCropBounds(
				mLocalGeometry.getPhotoBounds(), getLocalStraighten());
	...
	protected static Path drawClosedPath(Canvas canvas, Paint paint,
			float[] points) {
		Path crop = new Path();
	...
	protected static float getNewHeightForWidthAspect(float width, float w,
			float h) {
		return width * h / w;
	}

	protected static float getNewWidthForHeightAspect(float height, float w,
			float h) {
		return height * w / h;
	}
	...
	public void saveAndSetPreset() {
		ImagePreset lastHistoryItem = MasterImage.getImage().getHistory()
				.getLast();
		if (lastHistoryItem != null
				&& lastHistoryItem.historyName().equalsIgnoreCase(getName())) {
	...
	public static RectF getUntranslatedStraightenCropBounds(RectF imageRect,
	...
	protected RectF straightenBounds() {
		RectF bounds = getUntranslatedStraightenCropBounds(
				getLocalPhotoBounds(), getLocalStraighten());
	...
	protected static void drawRotatedShadows(Canvas canvas, Paint p,
			RectF innerBounds, RectF outerBounds, float rotation,
		...
		canvas.drawRect(innerBounds.left, innerBounds.bottom, maxX, maxY, p);
		canvas.drawRect(innerBounds.right, minY, maxX, innerBounds.bottom, p);
		canvas.rotate(-rotation, centerX, centerY);
		canvas.restore();
	}
	...
	protected void drawShadows(Canvas canvas, Paint p, RectF innerBounds) {
		...
		canvas.drawRect(0f, innerBounds.top, innerBounds.left,
				innerBounds.bottom, p);
		canvas.drawRect(innerBounds.right, innerBounds.top, w,
				innerBounds.bottom, p);
	 ...
	 */
	protected float getTransformState(RectF photo, RectF crop,
			float[] displayCenter) {
	...
	protected RectF drawTransformed(Canvas canvas, Bitmap photo, Paint p,
		...
		Matrix m = GeometryMetadata.buildCenteredPhotoMatrix(scaledPhoto,
				scaledCrop, getLocalRotation(), getLocalStraighten(),
				getLocalFlip(), displayCenter);
	...
	protected void drawTransformedCropped(Canvas canvas, Bitmap photo, Paint p) {
		...
		float scale = GeometryMath.scale(imageWidth, imageHeight, getWidth(),
				getHeight());
		// checks if local rotation is an odd multiple of 90.
		if (isHeightWidthSwapped()) {
			scale = GeometryMath.scale(imageWidth, imageHeight, getHeight(),
					getWidth());
		}
		// put in screen coordinates
		...
		m1.preRotate(getLocalStraighten(), scaledPhoto.centerX(),
				scaledPhoto.centerY());
		...
		p.setColor(getDefaultBackgroundColor());
		p.setStyle(Paint.Style.FILL);
		scaledCrop.offset(displayCenter[0] - scaledCrop.centerX(),
				displayCenter[1] - scaledCrop.centerY());
		RectF display = new RectF(0, 0, getWidth(), getHeight());
		drawRotatedShadows(canvas, p, scaledCrop, display, getLocalRotation(),
