...
public class ImageCrop extends ImageGeometry {
	...
	public void apply(float w, float h) {
		...
		mAspectWidth = w;
		mAspectHeight = h;
		setLocalCropBounds(getUntranslatedStraightenCropBounds(
	...
	private BoundedRect getBoundedCrop(RectF crop) {
		...
		float[] photoCenter = { photo.centerX(), photo.centerY() };
	...
	private void detectMovingEdges(float x, float y) {
		...
		if ((top <= mTouchTolerance) && ((x + mTouchTolerance) >= cropped.left)
			...
			movingEdges |= MOVE_TOP;
				...
				&& ((x + mTouchTolerance) >= cropped.left)
	...
	private RectF fixedCornerResize(RectF r, int moving_corner, float dx,
			float dy) {
		RectF newCrop = null;
		// Fix opposite corner in place and move sides
		if (moving_corner == BOTTOM_RIGHT) {
			newCrop = new RectF(r.left, r.top, r.left + r.width() + dx, r.top
					+ r.height() + dy);
		} else if (moving_corner == BOTTOM_LEFT) {
			newCrop = new RectF(r.right - r.width() + dx, r.top, r.right, r.top
					+ r.height() + dy);
		} else if (moving_corner == TOP_LEFT) {
			newCrop = new RectF(r.right - r.width() + dx, r.bottom - r.height()
					+ dy, r.right, r.bottom);
	...
	private void moveEdges(float dX, float dY) {
		...
		if (movingEdges == MOVE_BLOCK) {
			...
			if ((movingEdges & MOVE_LEFT) != 0) {
				dx = Math.min(crop.left + dX, crop.right - minWidthHeight)
						- crop.left;
			}
			if ((movingEdges & MOVE_TOP) != 0) {
				dy = Math.min(crop.top + dY, crop.bottom - minWidthHeight)
						- crop.top;
			}
	...
	private void cropSetup() {
		...
		if (!setCropBounds(crop)) {
			...
			crop.set(dCentX - h * wScale, dCentY - h * hScale, dCentX + h
					* wScale, dCentY + h * hScale);
			if (mFixAspectRatio) {
				CropMath.fixAspectRatio(crop, mAspectWidth, mAspectHeight);
			}
			r.setInner(crop);
	...
	protected void drawShape(Canvas canvas, Bitmap image) {
		...
		if (mFixAspectRatio) {
			...
			if (mDoingCropIntentAction && spotlightX > 0 && spotlightY > 0) {
				...
				RectF r1 = new RectF(cx - sx / 2, cy - sy / 2, cx + sx / 2, cy
						+ sy / 2);
				float temp = sx;
				sx = sy;
				sy = temp;
				RectF r2 = new RectF(cx - sx / 2, cy - sy / 2, cx + sx / 2, cy
						+ sy / 2);
				...
				for (int x = 0; x < num_intervals; x++) {
					...
					if ((x % 2) == 0 && Math.abs(x - num_intervals / 2) > 2) {
						canvas.drawLine(counter[0], counter[1], tempX, tempY,
								gPaint);
					}
					counter[0] = tempX;
					counter[1] = tempY;
		...
		if (mFixAspectRatio) {
			if ((movingEdges == TOP_LEFT) || notMoving) {
				drawIndicator(canvas, cropIndicator, scaledCrop.left,
						scaledCrop.top);
			}
			if ((movingEdges == TOP_RIGHT) || notMoving) {
				drawIndicator(canvas, cropIndicator, scaledCrop.right,
						scaledCrop.top);
			}
			...
			if (((movingEdges & MOVE_TOP) != 0) || notMoving) {
				drawIndicator(canvas, cropIndicator, scaledCrop.centerX(),
						scaledCrop.top);
			}
