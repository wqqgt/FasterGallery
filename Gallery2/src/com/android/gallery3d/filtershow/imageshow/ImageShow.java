...
public class ImageShow extends View implements OnGestureListener,
	...
	public Rect getImageCropBounds() {
		return GeometryMath.roundNearest(getImagePreset().mGeoData
				.getPreviewCropBounds());
	}
	...
	protected Matrix getImageToScreenMatrix(boolean reflectRotation) {
		...
		Matrix m = geo.getOriginalToScreen(reflectRotation, mImageLoader
				.getOriginalBounds().width(), mImageLoader.getOriginalBounds()
				.height(), getWidth(), getHeight());
		Point translate = MasterImage.getImage().getTranslation();
		float scaleFactor = MasterImage.getImage().getScaleFactor();
		m.postTranslate(translate.x, translate.y);
		m.postScale(scaleFactor, scaleFactor, getWidth() / 2.0f,
				getHeight() / 2.0f);
		return m;
	}
	...
	public void onDraw(Canvas canvas) {
		...
		if (partialPreview != null) {
			Rect src = new Rect(0, 0, partialPreview.getWidth(),
					partialPreview.getHeight());
	...
	public void drawImage(Canvas canvas, Bitmap image, boolean updateBounds) {
		if (image != null) {
			Rect s = new Rect(0, 0, image.getWidth(), image.getHeight());

			float scale = GeometryMath.scale(image.getWidth(),
	...
	public void drawPartialImage(Canvas canvas, Bitmap image) {
		...
		if (image != null) {
			if (mShowOriginalDirection == 0) {
				if (Math.abs(mTouch.y - mTouchDown.y) > Math.abs(mTouch.x
						- mTouchDown.x)) {
			...
			if (mShowOriginalDirection == UNVEIL_VERTICAL) {
				...
				canvas.drawLine(mTouch.x, mImageBounds.top, mTouch.x,
						mImageBounds.bottom, paint);
			}

			Rect bounds = new Rect();
			paint.setAntiAlias(true);
			paint.setTextSize(mOriginalTextSize);
			paint.getTextBounds(mOriginalText, 0, mOriginalText.length(),
					bounds);
			paint.setColor(Color.BLACK);
			paint.setStyle(Paint.Style.STROKE);
			paint.setStrokeWidth(3);
			canvas.drawText(mOriginalText, mImageBounds.left
					+ mOriginalTextMargin, mImageBounds.top + bounds.height()
					+ mOriginalTextMargin, paint);
	...
	public void drawBackground(Canvas canvas) {
		if (USE_BACKGROUND_IMAGE) {
			if (mBackgroundImage == null) {
				mBackgroundImage = mImageLoader
						.getBackgroundBitmap(getResources());
			}
			if (mBackgroundImage != null) {
	...
	public void saveToUri(Bitmap f, Uri u, String m,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (action == MotionEvent.ACTION_DOWN) {
			...
			MasterImage.getImage().setOriginalTranslation(
					MasterImage.getImage().getTranslation());
		}
