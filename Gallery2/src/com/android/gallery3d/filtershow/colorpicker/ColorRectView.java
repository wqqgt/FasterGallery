...
public class ColorRectView extends View implements ColorListener {
	...
	private int[] mColors = new int[] { 0xFFFF0000,// red
			0xFFFFFF00,// yellow
			0xFF00FF00,// green
			0xFF00FFFF,// cyan
			0xFF0000FF,// blue
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		invalidate((int) (mDotX - mDotRadus), (int) (mDotY - mDotRadus),
				(int) (mDotX + mDotRadus), (int) (mDotY + mDotRadus));
		float x = event.getX();
		float y = event.getY();
	...
	private void setUpColorPanel() {
		...
		LinearGradient lg = new LinearGradient(mBorder, 0, mWidth - mBorder, 0,
				wheelColor, null, Shader.TileMode.CLAMP);

		mWheelPaint1.setShader(lg);
		LinearGradient rg = new LinearGradient(0, mBorder, 0,
				mHeight - mBorder, colors, null, Shader.TileMode.CLAMP);
		mWheelPaint2.setShader(rg);
		LinearGradient rg2 = new LinearGradient(0, mBorder, 0, mHeight
				- mBorder, colors2, null, Shader.TileMode.CLAMP);
	...
	private void updateDot() {
		...
		mDotX = (float) (mBorder + (mHeight - 2 * mBorder)
				* Math.toRadians(hue) / (Math.PI * 2));
