...
public class CachingPipeline {
	...
	public static synchronized void createRenderscriptContext(Activity context) {
		if (sRS != null) {
			Log.w(LOGTAG,
					"A prior RS context exists when calling setRenderScriptContext");
	...
	public synchronized void reset() {
		synchronized (CachingPipeline.class) {
			...
			mOriginalBitmap = null; // just a reference to the bitmap in
									// ImageLoader
	...
	public void setOriginal(Bitmap bitmap) {
		...
		Log.v(LOGTAG,
				"setOriginal, size " + bitmap.getWidth() + " x "
						+ bitmap.getHeight());
	...
	private synchronized boolean updateOriginalAllocation(ImagePreset preset) {
		...
		mFiltersOnlyOriginalAllocation = Allocation.createFromBitmap(RS,
				originalBitmap, Allocation.MipmapControl.MIPMAP_NONE,
		...
		mResizedOriginalBitmap = preset.applyGeometry(originalBitmap,
				mEnvironment);
		mOriginalAllocation = Allocation.createFromBitmap(RS,
				mResizedOriginalBitmap, Allocation.MipmapControl.MIPMAP_NONE,
				Allocation.USAGE_SCRIPT);
	...
	public synchronized void render(RenderingRequest request) {
		synchronized (CachingPipeline.class) {
			...
			if (((request.getType() != RenderingRequest.PARTIAL_RENDERING && request
					.getType() != RenderingRequest.HIGHRES_RENDERING) && request
					.getBitmap() == null)
			...
			if (request.getType() == RenderingRequest.PARTIAL_RENDERING) {
				...
				if (loader == null) {
					Log.w(LOGTAG, "loader not yet setup, cannot handle: "
							+ getType(request));
					return;
				}
				...
				if (bitmap == null) {
					Log.w(LOGTAG, "could not get bitmap for: "
							+ getType(request));
					return;
				}
			}
			...
			if (DEBUG) {
				Log.v(LOGTAG, "after update, req bitmap (" + bitmap.getWidth()
						+ "x" + bitmap.getHeight() + " ? resizeOriginal ("
	...
	public synchronized void renderImage(ImagePreset preset, Allocation in,
			Allocation out) {
		synchronized (CachingPipeline.class) {
			if (getRenderScriptContext() == null) {
				return;
	...
	public synchronized Bitmap renderFinalImage(Bitmap bitmap,
			ImagePreset preset) {
		synchronized (CachingPipeline.class) {
			if (getRenderScriptContext() == null) {
				return bitmap;
	...
	public synchronized void compute(TripleBufferBitmap buffer,
			ImagePreset preset, int type) {
		synchronized (CachingPipeline.class) {
			if (getRenderScriptContext() == null) {
				return;
			}
			...
			if (bitmap == null
					|| (bitmap.getWidth() != resizedOriginalBitmap.getWidth())
			...
			if (DEBUG) {
				Log.v(LOGTAG,
						"Applying type " + type + " filters to bitmap "
								+ bitmap + " (" + bitmap.getWidth() + " x "
								+ bitmap.getHeight() + ") took " + time
								+ " ms, " + time2
								+ " ms for the filter, on thread " + thread);
			}
		}
	}
	...
	public boolean prepareRenderscriptAllocations(Bitmap bitmap) {
		...
		if (DEBUG) {
			Log.v(LOGTAG, "prepareRenderscriptAllocations: " + needsUpdate
					+ " in " + getName());
		}
		return needsUpdate;
	}
