...
public class ImageLoader {
	...
	public boolean loadBitmap(Uri uri, int size) {
		...
		if (MasterImage.getImage().supportsHighRes()) {
			...
			mOriginalBitmapHighres = loadScaledBitmap(uri, highresPreviewSize,
					false);
		}
		updateBitmaps();
		mLoadingLock.unlock();
	...
	public static int getOrientation(Context context, Uri uri) {
		if (ContentResolver.SCHEME_FILE.equals(uri.getScheme())) {
			...
			try {
				exif.readExif(path);
				orientation = ExifInterface.getRotationForOrientationValue(exif
	...
	private void updateBitmaps() {
		if (mOrientation > 1) {
			...
			if (mOriginalBitmapHighres != null) {
				mOriginalBitmapHighres = rotateToPortrait(
						mOriginalBitmapHighres, mOrientation);
			}
		}
	...
	private Bitmap loadRegionBitmap(Uri uri, BitmapFactory.Options options,
			Rect bounds) {
		InputStream is = null;
		try {
	...
	private Bitmap loadScaledBitmap(Uri uri, int size, boolean enforceSize) {
		...
		try {
			...
			while (true) {
				...
				if (!enforceSize
						|| (width_tmp <= MAX_BITMAP_DIM && height_tmp <= MAX_BITMAP_DIM)) {
	...
	public void saveImage(ImagePreset preset,
			final FilterShowActivity filterShowActivity, File destination) {
		new SaveCopyTask(mContext, mUri, destination,
