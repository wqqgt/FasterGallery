...
public class ImagePreset {
	...
	public ImagePreset(ImagePreset source) {
		try {
			...
			for (int i = 0; i < source.mFilters.size(); i++) {
				FilterRepresentation representation = source.mFilters
						.elementAt(i).clone();
	...
	public int getPositionForRepresentation(FilterRepresentation representation) {
		for (int i = 0; i < mFilters.size(); i++) {
			if (mFilters.elementAt(i).getFilterClass() == representation
					.getFilterClass()) {
				return i;
			}
		}
	...
	public FilterRepresentation getFilterRepresentationCopyFrom(
		...
		// TODO: add concept of position in the filters (to allow multiple
		// instances)
		...
		FilterRepresentation representation = null;
		if ((mBorder != null)
				&& (mBorder.getFilterClass() == filterRepresentation
						.getFilterClass())) {
			// TODO: instead of special casing for border, we should correctly
			// implements "filters priority set"
	...
	public void updateFilterRepresentation(FilterRepresentation representation) {
		...
		synchronized (mFilters) {
			if (representation instanceof GeometryMetadata) {
				...
				if ((mBorder != null)
						&& (mBorder.getFilterClass() == representation
								.getFilterClass())) {
	...
	public void removeFilter(FilterRepresentation filterRepresentation) {
		...
		for (int i = 0; i < mFilters.size(); i++) {
			if (mFilters.elementAt(i).getFilterClass() == filterRepresentation
					.getFilterClass()) {
				mFilters.remove(i);
				setHistoryName("Remove");
				return;
	...
	public FilterRepresentation getRepresentation(
		...
		for (int i = 0; i < mFilters.size(); i++) {
			...
			if (representation.getFilterClass() == filterRepresentation
					.getFilterClass()) {
				return representation;
			}
		}
	...
	public Bitmap applyFilters(Bitmap bitmap, int from, int to,
		...
		if (mDoApplyFilters) {
			...
			for (int i = from; i < to; i++) {
				...
				bitmap = environment
						.applyRepresentation(representation, bitmap);
				if (environment.getQuality() == QUALITY_FINAL) {
					UsageStatistics.onEvent(UsageStatistics.COMPONENT_EDITOR,
	...
	public void applyBorder(Allocation in, Allocation out,
		...
		if (mBorder != null && mDoApplyGeometry) {
			...
			Allocation bitmapIn = Allocation.createTyped(
	...
	public Vector<ImageFilter> getUsedFilters(BaseFiltersManager filtersManager) {
		...
		for (int i = 0; i < mFilters.size(); i++) {
			FilterRepresentation representation = mFilters.elementAt(i);
			ImageFilter filter = filtersManager
					.getFilterForRepresentation(representation);
