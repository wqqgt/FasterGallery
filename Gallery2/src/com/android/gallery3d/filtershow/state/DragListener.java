...
class DragListener implements View.OnDragListener {
	...
	private void setState(DragEvent event) {
		...
		float alpha = 1.0f - (Math.abs(translation) / mStatePanelTrack
		...
		if (mStatePanelTrack.getOrientation() == LinearLayout.VERTICAL) {
			...
			alpha = 1.0f - (Math.abs(translation) / mStatePanelTrack
			...
			if (mStatePanelTrack.getCurrentView() != null) {
				...
				if (over != null && over != mStatePanelTrack.getCurrentView()) {
					...
					if (stateView != mStatePanelTrack.getCurrentView()) {
						...
						int origin = mStatePanelTrack
								.findChild(mStatePanelTrack.getCurrentView());
						ArrayAdapter array = (ArrayAdapter) mStatePanelTrack
								.getAdapter();
						if (origin != -1 && pos != -1) {
							...
							mStatePanelTrack.setCurrentView(mStatePanelTrack
									.getChildAt(pos));
						}
					}
				}
