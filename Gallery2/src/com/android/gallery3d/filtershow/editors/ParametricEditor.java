...
public class ParametricEditor extends Editor {
	...
	static {
		...
		portraitMap.put(ParameterActionAndInt.sParameterType,
				ActionSlider.class);
		landscapeMap.put(ParameterActionAndInt.sParameterType,
				ActionSlider.class);
		...
		if (mShowParameter == SHOW_VALUE_INT & useCompact(context)) {
			if (getLocalRepresentation() instanceof FilterBasicRepresentation) {
				...
				apply += " " + effectName.toUpperCase() + " "
						+ interval.getStateRepresentation();
			} else {
	...
	protected void control(Parameter p, View editControl) {
		...
		Class c = ((useCompact(context)) ? portraitMap : landscapeMap)
				.get(pType);
		...
		if (c != null) {
			...
			for (String string : portraitMap.keySet()) {
				Log.e(LOGTAG,
						"for " + string + " use " + portraitMap.get(string));
			}
		}
	}
