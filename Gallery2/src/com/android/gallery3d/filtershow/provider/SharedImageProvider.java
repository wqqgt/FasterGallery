...
public class SharedImageProvider extends ContentProvider {
	...
	private final String[] mMimeStreamType = { MIME_TYPE };

	private static ConditionVariable mImageReadyCond = new ConditionVariable(
			false);
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		if (projection == null) {
			projection = new String[] { BaseColumns._ID,
					MediaStore.MediaColumns.DATA, OpenableColumns.DISPLAY_NAME,
		...
		for (int i = 0; i < projection.length; i++) {
			if (projection[i].equalsIgnoreCase(BaseColumns._ID)) {
				columns[i] = 0;
					...
					.equalsIgnoreCase(MediaStore.MediaColumns.DATA)) {
				columns[i] = uri;
			} else if (projection[i]
					.equalsIgnoreCase(OpenableColumns.DISPLAY_NAME)) {
