...
public class FilterDrawRepresentation extends FilterRepresentation {
	...
	public String toString() {
		return getName()
				+ " : strokes="
				+ mDrawing.size()
				+ ((mCurrent == null) ? " no current " : ("draw="
						+ mCurrent.mType + " " + mCurrent.noPoints));
	}
	...
	public FilterRepresentation clone() throws CloneNotSupportedException {
		FilterDrawRepresentation representation = (FilterDrawRepresentation) super
				.clone();
		...
		if (a instanceof FilterDrawRepresentation) {
			...
			try {
				...
				if (representation.mDrawing != null) {
					mDrawing = (Vector<StrokeData>) representation.mDrawing
							.clone();
				} else {
					mDrawing = null;
				}
	...
	public void startNewSection(byte type, int color, float size, float x,
			float y) {
