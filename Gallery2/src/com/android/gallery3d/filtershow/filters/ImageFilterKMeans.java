...
public class ImageFilterKMeans extends SimpleImageFilter {
	...
	public FilterRepresentation getDefaultRepresentation() {
		FilterBasicRepresentation representation = (FilterBasicRepresentation) super
	...
	native protected void nativeApplyFilter(Bitmap bitmap, int width,
			int height, Bitmap large_ds_bm, int lwidth, int lheight,
			Bitmap small_ds_bm, int swidth, int sheight, int p, int seed);
	...
	public Bitmap apply(Bitmap bitmap, float scaleFactor, int quality) {
		...
		if (getParameters() != null) {
			int p = Math.max(getParameters().getValue(), getParameters()
					.getMinimum())
					% (getParameters().getMaximum() + 1);
			nativeApplyFilter(bitmap, w, h, large_bm_ds, lw, lh, small_bm_ds,
					sw, sh, p, mSeed);
		}
		return bitmap;
	}
