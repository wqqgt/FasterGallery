...
public class ImageFilterGeometry extends ImageFilter {
	...
	native protected void nativeApplyFilterFlip(Bitmap src, int srcWidth,
			int srcHeight, Bitmap dst, int dstWidth, int dstHeight, int flip);

	native protected void nativeApplyFilterRotate(Bitmap src, int srcWidth,
			int srcHeight, Bitmap dst, int dstWidth, int dstHeight, int rotate);

	native protected void nativeApplyFilterCrop(Bitmap src, int srcWidth,
			int srcHeight, Bitmap dst, int dstWidth, int dstHeight,
			int offsetWidth, int offsetHeight);

	native protected void nativeApplyFilterStraighten(Bitmap src, int srcWidth,
			int srcHeight, Bitmap dst, int dstWidth, int dstHeight,
	...
	public Bitmap apply(Bitmap bitmap, float scaleFactor, int quality) {
		...
		if (cb.width() == 0 || cb.height() == 0 || pb.width() == 0
				|| pb.height() == 0) {
			Log.w(LOGTAG,
					"Cannot apply geometry: geometry metadata has not been initialized");
			return bitmap;
		}
		...
		Matrix m1 = mGeometry.buildTotalXform(bitmap.getWidth(),
				bitmap.getHeight(), displayCenter);
