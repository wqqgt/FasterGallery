...
public abstract class ImageFilterRS extends ImageFilter {
	...
	private static Script mGreyConvert = null;
	...
	public void apply(Allocation in, Allocation out) {
		...
		if (!mResourcesLoaded) {
			CachingPipeline pipeline = getEnvironment().getCachingPipeline();
			createFilter(pipeline.getResources(), getEnvironment()
					.getScaleFactor(), getEnvironment().getQuality(), in);
		...
		if (PERF_LOGGING) {
			...
			String msg = String.format("%s; image size %dx%d; ", getName(), in
					.getType().getX(), in.getType().getY());
	...
	public Bitmap apply(Bitmap bitmap, float scaleFactor, int quality) {
		...
		try {
			CachingPipeline pipeline = getEnvironment().getCachingPipeline();
			if (DEBUG) {
				Log.v(LOGTAG, "apply filter " + getName() + " in pipeline "
						+ pipeline.getName());
			}
			Resources rsc = pipeline.getResources();
			boolean sizeChanged = false;
			if (getInPixelsAllocation() != null
					&& ((getInPixelsAllocation().getType().getX() != mLastInputWidth) || (getInPixelsAllocation()
			...
			update(bitmap);
			if (DEBUG) {
				Log.v(LOGTAG, "DONE apply filter " + getName()
						+ " in pipeline " + pipeline.getName());
			}
			...
			// Many of the renderscript filters allocated large (>16Mb
			// resources) in order to apply.
			System.gc();
	...
	protected static Allocation convertBitmap(Bitmap bitmap) {
		return Allocation.createFromBitmap(
				CachingPipeline.getRenderScriptContext(), bitmap,
	...
	public Allocation loadScaledResourceAlpha(int resource, int w, int h,
			int inSampleSize) {
		Resources res = CachingPipeline.getResources();
		final BitmapFactory.Options options = new BitmapFactory.Options();
		options.inPreferredConfig = Bitmap.Config.ALPHA_8;
