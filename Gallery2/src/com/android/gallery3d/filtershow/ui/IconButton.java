...
public class IconButton extends Button {
	...
	/**
	 * Set the image that the button icon will use. The image bitmap will be
	 * scaled and cropped into the largest bitmap with dimensions given by
	 * getGoodIconSideSize() that will fit cleanly within the IconButton's
	 * layout.
	 * 
	 * @param image
	 *            image that icon will be set to before next draw.
	 */
	...
	/**
	 * Finds a side lengths for the icon that fits within the button. Only call
	 * after layout. The default implementation returns the best side lengths
	 * for a square icon.
	 * <p>
	...
	protected int[] getGoodIconSideSize() {
		...
		int vert = getHeight() - getPaddingTop() - getPaddingBottom()
				- bounds.height() - inner_padding;

		// find total horizontal space available for the icon
		int horiz = getWidth() - getPaddingLeft() - getPaddingRight()
				- inner_padding;
	...
	private boolean setImageIcon(Bitmap image) {
		...
		mIcon = image;
		this.setCompoundDrawablesWithIntrinsicBounds(null, new BitmapDrawable(
				getResources(), mIcon), null, null);
		return true;
	}
	...
	private Bitmap makeImageIcon(Bitmap image, int width, int height) {
		if (image == null || image.getHeight() < 1 || image.getWidth() < 1
			...
			throw new IllegalArgumentException(
					"input is null, or has invalid dimensions");
		}
		Bitmap icon = null;
