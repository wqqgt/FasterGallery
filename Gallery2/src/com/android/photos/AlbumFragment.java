...
public class AlbumFragment extends MultiSelectGridFragment implements
	...
	private void updateHeaderView() {
		if (mHeaderView == null) {
			mHeaderView = LayoutInflater.from(getActivity()).inflate(
					R.layout.album_header, getGridView(), false);
			((HeaderGridView) getGridView()).addHeaderView(mHeaderView, null,
					false);

			// TODO remove this when the data model stabilizes
			mHeaderView.setMinimumHeight(200);
		}
		ImageView iv = (ImageView) mHeaderView
				.findViewById(R.id.album_header_image);
		TextView title = (TextView) mHeaderView
				.findViewById(R.id.album_header_title);
		TextView subtitle = (TextView) mHeaderView
				.findViewById(R.id.album_header_subtitle);
		...
		if (count > 0) {
			iv.setImageDrawable(mLoaderCompatShim.drawableForItem(
					mAdapter.getItem(0), null));
		}
	}
	...
	public int getItemSupportedOperations(Object item) {
		return ((Cursor) item)
				.getInt(PhotoSetLoader.INDEX_SUPPORTED_OPERATIONS);
	}
