...
public class PhotoProvider extends SQLiteContentProvider {
	...
	static final Uri BASE_CONTENT_URI = new Uri.Builder().scheme("content")
	...
	public static interface Albums extends BaseColumns {
		...
		/**
		 * The type of album. Non-null, if album is auto-generated. String
		 * value.
		 */
	...
	protected static final String WHERE_METADATA_ID = Metadata.PHOTO_ID
			+ " = ? AND " + Metadata.KEY + " = ?";

	protected static final String SELECT_ALBUM_ID = "SELECT " + Albums._ID
			+ " FROM " + Albums.TABLE;
	protected static final String SELECT_PHOTO_ID = "SELECT " + Photos._ID
			+ " FROM " + Photos.TABLE;
	protected static final String SELECT_PHOTO_COUNT = "SELECT COUNT(*) FROM "
			+ Photos.TABLE;
	protected static final String DELETE_PHOTOS = "DELETE FROM " + Photos.TABLE;
	protected static final String DELETE_METADATA = "DELETE FROM "
			+ Metadata.TABLE;
	protected static final String SELECT_METADATA_COUNT = "SELECT COUNT(*) FROM "
			+ Metadata.TABLE;
	...
	protected static final String[] BASE_COLUMNS_ID = { BaseColumns._ID, };
	...
	@Override
	public int deleteInTransaction(Uri uri, String selection,
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		selectionArgs = addIdToSelectionArgs(match, uri, selectionArgs);
		String table = getTableFromMatch(match, uri);
		Cursor c = query(table, projection, selection, selectionArgs,
	...
	@Override
	public int updateInTransaction(Uri uri, ContentValues values,
	...
	protected static String[] addIdToSelectionArgs(int match, Uri uri,
	...
	protected static String[] addMetadataKeysToSelectionArgs(
			String[] selectionArgs, Uri uri) {
		List<String> segments = uri.getPathSegments();
		String[] additionalArgs = { segments.get(1), segments.get(2), };
	...
	private int modifyMetadata(SQLiteDatabase db, ContentValues values) {
		...
		if (values.get(Metadata.VALUE) == null) {
			String[] selectionArgs = { values.getAsString(Metadata.PHOTO_ID),
					values.getAsString(Metadata.KEY), };
			rowCount = db.delete(Metadata.TABLE, WHERE_METADATA_ID,
					selectionArgs);
		} else {
	...
	protected static String nestWhere(String matchColumn, String table,
			String nestedWhere) {
		String query = SQLiteQueryBuilder.buildQueryString(false, table,
				BASE_COLUMNS_ID, nestedWhere, null, null, null, null);
		return matchColumn + IN + NESTED_SELECT_START + query
				+ NESTED_SELECT_END;
	}
	...
	protected int deleteCascade(Uri uri, int match, String selection,
			String[] selectionArgs) {
		switch (match) {
		case MATCH_PHOTO:
		case MATCH_PHOTO_ID:
	...
	private static void validateMatchTable(int match) {
		...
		default:
			throw new IllegalArgumentException(
					"Operation not allowed on an existing row.");
		}
	}
	...
	protected Cursor query(String table, String[] columns, String selection,
		...
		if (ApiHelper.HAS_CANCELLATION_SIGNAL) {
			return db.query(false, table, columns, selection, selectionArgs,
					null, null, orderBy, null, cancellationSignal);
		} else {
			return db.query(table, columns, selection, selectionArgs, null,
					null, orderBy);
		}
	}
