...
public class PhotoSetLoader extends CursorLoader implements
		LoaderCompatShim<Cursor> {
	...
	public static final String[] PROJECTION = new String[] { FileColumns._ID,
			FileColumns.DATA, FileColumns.WIDTH, FileColumns.HEIGHT,
			FileColumns.DATE_ADDED, FileColumns.MEDIA_TYPE,
			SUPPORTED_OPERATIONS, };

	private static final String SORT_ORDER = FileColumns.DATE_ADDED + " DESC";
	private static final String SELECTION = FileColumns.MEDIA_TYPE + " == "
	...
	private static final Uri GLOBAL_CONTENT_URI = Uri.parse("content://"
			+ MediaStore.AUTHORITY + "/external/");
	...
	protected void onStartLoading() {
		...
		getContext().getContentResolver().registerContentObserver(
				GLOBAL_CONTENT_URI, true, mGlobalObserver);
	}
	...
	public Drawable drawableForItem(Cursor item, Drawable recycle) {
		...
		drawable.setImage(item.getString(INDEX_DATA), item.getInt(INDEX_WIDTH),
				item.getInt(INDEX_HEIGHT));
		return drawable;
	}
