...
public class GalleryThumbnailView extends ViewGroup {
	...
	public interface GalleryThumbnailAdapter extends ListAdapter {
		/**
		 * @param position
		 *            Position to get the intrinsic aspect ratio for
		 * @return width / height
		 */
	 ...
	 * 
	 * @param startPosition
	 *            Logical position in the list to start from
	 * @param x
	 *            Left or right edge of the view to add
	 * @param forward
	 *            If true, align left edge to x and increase position. If false,
	 *            align right edge to x and decrease position.
	...
	private int makeAndAddColumn(int startPosition, int x, boolean forward) {
		...
		int addViews = 0;
		for (int remaining = mLargeColumnUnitCount, i = 0; remaining > 0
				&& startPosition + i >= 0 && startPosition + i < mItemCount; i += forward ? 1
				: -1, addViews++) {
		...
		for (int i = 0; i < addViews; i++) {
			...
			if (child.getParent() != this) {
				if (mInLayout) {
					addViewInLayout(child, forward ? -1 : 0,
							child.getLayoutParams());
				} else {
					addView(child, forward ? -1 : 0);
				}
			}
			int heightSize = (int) (.5f + (mAdapter
					.getIntrinsicAspectRatio(position) >= 1f ? columnWidth
					/ ASPECT_RATIO : columnWidth * ASPECT_RATIO));
			int heightSpec = MeasureSpec.makeMeasureSpec(heightSize,
					MeasureSpec.EXACTLY);
			int widthSpec = MeasureSpec.makeMeasureSpec(columnWidth,
					MeasureSpec.EXACTLY);
			child.measure(widthSpec, heightSpec);
			int childLeft = forward ? x : x - columnWidth;
			child.layout(childLeft, nextTop, childLeft + columnWidth, nextTop
					+ heightSize);
			nextTop += heightSize;
		}
		return addViews;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP: {
			...
			final float velocity = VelocityTrackerCompat.getXVelocity(
					mVelocityTracker, mActivePointerId);
			if (Math.abs(velocity) > mFlingVelocity) { // TODO
				mTouchMode = TOUCH_MODE_FLINGING;
				mScroller.fling(0, 0, (int) velocity, 0, Integer.MIN_VALUE,
	...
	private boolean trackMotionScroll(int deltaX, boolean allowOverScroll) {
		...
		if (!contentFits) {
			...
			if (deltaX > 0) {
				...
				overhang = fillRight(mFirstPosition + getChildCount(),
						allowOverhang);
				up = false;
			}
	...
	private int fillRight(int pos, int overhang) {
		...
		int nextLeft = getChildCount() == 0 ? 0 : getChildAt(
