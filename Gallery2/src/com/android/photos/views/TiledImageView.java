...
public class TiledImageView extends FrameLayout implements
	...
	public TiledImageView(Context context, AttributeSet attrs) {
		...
		addView(mTextureView, new LayoutParams(LayoutParams.MATCH_PARENT,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		synchronized (mLock) {
			...
			case MotionEvent.ACTION_MOVE:
				mFocusedRenderer.centerX += (mLastX - x)
						/ mFocusedRenderer.scale;
				mFocusedRenderer.centerY += (mLastY - y)
						/ mFocusedRenderer.scale;
				invalidate();
				break;
			}
	...
	private void updateScaleIfNecessaryLocked(ImageRendererWrapper renderer) {
		if (renderer.scale > 0 || getWidth() == 0)
			return;
	...
	private class TileRenderer implements Renderer {
		...
		public void onDrawFrame(GL10 gl) {
			...
			synchronized (mLock) {
				for (ImageRendererWrapper renderer : mRenderers) {
					renderer.image.setModel(renderer.source, renderer.rotation);
					renderer.image.setPosition(renderer.centerX,
							renderer.centerY, renderer.scale);
				}
			}
			for (ImageRendererWrapper renderer : mRenderers) {
