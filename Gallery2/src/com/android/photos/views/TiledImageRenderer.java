...
public class TiledImageRenderer {
	...
	private static GalleryBitmapPool sTilePool = GalleryBitmapPool
			.getInstance();
	...
	private void layoutTiles() {
		...
		if (mLevel != mLevelCount) {
			...
			mOffsetX = Math.round(mViewWidth / 2f + (range.left - mCenterX)
					* mScale);
			mOffsetY = Math.round(mViewHeight / 2f + (range.top - mCenterY)
					* mScale);
		...
		synchronized (mQueueLock) {
			if (tile.mTileState == STATE_RECYCLING) {
				...
				if (tile.mDecodedTile != null) {
					if (sTilePool != null)
						sTilePool.put(tile.mDecodedTile);
	...
	private void drawTile(GLCanvas canvas, int tx, int ty, int level, float x,
	...
	private boolean drawTile(Tile tile, GLCanvas canvas, RectF source,
			RectF target) {
		while (true) {
			if (tile.isContentValid()) {
	...
	private class Tile extends UploadedTexture {
		...
		protected Bitmap onGetBitmap() {
			...
			setSize(Math.min(mTileSize, rightEdge),
					Math.min(mTileSize, bottomEdge));
		...
		public String toString() {
			return String.format("tile(%s, %s, %s / %s)", mX / mTileSize, mY
					/ mTileSize, mLevel, mLevelCount);
		}
	}
	...
	private class TileDecoder extends Thread {
		...
		public void finishAndWait() {
			...
			try {
				...
				Log.w(TAG,
						"Interrupted while waiting for TileDecoder thread to finish!");
			}
		}
