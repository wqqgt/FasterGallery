...
public class MultiChoiceManager implements MultiChoiceModeListener,
	...
	public void onItemCheckedStateChanged(ActionMode mode, int position,
		...
		mSelectionManager.onItemSelectedStateChanged(mShareActionProvider,
				mDelegate.getItemMediaType(item), supported, checked);
	...
	private void updateActionItemVisibilities(Menu menu, int supportedOperations) {
		...
		deleteItem
				.setVisible((supportedOperations & MediaObject.SUPPORT_DELETE) > 0);
		shareItem
				.setVisible((supportedOperations & MediaObject.SUPPORT_SHARE) > 0);
		...
		setAsItem
				.setVisible((supportedOperations & MediaObject.SUPPORT_SETAS) > 0);
	}
	...
	private void singleItemAction(Object item, int actionItemId) {
		...
		case R.id.menu_crop:
			...
			return;
		case R.id.menu_trim:
			intent.setData(uri).setClass(mContext, TrimVideo.class);
			mContext.startActivity(intent);
			return;
		case R.id.menu_mute:
			/*
			 * TODO need a way to get the file path of an item MuteVideo
			 * muteVideo = new MuteVideo(filePath, uri, (Activity) mContext);
		...
		case R.id.menu_set_as:
			...
			mContext.startActivity(Intent.createChooser(intent,
					mContext.getString(R.string.set_as)));
			return;
		default:
			return;
		}
