...
public class PhotoUI implements PieListener, SurfaceHolder.Callback,
		PreviewGestures.SingleTapListener, FocusUI, LocationManager.Listener,
		FaceDetectionListener, PreviewGestures.SwipeListener {
	...
	public PhotoUI(CameraActivity activity, PhotoController controller,
			View parent) {
		mActivity = activity;
		...
		mCountDownView = (CountDownView) (mRootView
				.findViewById(R.id.count_down_to_capture));
		mCountDownView
				.setCountDownFinishedListener((OnCountDownFinishedListener) mController);
	...
	public void onCameraOpened(PreferenceGroup prefGroup,
			ComboPreferences prefs, Camera.Parameters params,
	...
	public void initializeControlByIntent() {
		...
		if (mController.isImageCaptureIntent()) {
			...
			ViewGroup cameraControls = (ViewGroup) mActivity
					.findViewById(R.id.camera_controls);
			mActivity.getLayoutInflater().inflate(
					R.layout.review_module_control, cameraControls);
	...
	public void updateOnScreenIndicators(Camera.Parameters params,
		...
		int wbIndex = 2;
		ListPreference pref = group
				.findPreference(CameraSettings.KEY_WHITE_BALANCE);
		if (pref != null) {
	...
	public boolean removeTopLevelPopup() {
		// Remove the top level popup or dialog box and return true if there's
		// any
		if (mPopup != null) {
			dismissPopup();
			return true;
	...
	private class ZoomChangeListener implements
			ZoomRenderer.OnZoomChangedListener {
		@Override
	...
	public void showPreferencesToast() {
		if (mNotSelectableToast == null) {
			String str = mActivity.getResources().getString(
					R.string.not_selectable_in_scene_mode);
			mNotSelectableToast = Toast.makeText(mActivity, str,
					Toast.LENGTH_SHORT);
		}
	...
	private FocusIndicator getFocusIndicator() {
		return (mFaceView != null && mFaceView.faceExists()) ? mFaceView
				: mPieRenderer;
	}
