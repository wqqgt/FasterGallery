...
public class PreviewGestures implements
		...
		if (MotionEvent.ACTION_DOWN == m.getActionMasked()) {
			...
			if (MotionEvent.ACTION_UP == m.getActionMasked()) {
				...
				if (m.getEventTime() - mDown.getEventTime() < mTapTimeout
						&& checkClickable(m)) {
					cancelActivityTouchHandling(m);
					mTapListener.onSingleTapUp(null, (int) mDown.getX()
							- mOverlay.getWindowPositionX(), (int) mDown.getY()
	...
	private boolean isInside(MotionEvent evt, View v) {
		...
		return (v.getVisibility() == View.VISIBLE && evt.getX() >= mLocation[0]
				&& evt.getX() < mLocation[0] + v.getWidth()
				&& evt.getY() >= mLocation[1] && evt.getY() < mLocation[1]
				+ v.getHeight());
	}
