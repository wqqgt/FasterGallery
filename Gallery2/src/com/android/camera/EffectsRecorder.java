...
public class EffectsRecorder {
	...
	/**
	 * Determine if a given effect is supported at runtime Some effects require
	 * libraries not available on all devices
	 */
	public static boolean isEffectSupported(int effectId) {
		...
		try {
			...
			case EFFECT_GOOFY_FACE:
				return (Boolean) sFilterIsAvailable
						.invoke(null,
	...
	public EffectsRecorder(Context context) {
		...
		if (!sReflectionInited) {
			try {
				...
				Class<?> clsPoint = Class
						.forName("android.filterfw.geometry.Point");
				sCtPoint = clsPoint.getConstructor(new Class[] { float.class,
						float.class });

				Class<?> clsQuad = Class
						.forName("android.filterfw.geometry.Quad");
				sCtQuad = clsQuad.getConstructor(new Class[] { clsPoint,
						clsPoint, clsPoint, clsPoint });

				Class<?> clsBackDropperFilter = Class
						.forName("android.filterpacks.videoproc.BackDropperFilter");
				sClsLearningDoneListener = Class
						.forName("android.filterpacks.videoproc.BackDropperFilter$LearningDoneListener");
				sLearningDoneListenerOnLearningDone = sClsLearningDoneListener
						.getMethod("onLearningDone",
								new Class[] { clsBackDropperFilter });

				sObjectEquals = Object.class.getMethod("equals",
						new Class[] { Object.class });
				sObjectToString = Object.class.getMethod("toString");

				sClsOnRunnerDoneListener = Class
						.forName("android.filterfw.core.GraphRunner$OnRunnerDoneListener");
				sOnRunnerDoneListenerOnRunnerDone = sClsOnRunnerDoneListener
						.getMethod("onRunnerDone", new Class[] { int.class });

				sClsGraphRunner = Class
						.forName("android.filterfw.core.GraphRunner");
				...
				Class<?> clsFilterContext = Class
						.forName("android.filterfw.core.FilterContext");
				sFilterContextGetGLEnvironment = clsFilterContext
						.getMethod("getGLEnvironment");

				Class<?> clsFilterGraph = Class
						.forName("android.filterfw.core.FilterGraph");
				...
				sClsGraphEnvironment = Class
						.forName("android.filterfw.GraphEnvironment");
				sCtGraphEnvironment = sClsGraphEnvironment.getConstructor();
				sGraphEnvironmentCreateGLEnvironment = sClsGraphEnvironment
						.getMethod("createGLEnvironment");
				sGraphEnvironmentGetRunner = sClsGraphEnvironment.getMethod(
						"getRunner", new Class[] { int.class, int.class });
				sGraphEnvironmentAddReferences = sClsGraphEnvironment
						.getMethod("addReferences",
								new Class[] { Object[].class });
				sGraphEnvironmentLoadGraph = sClsGraphEnvironment.getMethod(
						"loadGraph", new Class[] { Context.class, int.class });
				sGraphEnvironmentGetContext = sClsGraphEnvironment
						.getMethod("getContext");

				Class<?> clsGLEnvironment = Class
						.forName("android.filterfw.core.GLEnvironment");
				sGLEnvironmentIsActive = clsGLEnvironment.getMethod("isActive");
				sGLEnvironmentActivate = clsGLEnvironment.getMethod("activate");
				sGLEnvironmentDeactivate = clsGLEnvironment
						.getMethod("deactivate");

				Class<?> clsSurfaceTextureTarget = Class
						.forName("android.filterpacks.videosrc.SurfaceTextureTarget");
				sSurfaceTextureTargetDisconnect = clsSurfaceTextureTarget
						.getMethod("disconnect",
								new Class[] { clsFilterContext });

				sClsOnRecordingDoneListener = Class
						.forName("android.filterpacks.videosink.MediaEncoderFilter$OnRecordingDoneListener");
				sOnRecordingDoneListenerOnRecordingDone = sClsOnRecordingDoneListener
						.getMethod("onRecordingDone");

				sClsSurfaceTextureSourceListener = Class
						.forName("android.filterpacks.videosrc.SurfaceTextureSource$SurfaceTextureSourceListener");
				sSurfaceTextureSourceListenerOnSurfaceTextureSourceReady = sClsSurfaceTextureSourceListener
						.getMethod("onSurfaceTextureSourceReady",
	...
	/**
	 * Sets the maximum filesize (in bytes) of the recording session. This will
	 * be passed on to the MediaEncoderFilter and then to the MediaRecorder
	 * ultimately. If zero or negative, the MediaRecorder will disable the limit
	 */
	...
	/**
	 * Sets the maximum recording duration (in ms) for the next recording
	 * session Setting it to zero (the default) disables the limit.
	 */
	public synchronized void setMaxDuration(int maxDurationMs) {
		switch (mState) {
		case STATE_RECORD:
			throw new RuntimeException(
					"setMaxDuration cannot be called while recording!");
		case STATE_RELEASED:
			throw new RuntimeException(
	...
	public void setCaptureRate(double fps) {
		...
		case STATE_RECORD:
			throw new RuntimeException(
					"setCaptureRate cannot be called while recording!");
		case STATE_RELEASED:
			throw new RuntimeException(
	...
	public void setEffect(int effect, Object effectParameter) {
		if (mLogVerbose)
			Log.v(TAG, "setEffect: effect ID " + effect + ", parameter "
					+ effectParameter.toString());
		switch (mState) {
		case STATE_RECORD:
			throw new RuntimeException(
					"setEffect cannot be called while recording!");
		case STATE_RELEASED:
			throw new RuntimeException(
					"setEffect called on an already released recorder!");
		default:
			break;
		}
	...
	private void setRecordingOrientation() {
		if (mState != STATE_RECORD && mRunner != null) {
			...
			if (mCameraFacing == Camera.CameraInfo.CAMERA_FACING_BACK) {
				...
				recordingRegion = newInstance(sCtQuad, new Object[] { bl, br,
						tl, tr });
			} else {
				// Recording region needs to be tweaked for front cameras, since
				// they
				// mirror their preview
				if (mOrientationHint == 0 || mOrientationHint == 180) {
					// Horizontal flip in landscape
					recordingRegion = newInstance(sCtQuad, new Object[] { br,
							bl, tr, tl });
				} else {
					// Horizontal flip in portrait
					recordingRegion = newInstance(sCtQuad, new Object[] { tl,
							tr, bl, br });
				}
			}
	...
	public void setOrientationHint(int degrees) {
		...
		if (mLogVerbose)
			Log.v(TAG, "Setting orientation hint to: " + degrees);
	...
	public void setOnInfoListener(MediaRecorder.OnInfoListener infoListener) {
		...
		case STATE_RECORD:
			throw new RuntimeException(
					"setInfoListener cannot be called while recording!");
		case STATE_RELEASED:
			throw new RuntimeException(
	...
	public void setOnErrorListener(MediaRecorder.OnErrorListener errorListener) {
		...
		case STATE_RECORD:
			throw new RuntimeException(
					"setErrorListener cannot be called while recording!");
		case STATE_RELEASED:
			throw new RuntimeException(
	...
	private void initializeFilterFramework() {
		...
		invoke(mGraphEnv, sGraphEnvironmentAddReferences,
				new Object[] { new Object[] { "textureSourceCallback",
						mSourceReadyCallback, "recordingWidth",
						videoFrameWidth, "recordingHeight", videoFrameHeight,
						"recordingProfile", mProfile, "learningDoneListener",
						mLearningDoneListener, "recordingDoneListener",
	...
	private synchronized void initializeEffect(boolean forceReset) {
		if (forceReset || mCurrentEffect != mEffect
				|| mCurrentEffect == EFFECT_BACKDROPPER) {

			invoke(mGraphEnv, sGraphEnvironmentAddReferences,
					new Object[] { new Object[] { "previewSurfaceTexture",
							mPreviewSurfaceTexture, "previewWidth",
							mPreviewWidth, "previewHeight", mPreviewHeight,
			...
			case EFFECT_GOOFY_FACE:
				mGraphId = (Integer) invoke(mGraphEnv,
						sGraphEnvironmentLoadGraph, new Object[] { mContext,
								R.raw.goofy_face });
				break;
			case EFFECT_BACKDROPPER:
			...
			mRunner = invoke(
					mGraphEnv,
					sGraphEnvironmentGetRunner,
					new Object[] {
							mGraphId,
							getConstant(sClsGraphEnvironment,
									"MODE_ASYNCHRONOUS") });
			invoke(mRunner, sGraphRunnerSetDoneCallback,
					new Object[] { mRunnerDoneCallback });
			if (mLogVerbose) {
				Log.v(TAG, "New runner: " + mRunner + ". Old runner: "
						+ mOldRunner);
			}
			if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
		...
		case EFFECT_BACKDROPPER:
			...
			// For front camera, the background video needs to be mirrored in
			// the
			// backdropper filter
	...
	public synchronized void startPreview() {
		...
		case STATE_RECORD:
			throw new RuntimeException(
					"Cannot start preview when already recording!");
		case STATE_RELEASED:
			throw new RuntimeException(
					"setEffect called on an already released recorder!");
		default:
	...
	private void invokeOnRunnerDone(Object[] args) {
		...
		synchronized (EffectsRecorder.this) {
			if (mLogVerbose) {
				Log.v(TAG, "Graph runner done (" + EffectsRecorder.this
						+ ", mRunner " + mRunner + ", mOldRunner " + mOldRunner
						+ ")");
			}
			if (runnerDoneResult == (Integer) getConstant(sClsGraphRunner,
					"RESULT_ERROR")) {
				// Handle error case
			...
			if (mOldRunner != null) {
				...
				if (glEnv != null
						&& !(Boolean) invoke(glEnv, sGLEnvironmentIsActive)) {
			...
			if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
				...
				if (mLogVerbose) {
					Log.v(TAG,
							"Previous effect halted. Running graph again. state: "
									+ mState);
				}
				tryEnable3ALocks(false);
				// In case of an error, the graph restarts from beginning and in
				// case
				// of the BACKDROPPER effect, the learner re-learns the
				// background.
				// Hence, we need to show the learning dialogue to the user
				// to avoid recording before the learning is done. Else, the
				// user
				...
				// For BACKDROPPER effect, this path is also executed sometimes
				// at
				// the end of a normal recording session. In such a case, the
				// graph
				// does not restart and hence the learner does not re-learn. So
				// we
				// do not want to show the learning dialogue then.
				if (runnerDoneResult == (Integer) getConstant(sClsGraphRunner,
						"RESULT_ERROR") && mCurrentEffect == EFFECT_BACKDROPPER) {
					sendMessage(EFFECT_BACKDROPPER, EFFECT_MSG_STARTED_LEARNING);
				}
				invoke(mRunner, sGraphRunnerRun);
				...
				// STATE_RELEASED - camera will be/has been released as well, do
				// nothing.
			}
		}
	}
	...
	private void invokeOnSurfaceTextureSourceReady(Object[] args) {
		...
		synchronized (EffectsRecorder.this) {
			...
			if (mState == STATE_CONFIGURE) {
				// Stop preview happened while the runner was doing startup
				// tasks
			...
			if (mState == STATE_RELEASED) {
				// EffectsRecorder has been released, so don't touch the camera
				// device
				// or anything else
				if (mLogVerbose)
			...
			if (source == null) {
				if (mLogVerbose) {
					Log.v(TAG,
							"Ready callback: source null! Looks like graph was closed!");
				}
				if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW
						|| mState == STATE_RECORD) {
	...
	private void invokeOnRecordingDone() {
		// Forward the callback to the VideoModule object (as an asynchronous
		// event).
		if (mLogVerbose)
	...
	public synchronized void startRecording() {
		...
		if ((mOutputFile == null) && (mFd == null)) {
			throw new RuntimeException(
					"No output file name or descriptor provided!");
		}
		...
		setInputValue(recorder, "audioSource",
				MediaRecorder.AudioSource.CAMCORDER);
		setInputValue(recorder, "recordingProfile", mProfile);
		setInputValue(recorder, "orientationHint", mOrientationHint);
		// Important to set the timelapseinterval to 0 if the capture rate is
		// not >0
		// since the recorder does not get created every time the recording
		// starts.
		// The recorder infers whether the capture is timelapsed based on the
		// value of
		// this interval
	...
	public synchronized void stopRecording() {
		...
		case STATE_RELEASED:
			throw new RuntimeException(
					"stopRecording called on released EffectsRecorder!");
		default:
			break;
		}
		Object recorder = getGraphFilter(mRunner, "recorder");
	...
	// Called to tell the filter graph that the display surfacetexture is not
	// valid anymore.
	// So the filter graph should not hold any reference to the surface created
	// with that.
	public synchronized void disconnectDisplay() {
		...
		invoke(display, sSurfaceTextureTargetDisconnect,
				new Object[] { invoke(mGraphEnv, sGraphEnvironmentGetContext) });
	}
	...
	public synchronized void stopPreview() {
		...
		case STATE_RELEASED:
			throw new RuntimeException(
					"stopPreview called on released EffectsRecorder!");
		default:
			break;
		}
		...
		// This will not do anything if the camera has already been
		// disconnected.
		stopCameraPreview();
	...
	// Try to enable/disable video stabilization if supported; otherwise return
	// false
	...
	synchronized void enable3ALocks(boolean toggle) {
		...
		if (!tryEnable3ALocks(toggle)) {
			throw new RuntimeException(
					"Attempt to lock 3A on camera with no locking support!");
		}
	}
		...
		public Object invoke(Object proxy, Method method, Object[] args)
			...
			if (method.equals(sObjectEquals)) {
				...
				sEffectsRecorder.invokeOnRunnerDone(args);
					...
					.equals(sSurfaceTextureSourceListenerOnSurfaceTextureSourceReady)) {
	...
	private Object getGraphFilter(Object receiver, String name) {
		try {
			return sFilterGraphGetFilter.invoke(
					sGraphRunnerGetGraph.invoke(receiver),
					new Object[] { name });
		} catch (Exception ex) {
			throw new RuntimeException(ex);
		}
