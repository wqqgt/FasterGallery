...
public class CaptureAnimManager {
	...
	private volatile int mAnimOrientation; // Could be 0, 90, 180 or 270
											// degrees.
	private long mAnimStartTime; // milliseconds.
	private float mX; // The center of the whole view including preview and
						// review.
	private float mY;
	...
	/*
	 * preview: camera preview view. review: view of picture just taken.
	 */
	...
	private void setAnimationGeometry(int x, int y, int w, int h) {
		mMarginRight = mResources
				.getDimensionPixelSize(R.dimen.capture_margin_right);
		mMarginTop = mResources
				.getDimensionPixelSize(R.dimen.capture_margin_top);
	...
	public boolean drawAnimation(GLCanvas canvas, CameraScreenNail preview,
		...
		if (mAnimType == ANIM_SLIDE && timeDiff > TIME_SLIDE2 - TIME_HOLD)
			return false;
		if (mAnimType == ANIM_BOTH && timeDiff > TIME_SLIDE2)
			return false;
		...
		if (animStep == ANIM_FLASH) {
			...
			float fraction = mSlideInterpolator
					.getInterpolation((float) (timeDiff)
							/ (TIME_SLIDE - TIME_HOLD));
			float x = mX;
			float y = mY;
			float w = 0;
			float h = 0;
			...
			preview.directDraw(canvas, (int) mX, (int) mY, mDrawWidth,
					mDrawHeight);
			...
			mBorder.draw(canvas, (int) mHoldX - mShadowSize, (int) mHoldY
					- mShadowSize, (int) mHoldW + 2 * mShadowSize, (int) mHoldH
					+ 2 * mShadowSize);
			...
			mBorder.draw(canvas, (int) x - mShadowSize, (int) y - mShadowSize,
					(int) mHoldW + 2 * mShadowSize, (int) mHoldH + 2
							* mShadowSize);
