...
class PanoProgressBar extends ImageView {
	...
	public void setProgress(int progress) {
		// The panning direction will be decided after user pan more than 10
		// degrees in one
		// direction.
		...
		// mDirection might be modified by setRightIncreasing() above. Need to
		// check again.
		if (mDirection != DIRECTION_NONE) {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mDirection != DIRECTION_NONE) {
			// the progress area
			canvas.drawRect(mLeftMostProgress, mDrawBounds.top,
					mRightMostProgress, mDrawBounds.bottom, mDoneAreaPaint);
			// the indication bar
			float l;
			float r;
			if (mDirection == DIRECTION_RIGHT) {
			...
			canvas.drawRect(l, mDrawBounds.top, r, mDrawBounds.bottom,
					mIndicatorPaint);
		}
