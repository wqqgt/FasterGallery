...
public class PieRenderer extends OverlayRenderer implements FocusIndicator {
	...
	private void init(Context ctx) {
		...
		mRadiusInc = (int) res
				.getDimensionPixelSize(R.dimen.pie_radius_increment);
		mCircleSize = mRadius
				- res.getDimensionPixelSize(R.dimen.focus_radius_offset);
		mTouchOffset = (int) res
				.getDimensionPixelSize(R.dimen.pie_touch_offset);
	...
	private void layoutLabel(int level) {
		int x = mPieCenterX
				- (int) (FloatMath.sin(mCenterAngle - CENTER) * (mArcRadius + (level + 2)
						* mRadiusInc));
	...
	private void layoutItems(int level, List<PieItem> items) {
		int extend = 1;
		Path path = makeSlice(getDegrees(0) + extend, getDegrees(SWEEP_ARC)
				- extend, mArcRadius, mArcRadius + mRadiusInc + mRadiusInc / 4,
	...
	private Path makeSlice(float start, float end, int inner, int outer,
			int cx, int cy) {
		RectF bb = new RectF(cx - outer, cy - outer, cx + outer, cy + outer);
		RectF bbi = new RectF(cx - inner, cy - inner, cx + inner, cy + inner);
		Path path = new Path();
	...
	private float getSliceCenter(PieItem item, int pos, int count) {
		...
		return center + (count - 1) * SWEEP_SLICE / 2f - pos * SWEEP_SLICE;
	}
	 ...
	 * 
	 * @param angle
	 *            from 0..PI to Android degrees (clockwise starting at 3
	 *            o'clock)
	 * @return skia angle
	 */
	...
	public void onDraw(Canvas canvas) {
		...
		if (!hasOpenItem() || (mXFade != null)) {
			...
			for (PieItem item : getParent().getItems()) {
				drawItem(Math.max(0, mOpen.size() - 2), pos, count, canvas,
						item, alpha);
				pos++;
			}
			mLabel.draw(canvas);
		...
		if (hasOpenItem()) {
			...
			for (PieItem inner : items) {
				if (mFadeOut != null) {
					...
					drawItem(level, pos, count, canvas, inner,
							(mXFade != null) ? (1 - 0.5f * alpha) : 1);
				}
				pos++;
			}
			mLabel.draw(canvas);
	...
	private void drawArc(Canvas canvas, int level, PieItem item) {
		...
		if (mState == STATE_PIE) {
			...
			canvas.drawArc(new RectF(mPieCenterX - mArcRadius, cy - mArcRadius,
					...
					getDegrees(end), getDegrees(start) - getDegrees(end),
					false, mMenuArcPaint);
		}
	}

	private void drawItem(int level, int pos, int count, Canvas canvas,
			PieItem item, float alpha) {
		if (mState == STATE_PIE) {
	...
	public boolean onTouchEvent(MotionEvent evt) {
		...
		if (MotionEvent.ACTION_DOWN == action) {
			...
			if ((item != null) && (mCurrentItem != item)
				...
				onEnterSelect(item);
				mHandler.sendEmptyMessageDelayed(MSG_OPENSUBMENU,
						PIE_OPEN_SUB_DELAY);
			}
		}
		return false;
	...
	private boolean inside(PointF polar, PieItem item, int pos, int count) {
		...
		boolean res = (mArcRadius < polar.y) && (start < polar.x)
	...
	private void onEnterOpen() {
		if ((mCurrentItem != null) && (mCurrentItem != getOpenItem())
				&& mCurrentItem.hasItems()) {
			openCurrentItem();
		}
	}
	...
	private void setCircle(int cx, int cy) {
		mCircle.set(cx - mCircleSize, cy - mCircleSize, cx + mCircleSize, cy
				+ mCircleSize);
		mDial.set(cx - mCircleSize + mInnerOffset, cy - mCircleSize
				+ mInnerOffset, cx + mCircleSize - mInnerOffset, cy
				+ mCircleSize - mInnerOffset);
	}
	...
	public void drawFocus(Canvas canvas) {
		...
		canvas.drawCircle((float) mFocusX, (float) mFocusY,
				(float) mCircleSize, mFocusPaint);
		if (mState == STATE_PIE)
			return;
		...
		canvas.save();
		// rotate the arc instead of its offset to better use framework's shape
		// caching
	...
	private void drawLine(Canvas canvas, int angle, Paint p) {
		...
		convertCart(angle, mCircleSize - mInnerOffset + mInnerOffset / 3,
				mPoint2);
		canvas.drawLine(mPoint1.x + mFocusX, mPoint1.y + mFocusY, mPoint2.x
				+ mFocusX, mPoint2.y + mFocusY, p);
	}
	...
	public void showStart() {
		...
		startAnimation(SCALING_UP_TIME, false, mStartAnimationAngle,
				mStartAnimationAngle + range);
