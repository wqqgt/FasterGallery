...
public class CameraSwitcher extends RotateImageView implements OnClickListener,
		OnTouchListener {
	...
	private static final int[] DRAW_IDS = { R.drawable.ic_switch_camera,
			R.drawable.ic_switch_video, R.drawable.ic_switch_pan,
	...
	public void initializeDrawables(Context context) {
		int totaldrawid = (LightCycleHelper.hasLightCycleCapture(context) ? DRAW_IDS.length
				: DRAW_IDS.length - 1);
		...
		for (int i = 0; i < DRAW_IDS.length; i++) {
			...
			if (i == LIGHTCYCLE_MODULE_INDEX
					&& !LightCycleHelper.hasLightCycleCapture(context)) {
				continue; // not enabled, so don't add to UI
			}
			moduleids[ix] = i;
	...
	private void onCameraSelected(int ix) {
		...
		if ((ix != mCurrentIndex) && (mListener != null)) {
			...
			UsageStatistics
					.setPendingTransitionCause(UsageStatistics.TRANSITION_MENU_TAP);
			setCurrentIndex(ix);
	...
	private void initPopup() {
		mParent = LayoutInflater.from(getContext()).inflate(
				R.layout.switcher_popup, (ViewGroup) getParent());
		LinearLayout content = (LinearLayout) mParent
		...
		for (int i = mDrawIds.length - 1; i >= 0; i--) {
			...
			case R.drawable.ic_switch_camera:
				item.setContentDescription(getContext().getResources()
						.getString(R.string.accessibility_switch_to_camera));
				break;
			case R.drawable.ic_switch_video:
				item.setContentDescription(getContext().getResources()
						.getString(R.string.accessibility_switch_to_video));
				break;
			case R.drawable.ic_switch_pan:
				item.setContentDescription(getContext().getResources()
						.getString(R.string.accessibility_switch_to_panorama));
				break;
			...
			content.addView(item, new LinearLayout.LayoutParams(mItemSize,
					mItemSize));
		}
		mPopup.measure(MeasureSpec.makeMeasureSpec(mParent.getWidth(),
				MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(
				mParent.getHeight(), MeasureSpec.AT_MOST));
	}
	...
	public boolean isInsidePopup(MotionEvent evt) {
		...
		return evt.getX() >= left && evt.getX() < right && evt.getY() >= top
				&& evt.getY() < bottom;
	}
	...
	private void layoutPopup() {
		...
		if (orientation == 0) {
			mPopup.layout(getRight() - w, getBottom() - h, getRight(),
					getBottom());
			mTranslationX = 0;
			mTranslationY = h / 3;
			...
			mPopup.layout(getLeft(), getBottom() - h, getLeft() + w,
					getBottom());
		}
	}
	...
	private boolean animateHidePopup() {
		...
		mPopup.animate().alpha(0f).scaleX(0.3f).scaleY(0.3f)
				.translationX(mTranslationX).translationY(mTranslationY)
	...
	private boolean animateShowPopup() {
		...
		mPopup.animate().alpha(1f).scaleX(1f).scaleY(1f).translationX(0)
				.translationY(0).setDuration(SWITCHER_POPUP_ANIM_DURATION)
				.setListener(null);
