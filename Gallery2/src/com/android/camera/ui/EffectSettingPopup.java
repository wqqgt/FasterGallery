...
public class EffectSettingPopup extends AbstractSettingPopup implements
	...
	ArrayList<HashMap<String, Object>> mSillyFacesItem = new ArrayList<HashMap<String, Object>>();

	// Data for background replacer items. (text, image, and preference value)
	ArrayList<HashMap<String, Object>> mBackgroundItem = new ArrayList<HashMap<String, Object>>();
	...
	public void initialize(IconListPreference preference) {
		...
		if (hasSillyFaces) {
			findViewById(R.id.effect_silly_faces_title).setVisibility(
					View.VISIBLE);
			findViewById(R.id.effect_silly_faces_title_separator)
			...
			SimpleAdapter sillyFacesItemAdapter = new SimpleAdapter(context,
					...
					new String[] { "text", "image" }, new int[] { R.id.text,
							R.id.image });
		...
		if (hasSillyFaces && hasBackground) {
			findViewById(R.id.effect_background_separator).setVisibility(
					View.VISIBLE);
		}

		// Initialize background replacer if it is supported.
		if (hasBackground) {
			findViewById(R.id.effect_background_title).setVisibility(
					View.VISIBLE);
			findViewById(R.id.effect_background_title_separator).setVisibility(
					View.VISIBLE);
	...
	public void setVisibility(int visibility) {
		if (visibility == View.VISIBLE) {
			if (getVisibility() != View.VISIBLE) {
				...
				mClearEffects
						.setVisibility(noEffect ? View.GONE : View.VISIBLE);
			}
			reloadPreference();
		}
	...
	public void reloadPreference() {
		mBackgroundGrid.setItemChecked(
				mBackgroundGrid.getCheckedItemPosition(), false);
		mSillyFacesGrid.setItemChecked(
				mSillyFacesGrid.getCheckedItemPosition(), false);
