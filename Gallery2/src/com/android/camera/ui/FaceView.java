		...
		if (mFaces != null) {
			if ((faces.length > 0 && mFaces.length == 0)
				...
				if (!mStateSwitchPending) {
					...
					mHandler.sendEmptyMessageDelayed(MSG_SWITCH_FACES,
							SWITCH_DELAY);
				}
				return;
			}
	...
	protected void onDraw(Canvas canvas) {
		if (!mBlocked && (mFaces != null) && (mFaces.length > 0)) {
			...
			if (mUncroppedWidth == 0) {
				// TODO: This check is temporary. It needs to be removed after
				// the
				// refactoring is fully functioning.
				final CameraScreenNail sn = ((CameraActivity) getContext())
						.getCameraScreenNail();
