	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mCurrentInsets != null) {
			if (mCurrentInsets.right > 0) {
				...
				mRightMargin = mRightMargin > 0 ? mRightMargin
						: mCurrentInsets.right;
			} else {
				// navigation bar on the bottom
				mBottomMargin = mBottomMargin > 0 ? mBottomMargin
						: mCurrentInsets.bottom;
			}
		}
	...
	public void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (v instanceof CameraControls) {
				// Lay out camera controls to center on the short side of the
				// screen
				...
				if (orientation == Configuration.ORIENTATION_PORTRAIT) {
					...
					v.layout(left, t + mTopMargin, left + width, b
							- mBottomMargin);
				} else {
					int top = (t + b - height) / 2;
					v.layout(l + mLeftMargin, top, r - mRightMargin, top
							+ height);
				}
			} else {
				v.layout(l + mLeftMargin, t + mTopMargin, r - mRightMargin, b
						- mBottomMargin);
			}
		}
	}
