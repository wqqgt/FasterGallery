...
public class VideoModule implements CameraModule, VideoController,
		...
		MediaRecorder.OnInfoListener, EffectsRecorder.EffectsListener {
	...
	private boolean mRestoreFlash; // This is used to check if we need to
									// restore the flash
	...
	private void openCamera() {
		try {
			synchronized (mCameraOpened) {
				if (!mCameraOpened) {
					mActivity.mCameraDevice = Util.openCamera(mActivity,
							mCameraId);
					mCameraOpened = true;
				}
			}
	...
	public void init(CameraActivity activity, View root, boolean reuseScreenNail) {
		...
		mActivity.mNumberOfCameras = CameraHolder.instance()
				.getNumberOfCameras();
		mPrefVideoEffectDefault = mActivity
	...
	public void onSingleTapUp(View view, int x, int y) {
		if (mMediaRecorderRecording && effectsActive()) {
			new RotateTextToast(mActivity,
					R.string.disable_video_snapshot_hint, mOrientation).show();
			return;
		}

		MediaSaveService s = mActivity.getMediaSaveService();
		if (mPaused || mSnapshotInProgress || effectsActive() || s == null
				|| s.isQueueFull()) {
			return;
		}
		...
		mActivity.mCameraDevice.takePicture(null, null, null,
	...
	private void initializeVideoControl() {
		...
		if (effectsActive()) {
			mUI.overrideSettings(CameraSettings.KEY_VIDEO_QUALITY,
	...
	private void onStopVideoRecording() {
		...
		if (mIsVideoCaptureIntent) {
			...
			if (!mPaused && ApiHelper.HAS_SURFACE_TEXTURE_RECORDING) {
				...
				((CameraScreenNail) mActivity.mCameraScreenNail)
						.animateCapture(mDisplayRotation);
	...
	public void onShutterButtonClick() {
		if (mUI.collapseCameraControls() || mSwitchingCamera)
			return;
	...
	private void setDisplayOrientation() {
		...
		if (ApiHelper.HAS_SURFACE_TEXTURE) {
			...
			mCameraDisplayOrientation = Util
					.getDisplayOrientation(0, mCameraId);
		} else {
			// We need to consider display rotation ourselves.
			mCameraDisplayOrientation = Util.getDisplayOrientation(
	...
	public int onZoomChanged(int index) {
		...
		if (mPaused)
			return index;
		mZoomValue = index;
		if (mParameters == null || mActivity.mCameraDevice == null)
			return index;
		...
		try {
			if (!effectsActive()) {
				if (ApiHelper.HAS_SURFACE_TEXTURE) {
					...
					mActivity.mCameraDevice.setPreviewDisplayAsync(mUI
							.getSurfaceHolder());
				}
	...
	private void setupMediaRecorderPreviewDisplay() {
		...
		if (!ApiHelper.HAS_SURFACE_TEXTURE) {
			mMediaRecorder.setPreviewDisplay(mUI.getSurfaceHolder()
					.getSurface());
			...
			mActivity.mCameraDevice.setDisplayOrientation(Util
					.getDisplayOrientation(mDisplayRotation, mCameraId));
			mActivity.mCameraDevice.startPreviewAsync();
			mPreviewing = true;
			mMediaRecorder.setPreviewDisplay(mUI.getSurfaceHolder()
					.getSurface());
	...
	private void initializeEffectsPreview() {
		...
		// TODO: Confirm none of the following need to go to
		...
		mEffectsRecorder.setPreviewSurfaceTexture(
				screenNail.getSurfaceTexture(), screenNail.getWidth(),
	...
	private void initializeEffectsRecording() {
		...
		mEffectsRecorder.setProfile(mProfile);
		// important to set the capture rate to zero if not timelapsed, since
		// the
		// effectsrecorder object does not get created again for each recording
		// session
		if (mCaptureTimeLapse) {
			mEffectsRecorder
					.setCaptureRate((1000 / (double) mTimeBetweenTimeLapseFrameCaptureMs));
		} else {
		...
		// Set output file
		if (mVideoFileDescriptor != null) {
			mEffectsRecorder.setOutputFile(mVideoFileDescriptor
					.getFileDescriptor());
		} else {
			generateVideoFilename(mProfile.fileFormat);
	...
	private void startVideoRecording() {
		...
		if (!mActivity.mCameraDevice.waitDone())
			return;
	...
	private boolean stopVideoRecording() {
		...
		if (mMediaRecorderRecording) {
			...
			try {
				if (effectsActive()) {
					// This is asynchronous, so we can't add to media store now
					// because thumbnail
					// may not be ready. In such case saveVideo() is called
					// later
					// through a callback from the MediaEncoderFilter to
					// EffectsRecorder,
				...
				AccessibilityUtils.makeAnnouncement(
						mActivity.getShutterButton(),
		...
		if (!effectsActive()) {
			...
			if (!mPaused) {
				...
				if (ApiHelper.HAS_SURFACE_TEXTURE
					...
					((CameraScreenNail) mActivity.mCameraScreenNail)
							.setOneTimeOnFrameDrawnListener(mFrameDrawnListener);
					startPreview();
				}
			}
		}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case REQUEST_EFFECT_BACKDROPPER:
			if (resultCode == Activity.RESULT_OK) {
				// onActivityResult() runs before onResume(), so this parameter
				// will be
				// seen by startPreview from onResume()
				mEffectUriFromGallery = data.getData().toString();
				Log.v(TAG, "Received URI from gallery: "
						+ mEffectUriFromGallery);
				mResetEffect = false;
			} else {
	...
	public synchronized void onEffectsError(Exception exception, String fileName) {
		// TODO: Eventually we may want to show the user an error dialog, and
		// then restart the
		// camera and encoder gracefully. For now, we just delete the file and
		// bail out.
		...
		try {
			if (Class.forName(
					"android.filterpacks.videosink.MediaRecorderStopException")
					.isInstance(exception)) {
				Log.w(TAG,
						"Problem recoding video file. Removing incomplete file.");
				return;
			}
	...
	private boolean updateEffectSelection() {
		...
		if (mEffectType == previousEffectType) {
			if (mEffectType == EffectsRecorder.EFFECT_NONE)
				return false;
			if (mEffectParameter.equals(previousEffectParameter))
				return false;
		}
		Log.v(TAG,
				"New effect selection: "
	...
	public void updateCameraAppView() {
		...
		// When going to and back from gallery, we need to turn off/on the
		// flash.
		if (!mActivity.mShowCameraAppView) {
	...
	private void storeImage(final byte[] data, Location loc) {
		...
		mActivity.getMediaSaveService().addImage(data, title, dateTaken, loc,
				s.width, s.height, orientation, exif, mOnPhotoSavedListener,
				mContentResolver);
	}
	...
	private boolean resetEffect() {
		if (mResetEffect) {
			String value = mPreferences.getString(
					CameraSettings.KEY_VIDEO_EFFECT, mPrefVideoEffectDefault);
