...
public class CameraSettings {
	...
	private void initPreference(PreferenceGroup group) {
		...
		IconListPreference exposure = (IconListPreference) group
				.findPreference(KEY_EXPOSURE);
		CountDownTimerPreference timer = (CountDownTimerPreference) group
				.findPreference(KEY_TIMER);
		ListPreference countDownSoundEffects = group
				.findPreference(KEY_TIMER_SOUND_EFFECTS);
	...
	public static Object readEffectParameter(SharedPreferences pref) {
		...
		String effectParameter = effectSelection.substring(separatorIndex + 1);
	...
	private void initVideoEffect(PreferenceGroup group,
			ListPreference videoEffect) {
		CharSequence[] values = videoEffect.getEntryValues();

		boolean goofyFaceSupported = EffectsRecorder
				.isEffectSupported(EffectsRecorder.EFFECT_GOOFY_FACE);
		boolean backdropperSupported = EffectsRecorder
				.isEffectSupported(EffectsRecorder.EFFECT_BACKDROPPER)
		...
		for (CharSequence value : values) {
			...
			if (!goofyFaceSupported && effectSelection.startsWith("goofy_face"))
				continue;
			if (!backdropperSupported
					&& effectSelection.startsWith("backdropper"))
				continue;
