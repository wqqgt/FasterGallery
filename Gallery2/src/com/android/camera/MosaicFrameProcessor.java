...
public class MosaicFrameProcessor {
	...
	public interface ProgressListener {
		public void onProgress(boolean isFinished, float panningRateX,
				float panningRateY, float progressX, float progressY);
	}
	...
	public void initialize(int previewWidth, int previewHeight, int bufSize) {
		...
		// after this initialization before calling other methods of this
		// object.
	}
	...
	private void setupMosaicer(int previewWidth, int previewHeight, int bufSize) {
		Log.v(TAG, "setupMosaicer w, h=" + previewWidth + ',' + previewHeight
				+ ',' + bufSize);
	...
	public void reset() {
		// reset() can be called even if MosaicFrameProcessor is not
		// initialized.
	...
	public void processFrame() {
		if (!mIsMosaicMemoryAllocated) {
			...
			// This can happen when the onPause() is called in the activity, but
			// still some frames
		...
		if (mCurrProcessFrameIdx != mLastProcessFrameIdx) {
			...
			if (mTotalFrameCount < MAX_NUMBER_OF_FRAMES) {
				...
				if (mProgressListener != null) {
					mProgressListener.onProgress(false, mPanningRateX,
							mPanningRateY, mTranslationLastX
									* HR_TO_LR_DOWNSAMPLE_FACTOR
									/ mPreviewWidth, mTranslationLastY
									* HR_TO_LR_DOWNSAMPLE_FACTOR
									/ mPreviewHeight);
				}
			} else {
				if (mProgressListener != null) {
					mProgressListener.onProgress(true, mPanningRateX,
							mPanningRateY, mTranslationLastX
									* HR_TO_LR_DOWNSAMPLE_FACTOR
									/ mPreviewWidth, mTranslationLastY
									* HR_TO_LR_DOWNSAMPLE_FACTOR
	...
	public void calculateTranslationRate() {
		...
		// The panning rate is measured as the rate of the translation
		// percentage in
		// image width/height. Take the horizontal panning rate for example, the
		// image width
		// used in finding the translation is (PreviewWidth /
		// HR_TO_LR_DOWNSAMPLE_FACTOR).
		// To get the horizontal translation percentage, the horizontal
		// translation,
		// (translationCurrX - mTranslationLastX), is divided by the
		// image width. We then get the rate by dividing the translation
		// percentage with the
		// number of frames.
