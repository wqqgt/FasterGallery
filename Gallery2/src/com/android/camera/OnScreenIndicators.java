...
public class OnScreenIndicators {
	...
	public OnScreenIndicators(Context ctx, View onScreenIndicatorsView) {
		...
		mWBArray = new int[n];
		for (int i = 0; i < n; i++) {
			mWBArray[i] = iconIds.getResourceId(i,
					R.drawable.ic_indicator_wb_off);
		}
		mOnScreenIndicators = onScreenIndicatorsView;
		mExposureIndicator = (ImageView) onScreenIndicatorsView
				.findViewById(R.id.menu_exposure_indicator);
		mFlashIndicator = (ImageView) onScreenIndicatorsView
				.findViewById(R.id.menu_flash_indicator);
		mSceneIndicator = (ImageView) onScreenIndicatorsView
				.findViewById(R.id.menu_scenemode_indicator);
		mLocationIndicator = (ImageView) onScreenIndicatorsView
				.findViewById(R.id.menu_location_indicator);
		mTimerIndicator = (ImageView) onScreenIndicatorsView
				.findViewById(R.id.menu_timer_indicator);
		mWBIndicator = (ImageView) onScreenIndicatorsView
				.findViewById(R.id.menu_wb_indicator);
	}
	 ...
	 */
	public void updateExposureOnScreenIndicator(Camera.Parameters params,
			int value) {
	 ...
	 * 
	 * @param value
	 *            Value between -3 and 3. If outside this range, 0 is used by
	 *            default.
	 */
	 ...
	 * 
	 * @param value
	 *            One of Parameters.FLASH_MODE_OFF, Parameters.FLASH_MODE_AUTO,
	...
	public void updateFlashOnScreenIndicator(String value) {
		...
		if (value == null || Parameters.FLASH_MODE_OFF.equals(value)) {
			...
			if (Parameters.FLASH_MODE_AUTO.equals(value)) {
				mFlashIndicator
						.setImageResource(R.drawable.ic_indicator_flash_auto);
			} else if (Parameters.FLASH_MODE_ON.equals(value)
					|| Parameters.FLASH_MODE_TORCH.equals(value)) {
				mFlashIndicator
						.setImageResource(R.drawable.ic_indicator_flash_on);
			} else {
				mFlashIndicator
						.setImageResource(R.drawable.ic_indicator_flash_off);
			}
		}
	}
	 ...
	 * 
	 * @param value
	 *            the current Parameters.SCENE_MODE_* value.
	 */
