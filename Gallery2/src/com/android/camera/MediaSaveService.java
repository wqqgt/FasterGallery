...
public class MediaSaveService extends Service {
	...
	public void addImage(final byte[] data, String title, long date,
		...
		ImageSaveTask t = new ImageSaveTask(data, title, date,
				(loc == null) ? null : new Location(loc), width, height,
				orientation, exif, resolver, l);
	...
	private void onQueueFull() {
		if (mListener != null)
			mListener.onQueueStatus(true);
	}

	private void onQueueAvailable() {
		if (mListener != null)
			mListener.onQueueStatus(false);
	}
	...
	private class ImageSaveTask extends AsyncTask<Void, Void, Uri> {
		...
		public ImageSaveTask(byte[] data, String title, long date,
				Location loc, int width, int height, int orientation,
		...
		protected void onPostExecute(Uri uri) {
			...
			mTaskNumber--;
			if (mTaskNumber == SAVE_TASK_LIMIT - 1)
				onQueueAvailable();
		}
	}
	...
	private class VideoSaveTask extends AsyncTask<Void, Void, Uri> {
		...
		protected Uri doInBackground(Void... v) {
			...
			try {
				...
				// certain that the previous insert to MediaProvider is
				// completed.
