...
public class CameraScreenNail extends SurfaceTextureScreenNail {
	...
	private final DrawClient mDefaultDraw = new DrawClient() {
		...
		@Override
		public RawTexture copyToTexture(GLCanvas c, RawTexture texture, int w,
				int h) {
	...
	public interface DrawClient {
		...
		// false;
		RawTexture copyToTexture(GLCanvas c, RawTexture texture, int width,
				int height);
	}
	...
	private void updateRenderSize() {
		...
		if (getTextureWidth() > getTextureHeight()) {
			aspectRatio = (float) getTextureWidth()
					/ (float) getTextureHeight();
		} else {
			aspectRatio = (float) getTextureHeight()
		...
		Log.i(TAG, "aspect ratio clamping enabled, surfaceTexture scale: "
				+ mScaleX + ", " + mScaleY);
	}
	...
	public void acquireSurfaceTexture() {
		synchronized (mLock) {
			...
			mAnimTexture = new RawTexture(getTextureWidth(),
	...
	public void draw(GLCanvas canvas, int x, int y, int width, int height) {
		synchronized (mLock) {
			...
			if (mDraw.requiresSurfaceTexture()
					&& (surfaceTexture == null || !mFirstFrameArrived)) {
				return;
			}
			...
			case ANIM_SWITCH_WAITING_FIRST_FRAME:
				...
				mSwitchAnimManager.drawDarkPreview(canvas, x, y, width, height,
						mAnimTexture);
				break;
			...
			if (mAnimState == ANIM_CAPTURE_RUNNING
				...
				if (mAnimState == ANIM_CAPTURE_RUNNING) {
					if (!mFullScreen) {
						...
						drawn = mCaptureAnimManager.drawAnimation(canvas, this,
								mAnimTexture, x, y, width, height);
					}
				} else {
	...
	private void copyPreviewTexture(GLCanvas canvas) {
		if (!mDraw.requiresSurfaceTexture()) {
			mAnimTexture = mDraw.copyToTexture(canvas, mAnimTexture,
			...
			canvas.drawTexture(mExtTexture, mTextureTransformMatrix, 0, 0,
					width, height);
