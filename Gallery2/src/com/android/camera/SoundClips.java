...
public class SoundClips {
	...
	/**
	 * This class implements SoundClips.Player using MediaActionSound, which
	 * exists since API level 16.
	 */
	...
	/**
	 * This class implements SoundClips.Player using SoundPool, which exists
	 * since API level 1.
	 */
	private static class SoundPoolPlayer implements Player,
		...
		R.raw.focus_complete, R.raw.video_record };
		...
		public synchronized void play(int action) {
			if (action < 0 || action >= mSoundRes.length) {
				Log.e(TAG, "Resource ID not found for action:" + action
						+ " in play().");
				return;
			}
