...
/**
 * This class helps an application manage a list of connected MTP or PTP
 * devices. It listens for MTP devices being attached and removed from the USB
 * host bus and notifies the application when the MTP device list changes.
 */
...
public class MtpClient {
	...
	// We add devices to this list if the user canceled a permission request or
	// we were
	...
	/**
	 * An interface for being notified when MTP or PTP devices are attached or
	 * removed. In the current implementation, only PTP devices are supported.
	 */
	...
	/**
	 * Tests to see if a {@link android.hardware.usb.UsbDevice} supports the PTP
	 * protocol (typically used by digital cameras)
	 * 
	 * @param device
	 *            the device to test
	 ...
	 * 
	 * @param context
	 *            the {@link android.content.Context} to use for the MtpClient
	 */
	...
	/**
	 * Registers a {@link com.android.fastergallery.data.MtpClient.Listener}
	 * interface to receive notifications when MTP or PTP devices are added or
	 * removed.
	 * 
	 * @param listener
	...
	/**
	 * Unregisters a {@link com.android.fastergallery.data.MtpClient.Listener}
	 * interface.
	 * 
	 * @param listener
	...
	/**
	 * Retrieves an {@link android.mtp.MtpDevice} object for the USB device with
	 * the given name.
	 * 
	 * @param deviceName
	...
	/**
	 * Retrieves a list of all {@link android.mtp.MtpStorageInfo} for the MTP or
	 * PTP device with the given USB device name
	 * 
	 * @param deviceName
	 *            the name of the USB device
	...
	public List<MtpStorageInfo> getStorageList(String deviceName) {
		...
		ArrayList<MtpStorageInfo> storageList = new ArrayList<MtpStorageInfo>(
				length);
	...
	/**
	 * Retrieves the {@link android.mtp.MtpObjectInfo} for an object on the MTP
	 * or PTP device with the given USB device name with the given object handle
	 * 
	 * @param deviceName
	 *            the name of the USB device
	 * @param objectHandle
	...
	/**
	 * Deletes an object on the MTP or PTP device with the given USB device
	 * name.
	 * 
	 * @param deviceName
	 *            the name of the USB device
	...
	/**
	 * Retrieves a list of {@link android.mtp.MtpObjectInfo} for all objects on
	 * the MTP or PTP device with the given USB device name and given storage ID
	 * and/or object handle. If the object handle is zero, then all objects in
	 * the root of the storage unit will be returned. Otherwise, all immediate
	 * children of the object will be returned. If the storage ID is also zero,
	 * then all objects on all storage units will be returned.
	 * 
	 * @param deviceName
	 *            the name of the USB device
	 * @param storageId
	 *            the ID of the storage unit to query, or zero for all
	 * @param objectHandle
	 *            the handle of the parent object to query, or zero for the
	 *            storage root
	 * @return the list of MtpObjectInfo
	 */
	public List<MtpObjectInfo> getObjectList(String deviceName, int storageId,
			int objectHandle) {
		MtpDevice device = getDevice(deviceName);
		if (device == null) {
			return null;
		...
		ArrayList<MtpObjectInfo> objectList = new ArrayList<MtpObjectInfo>(
				length);
		for (int i = 0; i < length; i++) {
	 ...
	 *            handle of the object to read
	 * @param destPath
	 *            path to destination for the file transfer. This path should be
	 *            in the external storage as defined by
	 *            {@link android.os.Environment#getExternalStorageDirectory}
	 * @return true if the file transfer succeeds
	 */
	public boolean importFile(String deviceName, int objectHandle,
			String destPath) {
		MtpDevice device = getDevice(deviceName);
		if (device == null) {
			return false;
