...
public class SizeClustering extends Clustering {
	...
	public void run(MediaSet baseSet) {
		...
		for (int i = group.length - 1; i >= 0; i--) {
			...
			mClusters[k] = group[i];
			if (i == 0) {
				mNames[k] = String.format(res.getString(R.string.size_below),
						getSizeString(i + 1));
			} else if (i == group.length - 1) {
				mNames[k] = String.format(res.getString(R.string.size_above),
						getSizeString(i));
			} else {
				String minSize = getSizeString(i);
				String maxSize = getSizeString(i + 1);
				mNames[k] = String.format(res.getString(R.string.size_between),
						minSize, maxSize);
			}
