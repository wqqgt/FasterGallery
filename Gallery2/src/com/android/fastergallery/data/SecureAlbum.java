...
public class SecureAlbum extends MediaSet implements StitchingChangeListener {
	...
	public SecureAlbum(Path path, GalleryApp application, MediaItem unlock) {
		...
		mUnlockItem = unlock;
		mShowUnlockItem = (!isCameraBucketEmpty(Images.Media.EXTERNAL_CONTENT_URI) || !isCameraBucketEmpty(Video.Media.EXTERNAL_CONTENT_URI));
	}
	...
	public ArrayList<MediaItem> getMediaItem(int start, int count) {
		...
		ArrayList<Path> subset = new ArrayList<Path>(mExistingItems.subList(
				start, end));
	...
	private boolean isCameraBucketEmpty(Uri baseUri) {
		Uri uri = baseUri.buildUpon().appendQueryParameter("limit", "1")
				.build();
