...
import com.android.fastergallery.common.LruCache;
import com.android.fastergallery.common.Utils;
import com.android.fastergallery.data.DownloadEntry.Columns;
...
public class DownloadCache {
	...
	private static final String TABLE_NAME = DownloadEntry.SCHEMA
			.getTableName();
	...
	private static final String FREESPACE_PROJECTION[] = { Columns.ID,
			Columns.DATA, Columns.CONTENT_URL, Columns.CONTENT_SIZE };
	...
	private static final String SUM_PROJECTION[] = { String.format("sum(%s)",
			Columns.CONTENT_SIZE) };
	private static final int SUM_INDEX_SUM = 0;

	private final LruCache<String, Entry> mEntryMap = new LruCache<String, Entry>(
			LRU_CAPACITY);
	...
	private void updateLastAccess(long id) {
		...
		mDatabase.update(TABLE_NAME, values, ID_WHERE,
	...
	private synchronized void freeSomeSpaceIfNeed(int maxDeleteFileCount) {
		if (mTotalBytes <= mCapacity)
			return;
		Cursor cursor = mDatabase.query(TABLE_NAME, FREESPACE_PROJECTION, null,
				null, null, null, FREESPACE_ORDER_BY);
		try {
			while (maxDeleteFileCount > 0 && mTotalBytes > mCapacity
				...
				if (!containsKey) {
					...
					mDatabase.delete(TABLE_NAME, ID_WHERE,
							new String[] { String.valueOf(id) });
				} else {
	...
	private synchronized void initialize() {
		...
		Cursor cursor = mDatabase.query(TABLE_NAME, SUM_PROJECTION, null, null,
				null, null, null);
		mTotalBytes = 0;
		try {
			if (cursor.moveToNext()) {
	...
	private class DownloadTask implements Job<File>, FutureListener<File> {
		...
		public File run(JobContext jc) {
			...
			try {
				...
				jc.setMode(ThreadPool.MODE_NETWORK);
				boolean downloaded = DownloadUtils.requestDownload(jc, url,
						tempFile);
