...
import com.android.fastergallery.util.BucketNames;
...
public class LocalAlbum extends MediaSet {
	...
	public LocalAlbum(Path path, GalleryApp application, int bucketId,
			boolean isImage) {
		this(path, application, bucketId, isImage, BucketHelper.getBucketName(
	...
	public ArrayList<MediaItem> getMediaItem(int start, int count) {
		...
		Cursor cursor = mResolver.query(uri, mProjection, mWhereClause,
				new String[] { String.valueOf(mBucketId) }, mOrderClause);
		if (cursor == null) {
	...
	public static MediaItem[] getMediaItemById(GalleryApp application,
		...
		Cursor cursor = resolver.query(baseUri, projection,
				"_id BETWEEN ? AND ?", new String[] { String.valueOf(idLow),
						String.valueOf(idHigh) }, "_id");
		if (cursor == null) {
		...
		try {
			...
			while (i < n && cursor.moveToNext()) {
				...
				MediaItem item = loadOrUpdateItem(childPath, cursor,
						dataManager, application, isImage);
				result[i] = item;
				++i;
			}
			return result;
	...
	public int getMediaItemCount() {
		if (mCachedCount == INVALID_COUNT) {
			Cursor cursor = mResolver.query(mBaseUri, COUNT_PROJECTION,
					mWhereClause, new String[] { String.valueOf(mBucketId) },
					null);
			if (cursor == null) {
	...
	public static String getRelativePath(int bucketId) {
		...
		if (bucketId == MediaSetUtils.CAMERA_BUCKET_ID) {
			...
			if (path == null) {
				Log.w(TAG, "Relative path for bucket id: " + bucketId
						+ " is not found.");
				relativePath = null;
			} else {
				relativePath = path.substring(extStorage.getAbsolutePath()
						.length());
			}
		}
		return relativePath;
