...
public abstract class MediaSet extends MediaObject {
	...
	public ArrayList<MediaItem> getCoverMediaItems(int start, int count) {
		ArrayList<MediaItem> items = getMediaItem(start, count);
		if (items.size() > 0)
			return items;
		for (int i = 0, n = getSubMediaSetCount(); i < n; i++) {
			ArrayList<MediaItem> cover = getSubMediaSet(i).getCoverMediaItems(0, 4);
			if (cover.size() > 0)
				return cover;
		}
		return null;
	...
	public int getIndexOfItem(Path path, int hint) {
		...
		ArrayList<MediaItem> list = getMediaItem(start,
	...
	protected int enumerateTotalMediaItems(ItemConsumer consumer, int startIndex) {
		...
		for (int i = 0; i < m; i++) {
			start += getSubMediaSet(i).enumerateTotalMediaItems(consumer,
					startIndex + start);
		}
		return start;
	}
	...
	protected Future<Integer> requestSyncOnMultipleSets(MediaSet[] sets,
	...
	private class MultiSetSyncFuture implements Future<Integer>, SyncListener {
		...
		MultiSetSyncFuture(MediaSet[] sets, SyncListener listener) {
			...
			synchronized (this) {
				for (int i = 0, n = sets.length; i < n; ++i) {
					...
					Log.d(TAG,
							"  request sync: "
									+ Utils.maskDebugInfo(sets[i].getName()));
				}
			}
		}
		...
		public void onSyncDone(MediaSet mediaSet, int resultCode) {
			...
			synchronized (this) {
				...
				Log.d(TAG,
						"onSyncDone: "
								+ Utils.maskDebugInfo(mediaSet.getName())
