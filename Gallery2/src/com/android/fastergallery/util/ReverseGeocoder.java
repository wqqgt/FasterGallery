...
import com.android.fastergallery.common.BlobCache;
...
public class ReverseGeocoder {
	...
	public ReverseGeocoder(Context context) {
		...
		mGeoCache = CacheManager.getCache(context, GEO_CACHE_FILE,
				GEO_CACHE_MAX_ENTRIES, GEO_CACHE_MAX_BYTES, GEO_CACHE_VERSION);
	...
	public String computeAddress(SetLatLong set) {
		...
		if (Math.abs(set.mMaxLatLatitude - set.mMinLatLatitude) < Math
				.abs(set.mMaxLonLongitude - set.mMinLonLongitude)) {
		...
		for (int i = 0; i < providers.size(); ++i) {
			...
			location = (provider != null) ? locationManager
		...
		if (location != null) {
			...
			if (currentAddress != null
					&& currentAddress.getCountryCode() != null) {
		...
		if (currentCity.equals(addr1Locality)
				|| currentCity.equals(addr2Locality)) {
			...
			closestCommonLocation = valueIfEqual(addr1.getAddressLine(0),
					addr2.getAddressLine(0));
			if (closestCommonLocation != null
					&& !("null".equals(closestCommonLocation))) {
			...
			closestCommonLocation = valueIfEqual(addr1.getThoroughfare(),
		...
		if (currentAdminArea.equals(addr1AdminArea)
				&& currentAdminArea.equals(addr2AdminArea)) {
			...
			if (!"".equals(addr1Locality)) {
				if (addr1Locality.equals(addr2Locality)) {
					closestCommonLocation = addr1Locality + ", "
							+ currentAdminArea;
				} else {
					closestCommonLocation = addr1Locality + " - "
							+ addr2Locality;
		...
		if (addr1Country == null || addr2Country == null)
			return null;
		if (addr1Country.length() > MAX_COUNTRY_NAME_LENGTH
				|| addr2Country.length() > MAX_COUNTRY_NAME_LENGTH) {
	...
	private String getLocalityAdminForAddress(final Address addr,
			final boolean approxLocation) {
		if (addr == null)
			return "";
