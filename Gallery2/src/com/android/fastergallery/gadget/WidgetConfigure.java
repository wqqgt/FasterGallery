...
import com.android.fastergallery.app.AlbumPicker;
import com.android.fastergallery.app.DialogPicker;
...
public class WidgetConfigure extends Activity {
	...
	protected void onCreate(Bundle savedState) {
		...
		if (savedState == null) {
			if (ApiHelper.HAS_REMOTE_VIEWS_SERVICE) {
				...
				setWidgetType(new Intent().putExtra(KEY_WIDGET_TYPE,
						R.id.widget_type_photo));
			}
		} else {
	...
	private void updateWidgetAndFinish(WidgetDatabaseHelper.Entry entry) {
		...
		RemoteViews views = PhotoAppWidgetProvider.buildWidget(this,
				mAppWidgetId, entry);
...
//		startActivityForResult(request, REQUEST_CROP_IMAGE);
	...
	private void setChoosenAlbum(Intent data) {
		...
		try {
			...
			helper.setWidget(mAppWidgetId, WidgetDatabaseHelper.TYPE_ALBUM,
	...
	private void setWidgetType(Intent data) {
		int widgetType = data.getIntExtra(KEY_WIDGET_TYPE,
				R.id.widget_type_shuffle);
		if (widgetType == R.id.widget_type_album) {
			...
			try {
				helper.setWidget(mAppWidgetId,
						WidgetDatabaseHelper.TYPE_SHUFFLE, null, null);
				updateWidgetAndFinish(helper.getEntry(mAppWidgetId));
			} finally {
				helper.close();
			}
			...
			Intent request = new Intent(this, DialogPicker.class).setAction(
