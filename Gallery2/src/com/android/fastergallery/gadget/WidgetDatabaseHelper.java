...
package com.android.fastergallery.gadget;
...
public class WidgetDatabaseHelper extends SQLiteOpenHelper {
	...
	private static final String[] PROJECTION = { FIELD_WIDGET_TYPE,
			FIELD_IMAGE_URI, FIELD_PHOTO_BLOB, FIELD_ALBUM_PATH,
	...
	public void onCreate(SQLiteDatabase db) {
		db.execSQL("CREATE TABLE " + TABLE_WIDGETS + " (" + FIELD_APPWIDGET_ID
				+ " INTEGER PRIMARY KEY, " + FIELD_WIDGET_TYPE
				+ " INTEGER DEFAULT 0, " + FIELD_IMAGE_URI + " TEXT, "
				+ FIELD_ALBUM_PATH + " TEXT, " + FIELD_PHOTO_BLOB + " BLOB, "
				+ FIELD_RELATIVE_PATH + " TEXT)");
	}

	private void saveData(SQLiteDatabase db, int oldVersion,
			ArrayList<Entry> data) {
		if (oldVersion <= 2) {
			Cursor cursor = db.query("photos", new String[] {
					FIELD_APPWIDGET_ID, FIELD_PHOTO_BLOB }, null, null, null,
					null, null);
			if (cursor == null)
				return;
			try {
			...
			Cursor cursor = db.query("photos", new String[] {
					FIELD_APPWIDGET_ID, FIELD_PHOTO_BLOB, FIELD_IMAGE_URI },
	...
	public Entry getEntry(int appWidgetId) {
		...
		try {
			...
			cursor = db.query(TABLE_WIDGETS, PROJECTION, WHERE_APPWIDGET_ID,
					new String[] { String.valueOf(appWidgetId) }, null, null,
					null);
	...
	public List<Entry> getEntries(int type) {
		...
		try {
			SQLiteDatabase db = getReadableDatabase();
			cursor = db.query(TABLE_WIDGETS, PROJECTION, WHERE_WIDGET_TYPE,
