...
package com.android.fastergallery.provider;
...
import com.android.fastergallery.common.AsyncTaskUtil;
...
public class GalleryProvider extends ContentProvider {
	...
	private static final String[] SUPPORTED_PICASA_COLUMNS = {
			PicasaColumns.USER_ACCOUNT, PicasaColumns.PICASA_ID,
	...
	public static Uri getUriFor(Context context, Path path) {
		if (sBaseUri == null) {
			sBaseUri = Uri.parse("content://" + context.getPackageName()
					+ ".provider");
		}
		return sBaseUri.buildUpon()
				.appendEncodedPath(path.toString().substring(1)) // ignore the
																	// leading
																	// '/'
				.build();
	}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		try {
			...
			if (PicasaSource.isPicasaImage(object)) {
				return queryPicasaItem(object, projection, selection,
	...
	private Cursor queryPicasaItem(MediaObject image, String[] projection,
		...
		boolean isValidLatlong = GalleryUtils.isValidLocation(latitude,
				longitude);

		for (int i = 0, n = projection.length; i < n; ++i) {
			String column = projection[i];
			if (PicasaColumns.USER_ACCOUNT.equals(column)) {
				columnValues[i] = PicasaSource.getUserAccount(getContext(),
						image);
	...
	// Modified from ContentProvider.openPipeHelper. We are target at API LEVEL
	// 10.
	// But openPipeHelper is available in API LEVEL 11.
	private static <T> ParcelFileDescriptor openPipeHelper(final T args,
			final PipeDataWriter<T> func) throws FileNotFoundException {
		try {
			final ParcelFileDescriptor[] pipe = ParcelFileDescriptor
					.createPipe();
