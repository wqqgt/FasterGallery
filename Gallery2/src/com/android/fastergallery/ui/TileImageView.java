...
import com.android.fastergallery.app.GalleryContext;
...
public class TileImageView extends GLView {
	...
	private void getRange(Rect out, int cX, int cY, int level, float scale,
		...
		int width = (int) Math.ceil(Math.max(Math.abs(cos * w - sin * h),
				Math.abs(cos * w + sin * h)));
		int height = (int) Math.ceil(Math.max(Math.abs(sin * w + cos * h),
				Math.abs(sin * w - cos * h)));
	...
	public boolean setPosition(int centerX, int centerY, float scale,
			int rotation) {
	...
	boolean decodeTile(Tile tile) {
		...
		synchronized (this) {
			...
			tile.mTileState = decodeComplete ? STATE_DECODED
					: STATE_DECODE_FAIL;
	...
	public void drawTile(GLCanvas canvas, int tx, int ty, int level, float x,
	...
	static boolean drawTile(Tile tile, GLCanvas canvas, RectF source,
			RectF target) {
		while (true) {
			if (tile.isContentValid()) {
	...
	private class Tile extends UploadedTexture {
		...
		boolean decode() {
			...
			try {
				mDecodedTile = DecodeUtils.ensureGLCompatibleBitmap(mModel
						.getTile(mTileLevel, mX, mY, sTileSize));
			} catch (Throwable t) {
		...
		protected Bitmap onGetBitmap() {
			...
			setSize(Math.min(sTileSize, rightEdge),
					Math.min(sTileSize, bottomEdge));
		...
		public String toString() {
			return String.format("tile(%s, %s, %s / %s)", mX / sTileSize, mY
					/ sTileSize, mLevel, mLevelCount);
		}
	}
