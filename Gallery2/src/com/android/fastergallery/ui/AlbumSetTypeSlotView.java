...
import com.android.fastergallery.anim.Animation;
...
public class AlbumSetTypeSlotView extends GLView {
	...
	public AlbumSetTypeSlotView(AbstractGalleryActivity activity, Spec spec) {
		mGestureDetector = new GestureDetector(activity,
				new MyGestureListener());
		mScroller = new ScrollerHelper(activity);
		mHandler = new SynchronizedHandler(activity.getGLRoot());
		mViewType = AlbumSetTypeManager.get().getCurrentType();
		setSlotSpec(spec);
	}
	...
	public void setSlotRenderer(SlotRenderer slotDrawer) {
		...
		if (mRenderer != null) {
			mRenderer
					.onSlotSizeChanged(mLayout.mSlotWidth, mLayout.mSlotHeight);
	...
	private int renderItem(GLCanvas canvas, int index, int pass,
		...
		int result = mRenderer.renderSlot(canvas, index, pass, rect.right
	...
	public class Layout {
		...
		private int calculateWidth(int type) {
			int width = 0;
			switch(type) {
...
//			absoluteY -= mVerticalPadding.get();
			...
			int index = (rowIdx * mUnitCount + columnIdx);
		...
		public boolean advanceAnimation(long animTime) {
			...
			return mVerticalPadding.calculate(animTime)
					| mHorizontalPadding.calculate(animTime);
		}
	}
