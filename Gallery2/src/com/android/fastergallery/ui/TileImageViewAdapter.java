...
public class TileImageViewAdapter implements TileImageView.TileSource {
	...
	public synchronized void setScreenNail(ScreenNail screenNail, int width,
			int height) {
	...
	private int calculateLevelCount() {
		return Math.max(0,
				Utils.ceilLog2((float) mImageWidth / mScreenNail.getWidth()));
	}
	...
	private Bitmap getTileWithoutReusingBitmap(int level, int x, int y,
			int tileSize) {
		int t = tileSize << level;
		Rect wantRegion = new Rect(x, y, x + t, y + t);
		...
		if (wantRegion.equals(overlapRegion))
			return bitmap;

		Bitmap result = Bitmap.createBitmap(tileSize, tileSize,
				Config.ARGB_8888);
