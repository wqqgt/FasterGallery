...
class Paper {
	...
	public float[] getTransform(Rect rect, float scrollX) {
		...
		float degrees = (1 / (1 + (float) Math.exp(-t * ROTATE_FACTOR)) - 0.5f)
				* 2 * -45;
		Matrix.setIdentityM(mMatrix, 0);
		Matrix.translateM(mMatrix, 0, mMatrix, 0, rect.centerX(),
				rect.centerY(), 0);
		Matrix.rotateM(mMatrix, 0, degrees, 0, 1, 0);
		Matrix.translateM(mMatrix, 0, mMatrix, 0, -rect.width() / 2,
				-rect.height() / 2, 0);
		return mMatrix;
	}
}
...
class EdgeAnimation {
	...
	public void onAbsorb(float velocity) {
		float finish = Utils.clamp(mValue + velocity * VELOCITY_FACTOR, -1.0f,
				1.0f);
	...
	public boolean update() {
		...
		float t = Utils.clamp((float) (now() - mStartTime) / mDuration, 0.0f,
				1.0f);
		/* Use linear interpolation for absorb, quadratic for others */
		float interp = (mState == STATE_ABSORB) ? t : mInterpolator
				.getInterpolation(t);
