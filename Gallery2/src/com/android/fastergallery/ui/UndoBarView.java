...
import com.android.fastergallery.glrenderer.NinePatchTexture;
...
public class UndoBarView extends GLView {
	...
	public UndoBarView(Context context) {
		...
		mUndoIcon = new ResourceTexture(context,
				R.drawable.ic_menu_revert_holo_dark);
	...
	protected void render(GLCanvas canvas) {
		...
		canvas.fillRect(x, y, mSeparatorWidth, mBarHeight - mSeparatorTopMargin
				- mSeparatorBottomMargin, GRAY);
	...
	public void animateVisibility(int visibility) {
		...
		if (mAnimationStartTime == NO_ANIMATION && mAlpha == target)
			return;
		if (mAnimationStartTime != NO_ANIMATION && mToAlpha == target)
			return;
