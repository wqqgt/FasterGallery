...
import com.android.fastergallery.glrenderer.ColorTexture;
import com.android.fastergallery.glrenderer.FadeInTexture;
...
import com.android.fastergallery.glrenderer.UploadedTexture;
...
public class AlbumSetTypeSlotRenderer extends AbstractSlotRenderer {
	...
	protected AlbumSetTypeSlidingWindow mDataWindow;
	...
	public AlbumSetTypeSlotRenderer(AbstractGalleryActivity activity,
			SelectionManager selectionManager, AlbumSetTypeSlotView slotView,
			LabelSpec labelSpec, int placeholderColor) {
		super(activity);
		mActivity = activity;
	...
	public void setModel(AlbumSetDataLoader model) {
		...
		if (model != null) {
			mDataWindow = new AlbumSetTypeSlidingWindow(mActivity, model,
					mLabelSpec, CACHE_SIZE);
	...
	private static Texture checkContentTexture(Texture texture) {
		return ((texture instanceof TiledTexture) && !((TiledTexture) texture)
				.isReady()) ? null : texture;
	}
	...
	protected int renderOverlay(GLCanvas canvas, int index,
			AlbumSetEntry entry, int width, int height) {
		int renderRequestFlags = 0;
		...
		if (mPressedIndex == index) {
			if (mAnimatePressedUp) {
				...
				&& (mHighlightItemPath == entry.setPath)) {
	...
	protected int renderContent(GLCanvas canvas, AlbumSetEntry entry,
	...
	protected int renderLabel(GLCanvas canvas, AlbumSetEntry entry, int width,
			int height) {
		...
		case FilterUtils.CLUSTER_BY_LIST:
		    ...
		    content.draw(canvas, height+b, b, width - b - height, height);
			break;
		default:
			content.draw(canvas, -b, height - h + b, width + b + b, h);
			break;
	...
	private class MyCacheListener implements AlbumSetTypeSlidingWindow.Listener {
