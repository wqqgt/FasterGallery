...
import com.android.fastergallery.anim.FloatAnimation;
...
public class SlideshowView extends GLView {
	...
	private final FloatAnimation mTransitionAnimation = new FloatAnimation(0,
			1, TRANSITION_DURATION);
	...
	private class SlideshowAnimation extends CanvasAnimation {
		...
		public SlideshowAnimation(int width, int height, Random random) {
			mWidth = width;
			mHeight = height;
			mMovingVector = new PointF(MOVE_SPEED * mWidth
					* (random.nextFloat() - 0.5f), MOVE_SPEED * mHeight
		...
		public void apply(GLCanvas canvas) {
			...
			float initScale = Math.min((float) viewWidth / mWidth,
