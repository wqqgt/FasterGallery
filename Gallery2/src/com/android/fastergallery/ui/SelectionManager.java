...
public class SelectionManager {
	...
	private int getTotalCount() {
		...
		if (mTotal < 0) {
			mTotal = mIsAlbumSet ? mSourceMediaSet.getSubMediaSetCount()
	...
	private static boolean expandMediaSet(ArrayList<Path> items, MediaSet set,
			int maxSelection) {
	...
	public ArrayList<Path> getSelected(boolean expandSet, int maxSelection) {
		...
		if (mIsAlbumSet) {
			...
			if (mInverseSelection) {
				...
				int index = 0;
				while (index < total) {
					int count = Math.min(total - index,
							MediaSet.MEDIAITEM_BATCH_FETCH_COUNT);
					ArrayList<MediaItem> list = mSourceMediaSet.getMediaItem(
							index, count);
					for (MediaItem item : list) {
