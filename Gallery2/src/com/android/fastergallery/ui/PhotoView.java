...
import com.android.fastergallery.glrenderer.RawTexture;
...
import com.android.fastergallery.util.UsageStatistics;
...
public class PhotoView extends GLView {
	...
	public PhotoView(AbstractGalleryActivity activity) {
		...
		mNoThumbnailText = StringTexture.newInstance(
				mContext.getString(R.string.no_thumbnail), DEFAULT_TEXT_SIZE,
				Color.WHITE);
	...
	private int getPanoramaRotation() {
		...
		// The second is that it assumes landscape is a 90 rotation from
		// portrait,
		// however on landscape devices this is not true. Thus, if we are in
		// portrait
		// on a landscape device, we need to invert the output
		int orientation = mContext.getResources().getConfiguration().orientation;
		boolean invertPortrait = (orientation == Configuration.ORIENTATION_PORTRAIT && (mDisplayRotation == 90 || mDisplayRotation == 270));
	...
	class FullPicture implements Picture {
		...
		private void setTileViewPosition(float cx, float cy, int viewW,
				int viewH, float scale) {
	...
	private class ScreenNailPicture implements Picture {
		...
		public void draw(GLCanvas canvas, Rect r) {
			...
			int cx = wantsCardEffect ? (int) (interpolate(filmRatio, w / 2,
					r.centerX()) + 0.5f) : r.centerX();
			int cy = r.centerY();
			canvas.save(GLCanvas.SAVE_FLAG_MATRIX | GLCanvas.SAVE_FLAG_ALPHA);
	...
	private class MyGestureListener implements GestureRecognizer.Listener {
		...
		private boolean flingImages(float velocityX, float velocityY, float dY) {
			...
			if (fastEnough) {
				...
				int duration = mPositionController.flingFilmY(mTouchBoxIndex,
						vy);
				if (duration >= 0) {
		...
		public boolean onScale(float focusX, float focusY, float scale) {
			...
			if (mCanChangeMode && largeEnough) {
				if ((outOfRange < 0 && !mFilmMode)
					...
					if (mFilmMode) {
						UsageStatistics
								.setPendingTransitionCause(UsageStatistics.TRANSITION_PINCH_OUT);
					} else {
						UsageStatistics
								.setPendingTransitionCause(UsageStatistics.TRANSITION_PINCH_IN);
					}
		...
		public void onDown(float x, float y) {
			...
			if (mFilmMode) {
				...
				// handling very wide images which are otherwise very hard to
				// fling
				mTouchBoxIndex = mPositionController.hitTest(xi,
						getHeight() / 2);

				if (mTouchBoxIndex < mPrevBound || mTouchBoxIndex > mNextBound) {
					mTouchBoxIndex = Integer.MAX_VALUE;
				} else {
					mTouchBoxDeletable = mPictures.get(mTouchBoxIndex)
							.isDeletable();
				}
			} else {
				mTouchBoxIndex = Integer.MAX_VALUE;
		...
		public void onUp() {
			...
			if (mFilmMode && mScrolledAfterDown && !mFirstScrollX
				...
				if (Math.abs(r.centerY() - h * 0.5f) > 0.4f * h) {
					int duration = mPositionController.flingFilmY(
							mTouchBoxIndex, 0);
					if (duration >= 0) {
						mPositionController
	...
	private boolean switchWithCaptureAnimationLocked(int offset) {
		...
		Message m = mHandler.obtainMessage(MSG_CAPTURE_ANIMATION_DONE, offset,
				0);
		mHandler.sendMessageDelayed(m,
	...
	private float getScrollScale(float scrollProgress) {
		float interpolatedProgress = mScaleInterpolator.getInterpolation(Math
				.abs(scrollProgress));
