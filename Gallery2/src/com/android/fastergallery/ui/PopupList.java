...
public class PopupList {
	...
	private final PopupWindow.OnDismissListener mOnDismissListener = new PopupWindow.OnDismissListener() {
		...
		public void onDismiss() {
			...
			if (observer.isAlive()) {
				...
				// The new "removeOnGlobalLayoutListener" is introduced in API
				// level 16
	...
	private final OnGlobalLayoutListener mOnGLobalLayoutListener = new OnGlobalLayoutListener() {
		...
		public void onGlobalLayout() {
			...
			mPopupWindow.update(mAnchorView, mPopupOffsetX, mPopupOffsetY,
	...
	public void show() {
		if (mPopupWindow != null)
			return;
		mAnchorView.getViewTreeObserver().addOnGlobalLayoutListener(
	...
	private void updatePopupLayoutParams() {
		...
		int maxHeight = mPopupWindow.getMaxAvailableHeight(mAnchorView) - p.top
				- p.bottom;
		mContentList.measure(
				MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED),
				MeasureSpec.makeMeasureSpec(maxHeight, MeasureSpec.AT_MOST));
		mPopupWidth = content.getMeasuredWidth() + p.top + p.bottom;
		mPopupHeight = Math.min(maxHeight, content.getMeasuredHeight() + p.left
				+ p.right);
