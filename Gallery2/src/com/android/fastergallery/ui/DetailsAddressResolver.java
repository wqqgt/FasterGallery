...
import com.android.fastergallery.util.ReverseGeocoder;
...
public class DetailsAddressResolver {
	...
	public String resolveAddress(double[] latlng,
			AddressResolvingListener listener) {
		mListener = listener;
		mAddressLookupJob = mContext.getThreadPool().submit(
				new AddressLookupJob(latlng), new FutureListener<Address>() {
					@Override
		...
		return GalleryUtils.formatLatitudeLongitude("(%f,%f)", latlng[0],
				latlng[1]);
	}
	...
	private void updateLocation(Address address) {
		if (address != null) {
			...
			String text = String.format("%s : %s", DetailsHelper
					.getDetailsName(context, MediaDetails.INDEX_LOCATION),
					addressText);
