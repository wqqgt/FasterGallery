...
import com.android.fastergallery.ui.DetailsAddressResolver.AddressResolvingListener;
import com.android.fastergallery.ui.DetailsHelper.CloseListener;
import com.android.fastergallery.ui.DetailsHelper.DetailsSource;
import com.android.fastergallery.ui.DetailsHelper.DetailsViewContainer;
import com.android.fastergallery.ui.DetailsHelper.ResolutionResolvingListener;
...
public class DialogDetailsView implements DetailsViewContainer {
	...
	public DialogDetailsView(AbstractGalleryActivity activity,
	...
	private void setDetails(MediaDetails details) {
		...
		String title = String
				.format(mActivity.getAndroidContext().getString(
						R.string.details_title), mIndex + 1, mSource.size());
		ListView detailsList = (ListView) LayoutInflater.from(
				mActivity.getAndroidContext()).inflate(R.layout.details_list,
				null, false);
	...
	private class DetailsAdapter extends BaseAdapter implements
			AddressResolvingListener, ResolutionResolvingListener {
		...
		private void setDetails(Context context, MediaDetails details) {
			...
			for (Entry<Integer, Object> detail : details) {
				...
				case MediaDetails.INDEX_LOCATION: {
					...
					value = DetailsHelper.resolveAddress(mActivity, latlng,
							this);
					break;
				}
				case MediaDetails.INDEX_SIZE: {
					value = Formatter.formatFileSize(context,
							(Long) detail.getValue());
					break;
				}
				case MediaDetails.INDEX_WHITE_BALANCE: {
					value = "1".equals(detail.getValue()) ? context
							.getString(R.string.manual) : context
							.getString(R.string.auto);
					break;
				}
				case MediaDetails.INDEX_FLASH: {
					MediaDetails.FlashState flash = (MediaDetails.FlashState) detail
							.getValue();
					// TODO: camera doesn't fill in the complete values, show
					// more information
					// when it is fixed.
				...
				default: {
					...
					// This shouldn't happen, log its key to help us diagnose
					// the problem.
					if (valueObj == null) {
						Utils.fail(
								"%s's value is Null",
								DetailsHelper.getDetailsName(context,
										detail.getKey()));
					}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				tv = (TextView) LayoutInflater.from(
						mActivity.getAndroidContext()).inflate(
		...
		public void onResolutionAvailable(int width, int height) {
			...
			Context context = mActivity.getAndroidContext();
			String widthString = String.format("%s: %d", DetailsHelper
					.getDetailsName(context, MediaDetails.INDEX_WIDTH), width);
			String heightString = String
					.format("%s: %d", DetailsHelper.getDetailsName(context,
							MediaDetails.INDEX_HEIGHT), height);
