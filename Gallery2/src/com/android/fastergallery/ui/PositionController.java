...
import com.android.fastergallery.app.PhotoPage;
import com.android.fastergallery.common.Utils;
import com.android.fastergallery.ui.PhotoView.Size;
import com.android.fastergallery.util.GalleryUtils;
import com.android.fastergallery.util.RangeArray;
import com.android.fastergallery.util.RangeIntArray;
...
class PositionController {
	...
	private RangeArray<Gap> mTempGaps = new RangeArray<Gap>(-BOX_MAX,
			BOX_MAX - 1);
	...
	public void setConstrainedFrame(Rect cFrame) {
		if (mConstrainedFrame.equals(cFrame))
			return;
	...
	private boolean setBoxSize(int i, int width, int height, boolean isViewSize) {
		...
		float ratio = (width > height) ? (float) b.mImageW / width
	...
	private boolean startOpeningAnimationIfNeeded() {
		...
		b.mCurrentScale = Math.max(r.width() / (float) b.mImageW, r.height()
				/ (float) b.mImageH);
	...
	public boolean flingPage(int velocityX, int velocityY) {
		...
		return startAnimation(targetX, targetY, b.mCurrentScale,
				ANIM_KIND_FLING);
	}
	...
	public boolean flingFilmX(int velocityX) {
		...
		mFilmScroller.fling(p.mCurrentX, 0, velocityX, 0, Integer.MIN_VALUE,
				Integer.MAX_VALUE, 0, 0);
		int targetX = mFilmScroller.getFinalX();
		return startAnimation(targetX, b.mCurrentY, b.mCurrentScale,
				ANIM_KIND_FLING_X);
	}
	...
	public int flingFilmY(int boxIndex, int velocityY) {
		...
		int duration;
		if (velocityY != 0) {
			duration = (int) (Math.abs(targetY - b.mCurrentY) * 1000f / Math
					.abs(velocityY));
	...
	public boolean inOpeningAnimation() {
		return (mPlatform.mAnimationKind == ANIM_KIND_OPENING && mPlatform.mAnimationStartTime != NO_ANIMATION)
				|| (mBoxes.get(0).mAnimationKind == ANIM_KIND_OPENING && mBoxes
						.get(0).mAnimationStartTime != NO_ANIMATION);
	}
	...
	private float getMinimalScale(Box b) {
		...
		float s = Math.min(wFactor * viewW / b.mImageW, hFactor * viewH
				/ b.mImageH);
	...
	private float getTargetScale(Box b) {
		return b.mAnimationStartTime == NO_ANIMATION ? b.mCurrentScale
				: b.mToScale;
	}
	...
	private abstract static class Animatable {
		...
		public boolean advanceAnimation() {
			...
			if (mAnimationDuration == 0) {
				...
				progress = (float) (now - mAnimationStartTime)
						/ mAnimationDuration;
			}
	...
	private class Platform extends Animatable {
		...
		public boolean startSnapback() {
			if (mAnimationStartTime != NO_ANIMATION)
				return false;
			if (mAnimationKind == ANIM_KIND_SCROLL && mListener.isHoldingDown())
				return false;
			if (mInScale)
				return false;

			Box b = mBoxes.get(0);
			float scaleMin = mExtraScalingRange ? b.mScaleMin * SCALE_MIN_EXTRA
					: b.mScaleMin;
			float scaleMax = mExtraScalingRange ? b.mScaleMax * SCALE_MAX_EXTRA
					: b.mScaleMax;
		...
		public void updateDefaultXY() {
			...
			if (mConstrained && !mConstrainedFrame.isEmpty()) {
				...
				mDefaultY = mFilmMode ? 0 : mConstrainedFrame.centerY()
						- mViewH / 2;
			} else {
				mDefaultX = 0;
				mDefaultY = 0;
		...
		private boolean doAnimation(int targetX, int targetY, int kind) {
			if (mCurrentX == targetX && mCurrentY == targetY)
				return false;
			mAnimationKind = kind;
			mFromX = mCurrentX;
			mFromY = mCurrentY;
	...
	private class Box extends Animatable {
		...
		public boolean startSnapback() {
			...
			if (mAnimationKind == ANIM_KIND_DELETE
					&& mListener.isHoldingDelete())
				return false;
			if (mInScale && this == mBoxes.get(0))
				return false;
			...
			if (this == mBoxes.get(0)) {
				float scaleMin = mExtraScalingRange ? mScaleMin
						* SCALE_MIN_EXTRA : mScaleMin;
				float scaleMax = mExtraScalingRange ? mScaleMax
		...
		public float clampScale(float s) {
			return Utils.clamp(s, SCALE_MIN_EXTRA * mScaleMin, SCALE_MAX_EXTRA
					* mScaleMax);
		}
