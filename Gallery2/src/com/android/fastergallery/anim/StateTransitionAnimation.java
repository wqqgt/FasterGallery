...
package com.android.fastergallery.anim;
...
public class StateTransitionAnimation extends Animation {
	...
	protected void onCalculate(float progress) {
		...
		mCurrentContentAlpha = mTransitionSpec.contentAlphaFrom
				+ (mTransitionSpec.contentAlphaTo - mTransitionSpec.contentAlphaFrom)
				* progress;
		...
		mCurrentOverlayScale = mTransitionSpec.overlayScaleFrom
				+ (mTransitionSpec.overlayScaleTo - mTransitionSpec.overlayScaleFrom)
				* progress;
		mCurrentOverlayAlpha = mTransitionSpec.overlayAlphaFrom
				+ (mTransitionSpec.overlayAlphaTo - mTransitionSpec.overlayAlphaFrom)
				* progress;
	}

	private void applyOldTexture(GLView view, GLCanvas canvas, float alpha,
			float scale, boolean clear) {
		if (mOldScreenTexture == null)
			return;
		if (clear)
	...
	public void applyBackground(GLView view, GLCanvas canvas) {
		if (mCurrentBackgroundAlpha > 0f) {
			applyOldTexture(view, canvas, mCurrentBackgroundAlpha,
					mCurrentBackgroundScale, true);
		}
	}
	...
	public void applyOverlay(GLView view, GLCanvas canvas) {
		if (mCurrentOverlayAlpha > 0f) {
			applyOldTexture(view, canvas, mCurrentOverlayAlpha,
					mCurrentOverlayScale, false);
		}
	}
}
