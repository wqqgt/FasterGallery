...
import com.android.fastergallery.ui.GLRoot.OnGLIdleListener;
...
public class TiledTexture implements Texture {
	...
	public static class Uploader implements OnGLIdleListener {
		private final ArrayDeque<TiledTexture> mTextures = new ArrayDeque<TiledTexture>(
				INIT_CAPACITY);
	...
	public static void prepareResources() {
		sUploadBitmap = Bitmap.createBitmap(TILE_SIZE, TILE_SIZE,
				Config.ARGB_8888);
	...
	// Texture +---------------+
	// Canvas
	private static void mapRect(RectF output, RectF src, float x0, float y0,
			float x, float y, float scaleX, float scaleY) {
		output.set(x + (src.left - x0) * scaleX, y + (src.top - y0) * scaleY, x
				+ (src.right - x0) * scaleX, y + (src.bottom - y0) * scaleY);
	}

	// Draws a mixed color of this texture and a specified color onto the
	// a rectangle. The used color is: from * (1 - ratio) + to * ratio.
	public void drawMixed(GLCanvas canvas, int color, float ratio, int x,
