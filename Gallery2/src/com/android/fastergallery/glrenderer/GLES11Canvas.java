...
public class GLES11Canvas implements GLCanvas {
	...
	public GLES11Canvas(GL11 gl) {
		...
		FloatBuffer xyBuffer = allocateDirectNativeOrderBuffer(size)
				.asFloatBuffer();
		...
		gl.glBufferData(GL11.GL_ARRAY_BUFFER, xyBuffer.capacity()
				* (Float.SIZE / Byte.SIZE), xyBuffer, GL11.GL_STATIC_DRAW);
	...
	public void drawMesh(BasicTexture tex, int x, int y, int xyBuffer,
		...
		mGL.glDrawElements(GL11.GL_TRIANGLE_STRIP, indexCount,
	...
	private void drawBoundTexture(BasicTexture texture, int x, int y,
		...
		if (isMatrixRotatedOrFlipped(mMatrixValues)) {
			...
			float points[] = mapPoints(mMatrixValues, x, y + height, x + width,
					y);
			x = (int) (points[0] + 0.5f);
			y = (int) (points[1] + 0.5f);
	...
	private void drawTexture(BasicTexture texture, int x, int y, int width,
	...
	@Override
	public void drawMixed(BasicTexture from, int toColor, float ratio, int x,
			int y, int w, int h) {
	...
	private void setMixedColor(int toColor, float ratio, float alpha) {
		...
		setTextureColor(((toColor >>> 16) & 0xff) * colorScale,
				((toColor >>> 8) & 0xff) * colorScale, (toColor & 0xff)
						* colorScale, combo);
		GL11 gl = mGL;
		...
		gl.glTexEnvf(GL11.GL_TEXTURE_ENV, GL11.GL_OPERAND1_RGB,
				GL11.GL_SRC_COLOR);
		gl.glTexEnvf(GL11.GL_TEXTURE_ENV, GL11.GL_SRC1_ALPHA, GL11.GL_CONSTANT);
		gl.glTexEnvf(GL11.GL_TEXTURE_ENV, GL11.GL_OPERAND1_ALPHA,
				GL11.GL_SRC_ALPHA);
	...
	private void drawMixed(BasicTexture from, int toColor, float ratio, int x,
	...
	private static boolean isMatrixRotatedOrFlipped(float matrix[]) {
		...
		return Math.abs(matrix[MSKEW_X]) > eps
				|| Math.abs(matrix[MSKEW_Y]) > eps || matrix[MSCALE_X] < -eps
				|| matrix[MSCALE_Y] > eps;
	}
	...
	private void setTextureCoords(float left, float top, float right,
			float bottom) {
	...
	public void deleteRecycledResources() {
		synchronized (mUnboundTextures) {
			...
			if (ids.size() > 0) {
				mGLId.glDeleteTextures(mGL, ids.size(), ids.getInternalArray(),
						0);
				ids.clear();
			}
			...
			ids = mDeleteBuffers;
			if (ids.size() > 0) {
				mGLId.glDeleteBuffers(mGL, ids.size(), ids.getInternalArray(),
						0);
				ids.clear();
			}
		}
	...
	public void setTextureParameters(BasicTexture texture) {
		...
		int target = texture.getTarget();
		mGL.glBindTexture(target, texture.getId());
		mGL.glTexParameterfv(target, GL11Ext.GL_TEXTURE_CROP_RECT_OES,
				sCropRect, 0);
		mGL.glTexParameteri(target, GL11.GL_TEXTURE_WRAP_S,
				GL11.GL_CLAMP_TO_EDGE);
		mGL.glTexParameteri(target, GL11.GL_TEXTURE_WRAP_T,
				GL11.GL_CLAMP_TO_EDGE);
	...
	public void initializeTextureSize(BasicTexture texture, int format, int type) {
		...
		mGL.glTexImage2D(target, 0, format, width, height, 0, format, type,
				null);
	}
	...
	private int uploadBuffer(Buffer buf, int elementSize) {
		...
		mGL.glBufferData(GL11.GL_ARRAY_BUFFER, buf.capacity() * elementSize,
				buf, GL11.GL_STATIC_DRAW);
		return bufferId;
	}
