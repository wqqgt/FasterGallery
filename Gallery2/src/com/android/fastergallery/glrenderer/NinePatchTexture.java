...
public class NinePatchTexture extends ResourceTexture {
	...
	protected Bitmap onGetBitmap() {
		...
		mChunk = chunkData == null ? null : NinePatchChunk.deserialize(bitmap
				.getNinePatchChunk());
		if (mChunk == null) {
...
class NinePatchInstance {
	 ...
	 */
	private static int stretch(float x[], float u[], int div[], int source,
			int target) {
	...
	public void draw(GLCanvas canvas, NinePatchTexture tex, int x, int y) {
		...
		canvas.drawMesh(tex, x, y, mXyBufferName, mUvBufferName,
				mIndexBufferName, mIdxCount);
	}
