 ...
 */
package com.android.fastergallery.glrenderer;
...
import com.android.fastergallery.util.IntArray;
...
public class GLES20Canvas implements GLCanvas {
	...
	private static final int OFFSET_DRAW_LINE = OFFSET_FILL_RECT
			+ COUNT_FILL_VERTEX;
	private static final int OFFSET_DRAW_RECT = OFFSET_DRAW_LINE
			+ COUNT_LINE_VERTEX;
	...
	private static final String DRAW_VERTEX_SHADER = "" + "uniform mat4 "
			+ MATRIX_UNIFORM + ";\n" + "attribute vec2 " + POSITION_ATTRIBUTE
			+ ";\n" + "void main() {\n" + "  vec4 pos = vec4("
			+ POSITION_ATTRIBUTE + ", 0.0, 1.0);\n" + "  gl_Position = "
			+ MATRIX_UNIFORM + " * pos;\n" + "}\n";
	...
	private static final String TEXTURE_VERTEX_SHADER = "" + "uniform mat4 "
	...
	private static final String MESH_VERTEX_SHADER = "" + "uniform mat4 "
			+ MATRIX_UNIFORM + ";\n" + "attribute vec2 " + POSITION_ATTRIBUTE
	...
	private static final String TEXTURE_FRAGMENT_SHADER = ""
			...
			+ "  gl_FragColor = texture2D(" + TEXTURE_SAMPLER_UNIFORM
	...
	private float[] mMatrices = new float[INITIAL_RESTORE_STATE_SIZE
			* MATRIX_SIZE];
	...
	public GLES20Canvas() {
		...
		int drawVertexShader = loadShader(GLES20.GL_VERTEX_SHADER,
				DRAW_VERTEX_SHADER);
		int textureVertexShader = loadShader(GLES20.GL_VERTEX_SHADER,
				TEXTURE_VERTEX_SHADER);
		int meshVertexShader = loadShader(GLES20.GL_VERTEX_SHADER,
				MESH_VERTEX_SHADER);
		int drawFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER,
				DRAW_FRAGMENT_SHADER);
		int textureFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER,
				TEXTURE_FRAGMENT_SHADER);
		int oesTextureFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER,
				OES_TEXTURE_FRAGMENT_SHADER);

		mDrawProgram = assembleProgram(drawVertexShader, drawFragmentShader,
				mDrawParameters);
		mTextureProgram = assembleProgram(textureVertexShader,
				textureFragmentShader, mTextureParameters);
		mOesTextureProgram = assembleProgram(textureVertexShader,
				oesTextureFragmentShader, mOesTextureParameters);
		mMeshProgram = assembleProgram(meshVertexShader, textureFragmentShader,
				mMeshParameters);
	...
	public void save(int saveFlags) {
		...
		if (saveMatrix) {
			...
			System.arraycopy(mMatrices, currentIndex, mMatrices,
					mCurrentMatrixIndex, MATRIX_SIZE);
		}
	...
	public void drawLine(float x1, float y1, float x2, float y2, GLPaint paint) {
		draw(GLES20.GL_LINE_STRIP, OFFSET_DRAW_LINE, COUNT_LINE_VERTEX, x1, y1,
				x2 - x1, y2 - y1, paint);
		mCountDrawLine++;
	}
	...
	public void drawRect(float x, float y, float width, float height,
			GLPaint paint) {
		draw(GLES20.GL_LINE_LOOP, OFFSET_DRAW_RECT, COUNT_RECT_VERTEX, x, y,
				width, height, paint);
		mCountDrawLine++;
	}

	private void draw(int type, int offset, int count, float x, float y,
			float width, float height, GLPaint paint) {
		draw(type, offset, count, x, y, width, height, paint.getColor(),
				paint.getLineWidth());
	}

	private void draw(int type, int offset, int count, float x, float y,
			float width, float height, int color, float lineWidth) {
	...
	private void prepareDraw(int offset, int color, float lineWidth) {
		...
		if (blendingEnabled) {
			GLES20.glBlendColor(colorArray[0], colorArray[1], colorArray[2],
					colorArray[3]);
			checkError();
		}

		GLES20.glUniform4fv(mDrawParameters[INDEX_COLOR].handle, 1, colorArray,
				0);
	...
	private void setPosition(ShaderParameter[] params, int offset) {
		...
		checkError();
		GLES20.glVertexAttribPointer(params[INDEX_POSITION].handle,
				COORDS_PER_VERTEX, GLES20.GL_FLOAT, false, VERTEX_STRIDE,
	...
	private void draw(ShaderParameter[] params, int type, int count, float x,
	...
	private void setMatrix(ShaderParameter[] params, float x, float y,
			float width, float height) {
		Matrix.translateM(mTempMatrix, 0, mMatrices, mCurrentMatrixIndex, x, y,
				0f);
		Matrix.scaleM(mTempMatrix, 0, width, height, 1f);
		Matrix.multiplyMM(mTempMatrix, MATRIX_SIZE, mProjectionMatrix, 0,
				mTempMatrix, 0);
		GLES20.glUniformMatrix4fv(params[INDEX_MATRIX].handle, 1, false,
				mTempMatrix, MATRIX_SIZE);
		checkError();
	}

	@Override
	public void fillRect(float x, float y, float width, float height, int color) {
		draw(GLES20.GL_TRIANGLE_STRIP, OFFSET_FILL_RECT, COUNT_FILL_VERTEX, x,
				y, width, height, color, 0f);
		mCountFillRect++;
	}
	...
	private static void copyTextureCoordinates(BasicTexture texture,
			RectF outRect) {
		int left = 0;
		int top = 0;
	...
	@Override
	public void drawTexture(BasicTexture texture, float[] textureTransform,
	...
	private void drawTextureRect(BasicTexture texture, RectF source,
			RectF target) {
	...
	private void drawTextureRect(BasicTexture texture, float[] textureMatrix,
			RectF target) {
		ShaderParameter[] params = prepareTexture(texture);
		setPosition(params, OFFSET_FILL_RECT);
		GLES20.glUniformMatrix4fv(params[INDEX_TEXTURE_MATRIX].handle, 1,
		...
		draw(params, GLES20.GL_TRIANGLE_STRIP, COUNT_FILL_VERTEX, target.left,
				target.top, target.width(), target.height());
		if (texture.isFlippedVertically()) {
			restore();
		}
		mCountTextureRect++;
	...
	private void prepareTexture(BasicTexture texture, int program,
			ShaderParameter[] params) {
	...
	@Override
	public void drawMesh(BasicTexture texture, int x, int y, int xyBuffer,
	...
	@Override
	public void drawMixed(BasicTexture texture, int toColor, float ratio,
			int x, int y, int w, int h) {
		copyTextureCoordinates(texture, mTempSourceRect);
		mTempTargetRect.set(x, y, x + w, y + h);
	...
	public void drawMixed(BasicTexture texture, int toColor, float ratio,
		...
		setAlpha(colorAlpha);
		fillRect(target.left, target.top, target.width(), target.height(),
				toColor);
	...
	public void deleteRecycledResources() {
		synchronized (mUnboundTextures) {
			...
			if (mUnboundTextures.size() > 0) {
				mGLId.glDeleteTextures(null, ids.size(),
						ids.getInternalArray(), 0);
				ids.clear();
			}

			ids = mDeleteBuffers;
			if (ids.size() > 0) {
				mGLId.glDeleteBuffers(null, ids.size(), ids.getInternalArray(),
						0);
				ids.clear();
			}
		}
	...
	public void dumpStatisticsAndClear() {
		String line = String.format(
				"MESH:%d, TEX_RECT:%d, FILL_RECT:%d, LINE:%d", mCountDrawMesh,
	...
	public void endRenderTarget() {
		RawTexture oldTexture = mTargetTextures
				.remove(mTargetTextures.size() - 1);
	...
	private void setRenderTarget(BasicTexture oldTexture, RawTexture texture) {
		...
		if (texture == null) {
			...
			GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER,
					GLES20.GL_COLOR_ATTACHMENT0, texture.getTarget(),
					texture.getId(), 0);
			checkError();
	...
	public void initializeTextureSize(BasicTexture texture, int format, int type) {
		...
		GLES20.glTexImage2D(target, 0, format, width, height, 0, format, type,
				null);
	}
	...
	@Override
	public void texSubImage2D(BasicTexture texture, int xOffset, int yOffset,
	...
	private int uploadBuffer(Buffer buffer, int elementSize) {
		...
		GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, buffer.capacity()
				* elementSize, buffer, GLES20.GL_STATIC_DRAW);
		checkError();
		return bufferId;
	}
	...
	public void getBounds(Rect bounds, int x, int y, int width, int height) {
		Matrix.translateM(mTempMatrix, 0, mMatrices, mCurrentMatrixIndex, x, y,
				0f);
		Matrix.scaleM(mTempMatrix, 0, width, height, 1f);
		Matrix.multiplyMV(mTempMatrix, MATRIX_SIZE, mTempMatrix, 0,
				BOUNDS_COORDINATES, 0);
		Matrix.multiplyMV(mTempMatrix, MATRIX_SIZE + 4, mTempMatrix, 0,
				BOUNDS_COORDINATES, 4);
