...
import com.android.fastergallery.ui.SlideshowView;
...
public class SlideshowPage extends ActivityState {
	...
	private void showPendingBitmap() {
		...
		setStateResult(
				Activity.RESULT_OK,
				mResultIntent.putExtra(KEY_ITEM_PATH,
						slide.item.getPath().toString()).putExtra(
						KEY_PHOTO_INDEX, slide.index));
	...
	private void initializeData(Bundle data) {
		...
		if (random) {
			...
			mModel = new SlideshowDataAdapter(mActivity, new ShuffleSource(
					mediaSet, repeat), 0, null);
			...
			boolean repeat = data.getBoolean(KEY_REPEAT);
			mModel = new SlideshowDataAdapter(mActivity, new SequentialSource(
					mediaSet, repeat), index, path);
			setStateResult(Activity.RESULT_OK,
	...
	private static class ShuffleSource implements
	...
	private static class SequentialSource implements
			SlideshowDataAdapter.SlideshowSource {
		...
		public MediaItem getMediaItem(int index) {
			...
			return (index < mDataStart || index >= dataEnd) ? null : mData
					.get(index - mDataStart);
		}
