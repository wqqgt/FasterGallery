...
import com.android.fastergallery.anim.StateTransitionAnimation;
import com.android.fastergallery.glrenderer.RawTexture;
import com.android.fastergallery.ui.GLView;
import com.android.fastergallery.ui.PreparePageFadeoutTexture;
...
abstract public class ActivityState {
	...
	protected void onCreate(Bundle data, Bundle storedState) {
		mBackgroundColor = GalleryUtils.intColorToFloatARGBArray(mActivity
				.getResources().getColor(getBackgroundColorId()));
	}
	...
	BroadcastReceiver mPowerIntentReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_BATTERY_CHANGED.equals(action)) {
				boolean plugged = (0 != intent.getIntExtra(
	...
	protected void transitionOnNextPause(
			Class<? extends ActivityState> outgoing,
		...
		} else if (outgoing == AlbumPage.class
				&& incoming == SinglePhotoPage.class) {
	...
	protected void onPause() {
		...
		if (mNextTransition != StateTransitionAnimation.Transition.None) {
			mActivity.getTransitionStore().put(KEY_TRANSITION_IN,
					mNextTransition);
			PreparePageFadeoutTexture.prepareFadeOutTexture(mActivity,
					mContentPane);
	...
	void resume() {
		...
		if (actionBar != null) {
			...
			mActivity.getGalleryActionBar().setDisplayOptions(stateCount > 1,
					true);
	...
	protected void onResume() {
		...
		if (mNextTransition != StateTransitionAnimation.Transition.None) {
			mIntroAnimation = new StateTransitionAnimation(mNextTransition,
					fade);
			mNextTransition = StateTransitionAnimation.Transition.None;
		}
	}
