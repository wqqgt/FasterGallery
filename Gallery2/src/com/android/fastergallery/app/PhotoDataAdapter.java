...
import com.android.fastergallery.common.BitmapUtils;
import com.android.fastergallery.common.Utils;
import com.android.fastergallery.data.ContentListener;
import com.android.fastergallery.data.LocalMediaItem;
...
import com.android.fastergallery.ui.PhotoView;
import com.android.fastergallery.ui.ScreenNail;
import com.android.fastergallery.ui.SynchronizedHandler;
import com.android.fastergallery.ui.TileImageViewAdapter;
...
import com.android.fastergallery.util.MediaSetUtils;
...
public class PhotoDataAdapter implements PhotoPage.Model {
	...
	private void fireDataChange() {
		...
		mPhotoView.notifyDataChange(fromIndex, -mCurrentIndex, mSize - 1
				- mCurrentIndex);
	}
	...
	private void updateTileProvider(ImageEntry entry) {
		...
		if (screenNail != null) {
			if (fullImage != null) {
				mTileProvider.setScreenNail(screenNail, fullImage.getWidth(),
						fullImage.getHeight());
	...
	private void updateSlidingWindow() {
		...
		int start = Utils.clamp(mCurrentIndex - IMAGE_CACHE_SIZE / 2, 0,
				Math.max(0, mSize - IMAGE_CACHE_SIZE));
		...
		start = Utils.clamp(mCurrentIndex - DATA_CACHE_SIZE / 2, 0,
				Math.max(0, mSize - DATA_CACHE_SIZE));
	...
	private class ScreenNailJob implements Job<ScreenNail> {
		...
		public ScreenNail run(JobContext jc) {
			...
			Bitmap bitmap = mItem.requestImage(MediaItem.TYPE_THUMBNAIL)
					.run(jc);
			if (jc.isCancelled())
				return null;
			if (bitmap != null) {
				bitmap = BitmapUtils.rotateBitmap(bitmap, mItem.getRotation()
						- mItem.getFullImageRotation(), true);
			}
	...
	private boolean isTemporaryItem(MediaItem mediaItem) {
		...
		if (item.getDateInMs() - System.currentTimeMillis() > 10000)
			return false;
		return true;
	}
	...
	private Future<?> startTaskIfNeeded(int index, int which) {
		...
		if (which == BIT_SCREEN_NAIL && entry.requestedScreenNail != version) {
			entry.requestedScreenNail = version;
			entry.screenNailTask = mThreadPool.submit(new ScreenNailJob(item),
		...
		if (which == BIT_FULL_IMAGE
			...
			entry.requestedFullImage = version;
			entry.fullImageTask = mThreadPool.submit(new FullImageJob(item),
	...
	private class UpdateContent implements Callable<Void> {
		...
		public Void call() throws Exception {
			...
			if (info.items != null) {
				...
				int end = Math.min(info.contentStart + info.items.size(),
						mContentEnd);
