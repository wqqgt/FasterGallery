...
//					UsageStatistics.COMPONENT_GALLERY, klass.getSimpleName());
//		}
	...
	public void notifyActivityResult(int requestCode, int resultCode,
			Intent data) {
	...
	void finishState(ActivityState state, boolean fireOnPause) {
		...
		ActivityState top = !mStack.isEmpty() ? mStack.peek().activityState
				: null;
		...
		if (top != null) {
			UsageStatistics.onContentViewChanged(
					UsageStatistics.COMPONENT_GALLERY, top.getClass()
							.getSimpleName());
		}
	}
	...
	public void restoreFromState(Bundle inState) {
		...
		if (topState != null) {
			UsageStatistics.onContentViewChanged(
					UsageStatistics.COMPONENT_GALLERY, topState.getClass()
							.getSimpleName());
		}
	}
