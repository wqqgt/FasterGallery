...
import com.android.fastergallery.ui.ManageCacheDrawer;
...
public class ManageCachePage extends ActivityState implements
	...
	private GLView mRootPane = new GLView() {
		...
		protected void onLayout(boolean changed, int left, int top, int right,
			...
			// We assume the other components will complete before we get a
			// change
	...
	public void onSingleTapUp(int slotIndex) {
		...
		mCacheStorageInfo
				.increaseTargetCacheSize((isFullyCached ^ isSelected) ? -sizeOfTarget
						: sizeOfTarget);
	...
	public void onConfigurationChanged(Configuration config) {
		...
		FrameLayout layout = (FrameLayout) ((Activity) mActivity)
	...
	public void onResume() {
		...
		mEyePosition.resume();
		mUpdateStorageInfo = mActivity.getThreadPool().submit(
				mUpdateStorageInfoJob);
		FrameLayout layout = (FrameLayout) ((Activity) mActivity)
				.findViewById(R.id.footer);
		layout.addView(mFooterContent);
	...
	private void refreshCacheStorageInfo() {
		ProgressBar progressBar = (ProgressBar) mFooterContent
		...
		if (totalBytes == 0) {
			...
			progressBar
					.setProgress((int) (usedBytes * PROGRESS_BAR_MAX / totalBytes));
			progressBar.setSecondaryProgress((int) (expectedBytes
