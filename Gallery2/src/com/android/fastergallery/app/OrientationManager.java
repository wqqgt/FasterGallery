...
import com.android.fastergallery.ui.OrientationSource;
...
public class OrientationManager implements OrientationSource {
	...
	private int calculateCurrentScreenOrientation() {
		...
		// Display rotation >= 180 means we need to use the REVERSE
		// landscape/portrait
		boolean standard = displayRotation < 180;
		if (mActivity.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
			return standard ? ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
			...
			if (displayRotation == 90 || displayRotation == 270) {
				...
				// to flip which portrait we pick as display rotation is counter
				// clockwise
				standard = !standard;
			}
			return standard ? ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
