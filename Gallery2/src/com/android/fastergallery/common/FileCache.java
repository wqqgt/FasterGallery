...
import com.android.fastergallery.common.Entry.Table;
...
public class FileCache implements Closeable {
	...
	private static final String QUERY_WHERE = FileEntry.Columns.HASH_CODE
			+ "=? AND " + FileEntry.Columns.CONTENT_URL + "=?";
	private static final String ID_WHERE = FileEntry.Columns.ID + "=?";
	private static final String[] PROJECTION_SIZE_SUM = { String.format(
			"sum(%s)", FileEntry.Columns.SIZE) };
	...
	private static final String FREESPACE_ORDER_BY = String.format("%s ASC",
			FileEntry.Columns.LAST_ACCESS);

	private final LruCache<String, CacheEntry> mEntryMap = new LruCache<String, CacheEntry>(
			LRU_CAPACITY);
	...
	public void store(String downloadUrl, File file) {
		...
		synchronized (this) {
			...
			FileEntry.SCHEMA.insertOrReplace(mDbHelper.getWritableDatabase(),
					entry);
	...
	public CacheEntry lookup(String downloadUrl) {
		...
		synchronized (this) {
			...
			if (file == null)
				return null;
			entry = new CacheEntry(file.id, downloadUrl, new File(mRootDir,
					file.filename));
			if (!entry.cacheFile.isFile()) { // file has been removed
				try {
					mDbHelper.getWritableDatabase().delete(TABLE_NAME,
							ID_WHERE, new String[] { String.valueOf(file.id) });
	...
	private FileEntry queryDatabase(String downloadUrl) {
		...
		Cursor cursor = mDbHelper.getReadableDatabase().query(TABLE_NAME,
				FileEntry.SCHEMA.getProjection(), QUERY_WHERE, whereArgs, null,
				null, null);
		try {
	...
	private void updateLastAccess(long id) {
		...
		mDbHelper.getWritableDatabase().update(TABLE_NAME, values, ID_WHERE,
	...
	private synchronized void initialize() {
		...
		Cursor cursor = mDbHelper.getReadableDatabase().query(TABLE_NAME,
				PROJECTION_SIZE_SUM, null, null, null, null, null);
		try {
			if (cursor.moveToNext())
	...
	private void freeSomeSpaceIfNeed(int maxDeleteFileCount) {
		Cursor cursor = mDbHelper.getReadableDatabase().query(TABLE_NAME,
				FREESPACE_PROJECTION, null, null, null, null,
				FREESPACE_ORDER_BY);
		try {
			while (maxDeleteFileCount > 0 && mTotalBytes > mCapacity
					&& cursor.moveToNext()) {
	...
	private static class FileEntry extends Entry {
		...
		public String toString() {
			return new StringBuilder().append("hash_code: ").append(hashCode)
					.append(", ").append("content_url").append(contentUrl)
					.append(", ").append("last_access").append(lastAccess)
