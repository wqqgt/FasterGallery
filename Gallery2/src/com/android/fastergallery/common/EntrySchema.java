...
public final class EntrySchema {
	...
	private static final String SQLITE_TYPES[] = { "TEXT", "INTEGER",
			"INTEGER", "INTEGER", "INTEGER", "REAL", "REAL", "NONE" };
	...
	public <T extends Entry> T cursorToObject(Cursor cursor, T object) {
		try {
			for (ColumnInfo column : mColumnInfo) {
				...
				switch (column.type) {
				case TYPE_STRING:
					field.set(object, cursor.isNull(columnIndex) ? null
		...
		try {
			for (ColumnInfo column : mColumnInfo) {
				...
				case TYPE_DOUBLE:
					...
					break;
				case TYPE_BLOB:
					setIfNotNull(field, object,
							values.getAsByteArray(columnName));
					break;
				}
			}
			return object;
	...
	public boolean deleteWithId(SQLiteDatabase db, long id) {
		return db.delete(mTableName, "_id=?",
	...
	private void parseColumnInfo(Class<? extends Object> clazz,
		...
		for (int i = 0; i != fields.length; ++i) {
			...
			Entry.Column info = ((AnnotatedElement) field)
					.getAnnotation(Entry.Column.class);
			if (info == null)
				continue;
			...
			columns.add(new ColumnInfo(info.value(), type, info.indexed(), info
					.unique(), info.fullText(), info.defaultValue(), field,
					index));
		}
	}
	...
	public static final class ColumnInfo {
		...
		public ColumnInfo(String name, int type, boolean indexed,
				boolean unique, boolean fullText, String defaultValue,
				Field field, int projectionIndex) {
