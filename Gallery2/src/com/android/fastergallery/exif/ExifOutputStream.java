...
class ExifOutputStream extends FilterOutputStream {
	...
	private ArrayList<ExifTag> stripNullValueTags(ExifData data) {
		...
		for (ExifTag t : data.getAllTags()) {
			if (t.getValue() == null
					&& !ExifInterface.isOffsetTag(t.getTagId())) {
	...
	private void writeAllTags(OrderedDataOutputStream dataOutputStream)
		...
		IfdData interoperabilityIfd = mExifData
				.getIfdData(IfdId.TYPE_IFD_INTEROPERABILITY);
	...
	private void createRequiredIfdAndTag() throws IOException {
		...
		ExifTag exifOffsetTag = mInterface
				.buildUninitializedTag(ExifInterface.TAG_EXIF_IFD);
		...
		IfdData gpsIfd = mExifData.getIfdData(IfdId.TYPE_IFD_GPS);
		if (gpsIfd != null) {
			ExifTag gpsOffsetTag = mInterface
					.buildUninitializedTag(ExifInterface.TAG_GPS_IFD);
		...
		if (mExifData.hasCompressedThumbnail()) {
			...
			ExifTag offsetTag = mInterface
					.buildUninitializedTag(ExifInterface.TAG_STRIP_OFFSETS);
			if (offsetTag == null) {
				throw new IOException("No definition for crucial exif tag: "
	...
	static void writeTagValue(ExifTag tag,
			OrderedDataOutputStream dataOutputStream) throws IOException {
