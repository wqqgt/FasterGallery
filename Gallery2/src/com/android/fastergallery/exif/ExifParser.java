...
class ExifParser {
	...
	protected static final short EXIF_HEADER_TAIL = (short) 0x0000; // EXIF
																	// header in
																	// APP1
	...
	private static final short TAG_GPS_IFD = ExifInterface
			.getTrueTagKey(ExifInterface.TAG_GPS_IFD);
	...
	private ExifParser(InputStream inputStream, int options, ExifInterface iRef)
		...
		if (isIfdRequested(IfdId.TYPE_IFD_0)
	 ...
	 */
	protected static ExifParser parse(InputStream inputStream, int options,
			ExifInterface iRef) throws IOException, ExifInvalidFormatException {
	 ...
	 */
	protected static ExifParser parse(InputStream inputStream,
			ExifInterface iRef) throws IOException, ExifInvalidFormatException {
	...
	protected int next() throws IOException, ExifInvalidFormatException {
		...
		int endOfTags = mIfdStartOffset + OFFSET_SIZE + TAG_SIZE
				* mNumOfTagInIfd;
		if (offset < endOfTags) {
			mTag = readTag();
			if (mTag == null) {
	 ...
	 */
	protected void skipRemainingTagsInCurrentIfd() throws IOException,
	...
	private void skipTo(int offset) throws IOException {
		...
		while (!mCorrespondingEvent.isEmpty()
				&& mCorrespondingEvent.firstKey() < offset) {
	...
	protected void registerForTagValue(ExifTag tag) {
		if (tag.getOffset() >= mTiffStream.getReadByteCount()) {
			mCorrespondingEvent.put(tag.getOffset(),
	...
	private void registerIfd(int ifdType, long offset) {
		...
		mCorrespondingEvent.put((int) offset, new IfdEvent(ifdType,
	...
	private void registerUncompressedStrip(int stripIndex, long offset) {
		mCorrespondingEvent.put((int) offset, new ImageEvent(
				EVENT_UNCOMPRESSED_STRIP, stripIndex));
	}
	...
	private ExifTag readTag() throws IOException, ExifInvalidFormatException {
		...
		if (!ExifTag.isValidType(dataFormat)) {
			Log.w(TAG, String.format("Tag %04x: Invalid data type %d", tagId,
					dataFormat));
			mTiffStream.skip(4);
			return null;
		}
		...
		if (dataSize > 4) {
			...
			// Read the data here.
			if ((offset < mIfd0Position)
					&& (dataFormat == ExifTag.TYPE_UNDEFINED)) {
				byte[] buf = new byte[(int) numOfComp];
				System.arraycopy(mDataAboveIfd0, (int) offset
						- DEFAULT_IFD0_OFFSET, buf, 0, (int) numOfComp);
				tag.setValue(buf);
			} else {
	...
	private void checkOffsetOrImageTag(ExifTag tag) {
		...
		int ifd = tag.getIfd();
		if (tid == TAG_EXIF_IFD
				&& checkAllowed(ifd, ExifInterface.TAG_EXIF_IFD)) {
			if (isIfdRequested(IfdId.TYPE_IFD_EXIF)
					|| isIfdRequested(IfdId.TYPE_IFD_INTEROPERABILITY)) {
				registerIfd(IfdId.TYPE_IFD_EXIF, tag.getValueAt(0));
			}
		} else if (tid == TAG_GPS_IFD
				&& checkAllowed(ifd, ExifInterface.TAG_GPS_IFD)) {
			...
			if (isThumbnailRequested()) {
				...
				&& checkAllowed(ifd, ExifInterface.TAG_STRIP_OFFSETS)) {
			if (isThumbnailRequested()) {
				if (tag.hasValue()) {
					...
					mCorrespondingEvent.put(tag.getOffset(), new ExifTagEvent(
							tag, false));
				}
			}
	...
	protected void readFullTagValue(ExifTag tag) throws IOException {
		...
		if (type == ExifTag.TYPE_ASCII || type == ExifTag.TYPE_UNDEFINED
			...
			if (mCorrespondingEvent.size() > 0) {
				if (mCorrespondingEvent.firstEntry().getKey() < mTiffStream
						.getReadByteCount() + size) {
					...
					if (event instanceof ImageEvent) {
						...
						Log.w(TAG,
								"Thumbnail overlaps value for tag: \n"
										+ tag.toString());
						Entry<Integer, Object> entry = mCorrespondingEvent
								.pollFirstEntry();
						Log.w(TAG,
								"Invalid thumbnail offset: " + entry.getKey());
					} else {
						// Tag value overlaps another tag, shorten count
						if (event instanceof IfdEvent) {
							Log.w(TAG,
									"Ifd " + ((IfdEvent) event).ifd
	...
	private boolean seekTiffData(InputStream inputStream) throws IOException,
			ExifInvalidFormatException {
		CountedDataInputStream dataStream = new CountedDataInputStream(
				inputStream);
