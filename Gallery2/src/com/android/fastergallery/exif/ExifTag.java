...
public class ExifTag {
	...
	public boolean setValue(String value) {
		...
		byte[] finalBuf = buf;
		if (buf.length > 0) {
			finalBuf = (buf[buf.length - 1] == 0 || mDataType == TYPE_UNDEFINED) ? buf
	...
	protected Rational getRational(int index) {
		if ((mDataType != TYPE_RATIONAL)
				&& (mDataType != TYPE_UNSIGNED_RATIONAL)) {
			throw new IllegalArgumentException(
					"Cannot get RATIONAL value from "
							+ convertTypeToString(mDataType));
		}
	...
	protected void getBytes(byte[] buf, int offset, int length) {
		...
		System.arraycopy(mValue, 0, buf, offset,
				(length > mComponentCountActual) ? mComponentCountActual
						: length);
	}
	...
	private boolean checkBadComponentCount(int count) {
		if (mHasDefinedDefaultComponentCount
				&& (mComponentCountActual != count)) {
			return true;
		}
		return false;
		...
		if (obj instanceof ExifTag) {
			...
			if (mValue != null) {
				if (tag.mValue == null) {
					...
					return Arrays.equals((Rational[]) mValue,
							(Rational[]) tag.mValue);
				} else if (mValue instanceof byte[]) {
