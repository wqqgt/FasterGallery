...
import com.android.fastergallery.edit.GeometryMath;
...
public class CropObject {
	...
	public void setMinInnerSideSize(float minSide) {
		if (minSide <= 0) {
			throw new IllegalArgumentException(
					"Min dide must be greater than zero");
		}
	...
	public static boolean checkCorner(int selected) {
		return selected == TOP_LEFT || selected == TOP_RIGHT
				|| selected == BOTTOM_RIGHT || selected == BOTTOM_LEFT;
	}

	public static boolean checkEdge(int selected) {
		return selected == MOVE_LEFT || selected == MOVE_TOP
				|| selected == MOVE_RIGHT || selected == MOVE_BOTTOM;
	}
	...
	public static boolean checkValid(int selected) {
		return selected == MOVE_NONE || checkBlock(selected)
				|| checkEdge(selected) || checkCorner(selected);
	}
	...
	private static RectF fixedCornerResize(RectF r, int moving_corner,
			float dx, float dy) {
		RectF newCrop = null;
