...
public class BoundedRect {
	...
	public void moveInner(float dx, float dy) {
		...
		float[] translatedInnerCorners = CropMath
		...
		for (int i = 0; i < translatedInnerCorners.length; i += 2) {
			...
			float correctedInnerY = translatedInnerCorners[i + 1]
					+ correction[1];
			if (!CropMath.inclusiveContains(outer, correctedInnerX,
					correctedInnerY)) {
				float[] badCorner = { correctedInnerX, correctedInnerY };
				float[] nearestSide = CropMath.closestSide(badCorner,
						outerCorners);
				float[] correctionVec = GeometryMath
	...
	public void fixedAspectResizeInner(RectF newInner) {
		...
		for (int i = 0; i < newInnerCorners.length; i += 2) {
			...
			if (!CropMath.inclusiveContains(outer, c0[0], c0[1])) {
				...
				float[] l1 = { newInnerCorners[i], newInnerCorners[i + 1],
	...
	private boolean isConstrained() {
		for (int i = 0; i < 8; i += 2) {
			if (!CropMath.inclusiveContains(outer, innerRotated[i],
					innerRotated[i + 1]))
				return false;
		}
		return true;
