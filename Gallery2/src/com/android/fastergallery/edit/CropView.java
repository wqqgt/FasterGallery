...
public class CropView extends View {
	...
	public CropView(Context context, AttributeSet attrs) {
		...
		mShadow = (NinePatchDrawable) rsc
				.getDrawable(R.drawable.geometry_shadow);
	...
	public void initialize(Bitmap image, RectF newCropBounds,
			RectF newPhotoBounds, int rotation) {
		mBitmap = image;
		if (mCropObj != null) {
	...
	public void onDraw(Canvas canvas) {
		...
		if (mDisplayMatrix.mapRect(mScreenImageBounds)) {
			...
			mShadowBounds.set(mShadowBounds.left - margin, mShadowBounds.top
					- margin, mShadowBounds.right + margin,
					mShadowBounds.bottom + margin);
		...
		if (mDisplayMatrix.mapRect(mScreenCropBounds)) {
			...
			if (!mDoSpot) {
				...
				CropDrawingUtils.drawWallpaperSelectionFrame(canvas,
						mScreenCropBounds, mSpotX, mSpotY, wpPaint, p);
			}
			CropDrawingUtils.drawIndicators(canvas, mCropIndicator,
