...
public class CropMath {
	...
	public static void getEdgePoints(RectF imageBound, float[] array) {
		...
		for (int x = 0; x < array.length; x += 2) {
			array[x] = GeometryMath.clamp(array[x], imageBound.left,
					imageBound.right);
			array[x + 1] = GeometryMath.clamp(array[x + 1], imageBound.top,
					imageBound.bottom);
		}
	}
	...
	public static float[] closestSide(float[] point, float[] corners) {
		...
		for (int i = 0; i < len; i += 2) {
			...
			float mag = GeometryMath.vectorLength(GeometryMath
