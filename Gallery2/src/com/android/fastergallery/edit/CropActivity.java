...
public class CropActivity extends Activity {
	...
	private static final int FLAG_CHECK = DO_SET_WALLPAPER | DO_RETURN_DATA
			| DO_EXTRA_OUTPUT;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		actionBar.setCustomView(R.layout.cropshow_actionbar);
	...
	private void doneLoadBitmap(Bitmap bitmap, RectF bounds, int orientation) {
		...
		if (bitmap != null && bitmap.getWidth() != 0 && bitmap.getHeight() != 0) {
			RectF imgBounds = new RectF(0, 0, bitmap.getWidth(),
					bitmap.getHeight());
	...
	private class LoadBitmapTask extends AsyncTask<Uri, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Uri... params) {
			Uri uri = params[0];
			Bitmap bmap = CropLoader.getConstrainedBitmap(uri, mContext,
					mBitmapSize, mOriginalBounds);
	...
	private void startFinishOutput() {
		...
		if ((flags & FLAG_CHECK) != 0 && mOriginalBitmap != null) {
			RectF photo = new RectF(0, 0, mOriginalBitmap.getWidth(),
					mOriginalBitmap.getHeight());
			RectF crop = getBitmapCrop(photo);
			startBitmapIO(flags, mOriginalBitmap, mSourceUri, destinationUri,
					crop, photo, mOriginalBounds, (mCropExtras == null) ? null
							: mCropExtras.getOutputFormat(), mOriginalRotation);
			return;
		}
		setResult(RESULT_CANCELED, new Intent());
		done();
	...
	private void startBitmapIO(int flags, Bitmap currentBitmap, Uri sourceUri,
			Uri destUri, RectF cropBounds, RectF photoBounds,
			RectF currentBitmapBounds, String format, int rotation) {
		if (cropBounds == null || photoBounds == null || currentBitmap == null
				|| currentBitmap.getWidth() == 0
				|| currentBitmap.getHeight() == 0 || cropBounds.width() == 0
				|| cropBounds.height() == 0 || photoBounds.width() == 0
		...
		if ((flags & DO_SET_WALLPAPER) != 0) {
			Toast.makeText(this, R.string.setting_wallpaper, Toast.LENGTH_LONG)
					.show();
		}

		final View loading = findViewById(R.id.loading);
		loading.setVisibility(View.VISIBLE);
		BitmapIOTask ioTask = new BitmapIOTask(sourceUri, destUri, format,
				flags, cropBounds, photoBounds, currentBitmapBounds, rotation,
				mOutputX, mOutputY);
	...
	private class BitmapIOTask extends AsyncTask<Bitmap, Void, Boolean> {
		...
		public BitmapIOTask(Uri sourceUri, Uri destUri, String outputFormat,
				int flags, RectF cropBounds, RectF photoBounds,
				RectF originalBitmapBounds, int rotation, int outputX,
				int outputY) {
			...
			if ((flags & DO_EXTRA_OUTPUT) != 0) {
				if (mOutUri == null) {
					...
					try {
						...
						Log.w(LOGTAG,
								"cannot write file: " + mOutUri.toString(), e);
					}
				}
			}
		...
		protected Boolean doInBackground(Bitmap... params) {
			...
			if (mCrop != null && mPhoto != null && mOrig != null) {
				RectF trueCrop = CropMath.getScaledCropBounds(mCrop, mPhoto,
						mOrig);
				Matrix m = new Matrix();
				...
				if (trueCrop != null) {
					...
					mResultIntent
							.putExtra(CropExtras.KEY_CROPPED_RECT, rounded);
				}
			}
			...
			if ((mFlags & DO_RETURN_DATA) != 0) {
				...
				if (ret == null) {
					Log.w(LOGTAG,
							"could not downsample bitmap to return in data");
					failure = true;
				} else {
					if (mRotation > 0) {
						Matrix m = new Matrix();
						m.setRotate(mRotation);
						Bitmap tmp = Bitmap.createBitmap(ret, 0, 0,
								ret.getWidth(), ret.getHeight(), m, true);
						if (tmp != null) {
							ret = tmp;
						}
					}
			...
			if ((mFlags & (DO_EXTRA_OUTPUT | DO_SET_WALLPAPER)) != 0
				...
				if (mOutputX > 0 && mOutputY > 0) {
					Matrix m = new Matrix();
					RectF cropRect = new RectF(0, 0, crop.getWidth(),
							crop.getHeight());
					if (mRotation > 0) {
						m.setRotate(mRotation);
						m.mapRect(cropRect);
					...
					m.setRectToRect(cropRect, returnRect,
					...
					Matrix m = new Matrix();
					m.setRotate(mRotation);
					Bitmap tmp = Bitmap.createBitmap(crop, 0, 0,
							crop.getWidth(), crop.getHeight(), m, true);
					if (tmp != null) {
						crop = tmp;
					}
				}
				...
				if (mFlags == DO_EXTRA_OUTPUT) {
					if (mOutStream == null
							|| !crop.compress(cf, DEFAULT_COMPRESS_QUALITY,
									mOutStream)) {
						...
						if ((mFlags & DO_SET_WALLPAPER) != 0
							...
							if (mWPManager == null) {
								...
								try {
									...
									Log.w(LOGTAG,
											"cannot write stream to wallpaper",
											e);
									failure = true;
								}
							}
	...
	protected static Bitmap getCroppedImage(Bitmap image, RectF cropBounds,
			RectF photoBounds) {
		RectF imageBounds = new RectF(0, 0, image.getWidth(), image.getHeight());
		RectF crop = CropMath.getScaledCropBounds(cropBounds, photoBounds,
				imageBounds);
		if (crop == null) {
			return null;
		}
		...
		return Bitmap.createBitmap(image, intCrop.left, intCrop.top,
				intCrop.width(), intCrop.height());
	}
	...
	protected static Bitmap getDownsampledBitmap(Bitmap image, int max_size) {
		if (image == null || image.getWidth() == 0 || image.getHeight() == 0
			...
			throw new IllegalArgumentException(
					"Bad argument to getDownsampledBitmap()");
		}
		int shifts = 0;
		...
		Bitmap ret = Bitmap.createScaledBitmap(image,
				image.getWidth() >> shifts, image.getHeight() >> shifts, true);
		if (ret == null) {
			return null;
		}
		// Handle edge case for rounding.
		if (CropMath.getBitmapSize(ret) > max_size) {
			return Bitmap.createScaledBitmap(ret, ret.getWidth() >> 1,
					ret.getHeight() >> 1, true);
		}
		return ret;
	}
		...
		if (extras != null) {
			return new CropExtras(extras.getInt(CropExtras.KEY_OUTPUT_X, 0),
					...
					extras.getBoolean(CropExtras.KEY_SCALE, true)
							&& extras.getBoolean(
									CropExtras.KEY_SCALE_UP_IF_NEEDED, false),
					extras.getInt(CropExtras.KEY_ASPECT_X, 0), extras.getInt(
							CropExtras.KEY_ASPECT_Y, 0), extras.getBoolean(
							CropExtras.KEY_SET_AS_WALLPAPER, false),
	...
	protected static CompressFormat convertExtensionToCompressFormat(
			String extension) {
	...
	private RectF getBitmapCrop(RectF imageBounds) {
		...
		RectF scaledCrop = CropMath.getScaledCropBounds(crop, photo,
				imageBounds);
		return scaledCrop;
	}
}
